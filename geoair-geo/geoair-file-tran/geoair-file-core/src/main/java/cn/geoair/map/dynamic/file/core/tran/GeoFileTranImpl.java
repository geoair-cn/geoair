...
public class GeoFileTranImpl implements GeoFileTran {
		...
		try {
			...
			while ((oneRow = reader.readOneRow(this::handleException)) != null) {
				...
				if (totalCount.get() % context.getBatchLogThreshold() == 0) {
					log.info("转换进度：已处理 {} 条，成功 {} 条，失败 {} 条，成功率 {:.2f}%", totalCount.get(), successCount.get(),
							failCount.get(), (double) successCount.get() / totalCount.get() * 100);
				}
			}
			...
			if (status == TranStatus.RUNNING) {
				...
				result.setTotalCount(totalCount.get()).setSuccessCount(successCount.get()).setFailCount(failCount.get())
						.calculateSuccessRate().setStartTime(startTime).setEndTime(System.currentTimeMillis())
	...
	private void updateProgress() {
		...
		TranProgress progress = new TranProgress().setTotalCount(totalCount.get()).setSuccessCount(successCount.get())
				.setFailCount(failCount.get()).calculateSuccessRate()
				.setElapsedTime(System.currentTimeMillis() - startTime).setStatus(status).setMessage("转换中...");
