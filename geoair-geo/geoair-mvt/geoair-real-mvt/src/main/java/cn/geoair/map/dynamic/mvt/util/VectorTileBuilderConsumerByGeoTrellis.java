...
//
//
// private VectorTileBuilderConsumerByGeoTrellis(Envelope envelope, String layerName, int
// outGridSrid) {
// this.extent = new Extent(envelope.getMinX(), envelope.getMinY(), envelope.getMaxX(),
// envelope.getMaxY());
...
// */
// public static VectorTileBuilderConsumerByGeoTrellis create(Envelope envelope, String
// layerName, int outGridSrid) {
...
// scala.collection.immutable.Map<String, Value> prop = new
...
// Tuple2<String, Value> kv = new Tuple2<>(key, new
// VInt64(Integer.parseInt(pro.toString())));
// prop = prop.$plus(kv);
// } else if (pro instanceof Double) {
// Tuple2<String, Value> kv = new Tuple2<>(key, new
// VDouble(Double.parseDouble(pro.toString())));
// prop = prop.$plus(kv);
// } else if (pro instanceof Float) {
// Tuple2<String, Value> kv = new Tuple2<>(key, new
// VFloat(Float.parseFloat(pro.toString())));
// prop = prop.$plus(kv);
// } else if (pro instanceof Boolean) {
// Tuple2<String, Value> kv = new Tuple2<>(key, new
// VBool(Boolean.parseBoolean(pro.toString())));
// prop = prop.$plus(kv);
// } else if (pro instanceof Long) {
// Tuple2<String, Value> kv = new Tuple2<>(key, new
// VInt64(Long.parseLong(pro.toString())));
// prop = prop.$plus(kv);
// }
// }
//
...
// tempMultiLineList.add(new MVTFeature<>(Option.apply(0L), (MultiLineString) geom,
// prop));
...
// Seq<MVTFeature<Point>> mvtPntSeq =
// JavaConverters.asScalaIteratorConverter(tempPntList.iterator()).asScala().toSeq();
// Seq<MVTFeature<MultiPoint>> mvtMultiPntSeq =
// JavaConverters.asScalaIteratorConverter(tempMultipntList.iterator()).asScala().toSeq();
// Seq<MVTFeature<LineString>> mvtLineSeq =
// JavaConverters.asScalaIteratorConverter(tempLineList.iterator()).asScala().toSeq();
// Seq<MVTFeature<MultiLineString>> mvtMultilineSeq =
// JavaConverters.asScalaIteratorConverter(tempMultiLineList.iterator()).asScala().toSeq();
// Seq<MVTFeature<Polygon>> mvtPolygonSeq =
// JavaConverters.asScalaIteratorConverter(tempPolyList.iterator()).asScala().toSeq();
// Seq<MVTFeature<MultiPolygon>> mvtMultpolygonSeq =
// JavaConverters.asScalaIteratorConverter(tempMultPolyList.iterator()).asScala().toSeq();
//
...
// scala.collection.mutable.Map<String, Layer> scalaLayers =
// JavaConverters.mapAsScalaMapConverter(layers).asScala();
// scala.collection.immutable.HashMap<String, Layer> m = new
// scala.collection.immutable.HashMap<>();
