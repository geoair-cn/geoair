 ...
 * 空间处理相关操作接口
 * <p>
 * 核心功能：提供基于MyBatis的空间数据查询、空间字段类型识别、空间字段信息提取等核心能力， 专注于处理包含地理几何信息（点、线、面等）的数据查询与解析。
 * </p>
...
public interface IAdvGeoPreOpt extends IAdvGeoOpt {
	 ...
	 * <p>
	 * 适用场景：明确查询结果仅返回一行，且需要自动处理所有空间字段的场景（如查询单条空间记录详情）。
	 * </p>
	 * @param sqlStatement 待执行的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 ...
	 * <p>
	 * 适用场景：仅需处理结果中某一个特定空间字段的场景（如仅提取单条记录的几何中心点字段）。
	 * </p>
	 * @param sqlStatement 待执行的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 ...
	 * <p>
	 * 适用场景：需要精准处理结果中多个指定空间字段的场景（如同时提取点坐标字段和面边界字段）。
	 * </p>
	 * @param sqlStatement 待执行的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
	 * @param advEnumsGeomOpt 空间字段处理策略枚举
	 ...
	 * <p>
	 * 适用场景：查询多条空间记录，需要批量处理所有空间字段的场景（如查询某区域内的所有点位记录）。
	 * </p>
	 * @param sqlStatement 待执行的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
	 * @param advEnumsGeomOpt 空间字段处理策略枚举
	 ...
	 * <p>
	 * 适用场景：批量查询记录，仅需处理某一个特定空间字段的场景（如批量提取多条记录的面边界字段）。
	 * </p>
	 * @param sqlStatement 待执行的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
	 * @param advEnumsGeomOpt 空间字段处理策略枚举
	 ...
	 * <p>
	 * 适用场景：批量查询记录，需要精准处理多个指定空间字段的场景（如批量提取点坐标和所属面边界字段）。
	 * </p>
	 * @param sqlStatement 待执行的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
	 * @param advEnumsGeomOpt 空间字段处理策略枚举
	 ...
	 * <p>
	 * 适用场景：快速判断SQL查询结果是否包含空间字段，以及该字段的类型（无需指定字段名）。
	 * </p>
	 * @param sqlStatement 待解析的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
	 ...
	 * @throws RuntimeException SQL语法错误、MyBatis参数绑定异常、数据库连接异常时抛出运行时异常
	 */
	Map<String, AdvEnumsTypeGeom> eGetGeoTypeBySql(String sqlStatement, SqlParamMap sqlParam,
			List<String> geomFieldNames);

	/**
	 * 判断SQL查询语句的结果是否包含空间字段
	 * <p>
	 * 适用场景：前置校验SQL是否返回空间数据，避免后续空间处理逻辑空跑。
	 * </p>
	 * @param sqlStatement 待校验的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
	 ...
	 * <p>
	 * 适用场景：需要批量处理所有空间字段时，先获取完整的空间字段名称列表。
	 * </p>
	 * @param sqlStatement 待解析的SQL查询语句，使用MyBatis风格占位符（#{参数名}）传递参数 sqlParam
	 * SQL语句的参数映射表，无参数时传空Map或null
