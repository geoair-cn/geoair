 ...
 *
 * <p>SQL Server 使用分号参数区，例如 {@code ;databaseName=db;encrypt=true}，
 * 与使用 {@code ?key=value} 的 PostgreSQL/MySQL 明确区分。</p>
 *
 * @author 张逢吉
 */
public class SqlServerJdbcUrlDialect implements JdbcUrlDialect {
    @Override public DatabaseType getDatabaseType() { return DatabaseType.SQLSERVER; }
    @Override public String getSchemaPropertyName() { return "schemaName"; }
    @Override public boolean supports(String jdbcUrl) { return jdbcUrl.regionMatches(true, 0, "jdbc:sqlserver://", 0, 17); }

    @Override
    public JdbcUrl parse(String jdbcUrl) {
        JdbcUrlDialectSupport.ParsedTail tail = JdbcUrlDialectSupport.splitTail(jdbcUrl, JdbcUrl.PropertyStyle.SEMICOLON);
        ...
        return new JdbcUrl(jdbcUrl, DatabaseType.SQLSERVER, "sqlserver", null, tail.coreUrl,
                JdbcUrlDialectSupport.parseEndpoints(authority), databaseName, tail.style, tail.properties);
    }
    ...
    public JdbcUrl create(String host, Integer port, String databaseName) {
        ...
        return new JdbcUrl(core, DatabaseType.SQLSERVER, "sqlserver", null, core,
