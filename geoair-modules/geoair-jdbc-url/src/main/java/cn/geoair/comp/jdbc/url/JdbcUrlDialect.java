 ...
 *
 * <p>每个数据库驱动使用独立实现，禁止由一个通用正则猜测所有 URL 格式。</p>
 *
 * @author 张逢吉
 */
public interface JdbcUrlDialect {
     ...
     * 获取此方言服务的统一数据库类型。
     *
     * <p>编解码器使用该类型在创建 URL、读取 schema 参数和选择方言时建立关联。</p>
     *
     ...
     *
     * <p>实现应只依据稳定的协议前缀或明确特征判断，不应通过宽泛正则猜测其它驱动的 URL。</p>
     *
     ...
     *
     * <p>解析结果应保留参数顺序及参数分隔风格；对驱动私有且无法安全拆分的片段，应保留在连接主体中。</p>
     *
     ...
     *
     * <p>例如 Oracle 默认构建 Thin/SID 格式，SQL Server 将数据库名写入分号参数。
     * 额外驱动参数由 {@link JdbcUrlCodec#withProperty(JdbcUrl, String, String)} 追加。</p>
     *
     ...
     *
     * <p>典型值为 PostgreSQL 的 {@code currentSchema}、Oracle 的 {@code defaultSchema}、
     * SQL Server 的 {@code schemaName} 和 H2 的 {@code schema}。</p>
     *
