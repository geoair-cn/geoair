 ...
 * Phoenix JDBC URL 方言。
 *
 * <p>Phoenix 使用 {@code jdbc:phoenix:host:port}，没有通用数据库名或 schema 参数区。
 *
 * @author 张逢吉
 */
public class PhoenixJdbcUrlDialect implements JdbcUrlDialect {
    ...
    public boolean supports(String jdbcUrl) {
        return jdbcUrl.regionMatches(true, 0, "jdbc:phoenix:", 0, 13);
    }
    ...
    public JdbcUrl parse(String jdbcUrl) {
        ...
        List<JdbcEndpoint> endpoints =
                parts.length == 0 || JdbcUrlDialectSupport.isBlank(parts[0])
                        ? Collections.<JdbcEndpoint>emptyList()
                        : Collections.singletonList(
                                new JdbcEndpoint(
                                        parts[0],
                                        parts.length > 1
                                                ? JdbcUrlDialectSupport.parsePort(parts[1])
                                                : null));
        return new JdbcUrl(
                jdbcUrl,
