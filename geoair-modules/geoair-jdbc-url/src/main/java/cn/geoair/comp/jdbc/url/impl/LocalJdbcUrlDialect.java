 ...
 *
 * <p>用于 H2、SQLite 等不必拥有 host/port 的连接。H2 的配置项采用分号分隔，
 * SQLite 的常见参数采用查询参数分隔。</p>
 *
 * @author 张逢吉
 */
public class LocalJdbcUrlDialect implements JdbcUrlDialect {
    ...
    public LocalJdbcUrlDialect(DatabaseType databaseType, String driverName, String schemaPropertyName) {
    ...
    @Override public boolean supports(String jdbcUrl) {
        return jdbcUrl.regionMatches(true, 0, "jdbc:" + driverName + ":", 0, driverName.length() + 6);
    }

    @Override
    public JdbcUrl parse(String jdbcUrl) {
        JdbcUrl.PropertyStyle preferredStyle = databaseType == DatabaseType.H2
                ? JdbcUrl.PropertyStyle.SEMICOLON : JdbcUrl.PropertyStyle.QUERY;
        JdbcUrlDialectSupport.ParsedTail tail = JdbcUrlDialectSupport.splitTail(jdbcUrl, preferredStyle);
        ...
        return new JdbcUrl(jdbcUrl, databaseType, driverName, null, tail.coreUrl,
                Collections.<JdbcEndpoint>emptyList(), databaseName, tail.style, tail.properties);
    }
    ...
    public JdbcUrl create(String host, Integer port, String databaseName) {
        ...
        return new JdbcUrl(core, databaseType, driverName, null, core,
                Collections.<JdbcEndpoint>emptyList(), localDatabase,
                databaseType == DatabaseType.H2 ? JdbcUrl.PropertyStyle.SEMICOLON : JdbcUrl.PropertyStyle.QUERY,
