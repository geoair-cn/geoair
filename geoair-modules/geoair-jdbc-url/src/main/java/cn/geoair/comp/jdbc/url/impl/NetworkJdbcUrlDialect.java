 ...
 *
 * <p>用于 PostgreSQL、MySQL 与达梦。它支持逗号分隔的多主机地址，并保留查询参数原始顺序。</p>
 *
 * @author 张逢吉
 */
public class NetworkJdbcUrlDialect implements JdbcUrlDialect {
    ...
    public NetworkJdbcUrlDialect(DatabaseType databaseType, String driverName, String schemaPropertyName) {
        this.databaseType = databaseType;
        this.driverName = driverName;
        this.schemaPropertyName = schemaPropertyName;
    }

    @Override public DatabaseType getDatabaseType() { return databaseType; }
    @Override public String getSchemaPropertyName() { return schemaPropertyName; }
    @Override public boolean supports(String jdbcUrl) {
        return jdbcUrl.regionMatches(true, 0, "jdbc:" + driverName + "://", 0, driverName.length() + 8);
    }
    ...
    public JdbcUrl parse(String jdbcUrl) {
        ...
        return new JdbcUrl(jdbcUrl, databaseType, driverName, null, tail.coreUrl,
                JdbcUrlDialectSupport.parseEndpoints(connection.substring(0, slash)), databaseName,
                tail.style, tail.properties);
    }

    @Override
    public JdbcUrl create(String host, Integer port, String databaseName) {
        String core = "jdbc:" + driverName + "://" + JdbcUrlDialectSupport.endpoint(host, port)
                + "/" + (databaseName == null ? "" : databaseName);
        return new JdbcUrl(core, databaseType, driverName, null, core,
