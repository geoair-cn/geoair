...
class DefaultJdbcUrlCodecTest {
    ...
    void shouldParseAndRewritePostgreSqlUrl() {
        String original =
                "jdbc:postgresql://db1:5432,db2:5433/gis?currentSchema=old&sslmode=require";
        ...
        Assertions.assertEquals(
                "jdbc:postgresql://db1:5432,db2:5433/gis?currentSchema=geo+data&sslmode=require",
    ...
    void shouldKeepOracleSidAndServiceNameForms() {
        ...
        Assertions.assertEquals(
                "jdbc:oracle:thin:@db1:1521:ORCL?defaultSchema=NEW",
    ...
    void shouldUseSemicolonPropertiesForSqlServerAndH2() {
        ...
        Assertions.assertEquals(
                "jdbc:sqlserver://sql:1433;databaseName=gis;encrypt=true;schemaName=gis_schema",
                codec.rewriteSchema(sqlServer, "gis_schema"));
        Assertions.assertEquals(
                "jdbc:h2:./data/gis;MODE=PostgreSQL;schema=GIS", codec.rewriteSchema(h2, "GIS"));
    }

    @Test
    void shouldBuildLegacySupportedUrls() {
        Assertions.assertEquals(
                "jdbc:mysql://mysql:3306/gis",
                codec.format(codec.create(DatabaseType.MYSQL, "mysql", 3306, "gis")));
        Assertions.assertEquals(
                "jdbc:sap://hana:30015/?databaseName=SYSTEMDB",
                codec.format(codec.create(DatabaseType.SAP_HANA, "hana", 30015, "SYSTEMDB")));
        Assertions.assertEquals(
                "jdbc:phoenix:zookeeper:2181",
                codec.format(codec.create(DatabaseType.PHOENIX, "zookeeper", 2181, null)));
    }
    ...
    void shouldUseHutoolDialectAndDriverMapping() {
        ...
        Assertions.assertEquals(
                DatabaseType.SQLSERVER, DatabaseType.fromDialectName(DialectName.SQLSERVER));
    }
