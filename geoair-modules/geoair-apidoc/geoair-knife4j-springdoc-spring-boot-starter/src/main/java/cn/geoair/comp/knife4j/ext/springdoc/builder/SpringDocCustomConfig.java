		...
		return openApi -> {
			...
			openApi.getPaths().values().forEach(pathItem -> {
				...
				pathItem.readOperations().forEach(operation -> {
					Object handlerMethodObj = operation.getExtensions() == null ? null
					...
					GaApiAction gaApiAction = AnnotationUtils.findAnnotation(handlerMethod.getMethod(),
							GaApiAction.class);
					if (gaApiAction != null) {
						// ========== 核心修复7：校验text属性非空 ==========
		...
		return (operation, handlerMethod) -> {
			...
			if (gaApiAction != null) {
				...
				operation.setOperationId(
						handlerMethod.getBeanType().getSimpleName() + "_" + handlerMethod.getMethod().getName());
			}
