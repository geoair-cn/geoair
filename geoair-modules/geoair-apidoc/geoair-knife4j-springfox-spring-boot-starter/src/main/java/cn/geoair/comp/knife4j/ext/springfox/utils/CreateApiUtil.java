...
public class CreateApiUtil {
    ...
    public static Docket createGroup(ApiModelInfo apiModelInfo, DocketInfo docketInfo) {
        ...
        if (docketInfo.getModelscan() != null
                && !docketInfo.getModelscan().equals("")
                && (docketInfo.getModelClassList() == null
                        || docketInfo.getModelClassList().equals(""))) {
            // 如果存在扫描参数 modelscan 则添加扫描additionalModels
            springAddtionalModel = SpringAddtionalModelUtils.scan(docketInfo.getModelscan());
        } else if (docketInfo.getModelClassList() != null
                && !docketInfo.getModelClassList().equals("")
                && (docketInfo.getModelscan() == null && docketInfo.getModelscan().equals(""))) {
            springAddtionalModel =
                    SpringAddtionalModelUtils.listClass(docketInfo.getModelClassList());
        } else if (docketInfo.getModelClassList() != null
                && !docketInfo.getModelClassList().equals("")
                && docketInfo.getModelscan() != null
                && !docketInfo.getModelscan().equals("")) {
            springAddtionalModel =
                    SpringAddtionalModelUtils.scanAndClassList(
                            docketInfo.getModelClassList(), docketInfo.getModelscan());
        }
        ...
        if (springAddtionalModel == null) {
            ...
            return new Docket(DocumentationType.SWAGGER_2)
                    ...
                    .useDefaultResponseMessages(false)
                    .additionalModels(
                            springAddtionalModel.getFirst(), springAddtionalModel.getRemaining())
                    .select()
                    .apis(RequestHandlerSelectors.basePackage(docketInfo.getBasePackage()))
                    .paths(specifyScan)
                    .build();
