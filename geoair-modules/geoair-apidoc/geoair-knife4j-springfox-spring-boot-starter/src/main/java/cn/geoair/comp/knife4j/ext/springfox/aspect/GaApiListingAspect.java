...
public class GaApiListingAspect {
    ...
    public Object typeResolverAspect(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (args[0] instanceof ApiListingContext) {
            ...
            Optional<? extends Class<?>> controller =
                    apiListingContext.getResourceGroup().getControllerClass();
            if (controller.isPresent()) {
                ...
                Set<String> tagSet =
                        apiAnnotation != null ? extractApiTags(apiAnnotation) : new TreeSet<>();

                // ========== GTC自定义@GaApi注解处理 ==========
                GaApi gaApiAnnotation =
                        AnnotationUtils.findAnnotation(controller.get(), GaApi.class);
                String descriptiongtc = gaApiAnnotation != null ? gaApiAnnotation.text() : null;
                descriptiongtc =
                        (descriptiongtc == null || descriptiongtc.trim().isEmpty())
                                ? null
                                : descriptiongtc;

                Set<String> taggtcSet =
                        gaApiAnnotation != null
                                ? extractGaApiTags(gaApiAnnotation)
                                : new TreeSet<>();
                ...
                if (uniontags.isEmpty()) {
                    ...
                    apiListingContext
                            ...
                            // 描述优先使用GTC注解，保持原有逻辑
                            .description(
                                    GutilStr.isBlank(descriptiongtc) ? description : descriptiongtc)
                            .tagNames(uniontags);
                }
            }
        }
    ...
    private Set<String> extractApiTags(Api api) {
        ...
        return Arrays.stream(api.tags())
                .filter(tag -> tag != null && !emptyTags().test(tag))
