...
public class TypeResolverAspect implements ApplicationContextAware {
    ...
    public Object typeResolverAspect(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (proceed instanceof ResolvedInterfaceType) {
            ...
            if (GutilClass.isAssignable(erasedType, GiResult.class)) {
                ...
                return ResolvedObjectType.create(
                        aClass,
    ...
    /** 设置父类的泛型 */
    private ResolvedObjectType setSuperClassTypeBind(
            ResolvedObjectType resolvedObjectType, TypeBindings typeBindings) {
        ...
        if (GutilClass.isNormalClass(parentClass.getErasedType())) {
            ResolvedObjectType newparentClass =
                    ResolvedObjectType.create(
                            parentClass.getErasedType(),
                            typeBindings,
                            parentClass.getParentClass(),
                            parentClass.getImplementedInterfaces());
            // 递归设置该父类的父类的泛型，直到到达getParentClass（）的值为null为止
            ResolvedObjectType newNewParentClass =
                    setSuperClassTypeBind(newparentClass, typeBindings);
            return ResolvedObjectType.create(
                    resolvedObjectType.getErasedType(),
                    typeBindings,
                    newNewParentClass,
