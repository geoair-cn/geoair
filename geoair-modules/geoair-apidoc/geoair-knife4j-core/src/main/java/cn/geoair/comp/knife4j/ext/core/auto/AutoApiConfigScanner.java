...
public class AutoApiConfigScanner extends GirOpenApiConfig {
	...
	private List<String> getControllerRootPackages() {
		...
		// 优先级1：手动配置的多根包
		String[] multiPackages = environment.getProperty("geoair.apidoc.controllerRootPackages", String[].class,
				new String[0]);
	...
	private Set<String> extractAndFilterPackages(Set<Class<?>> classes) {
		...
		// 过滤排除包
		String[] excludePackages = environment.getProperty("geoair.apidoc.excludePackages", String[].class,
				new String[0]);
		...
		return allPackages.stream().filter(pkg -> !excludeList.contains(pkg)) // 精准匹配排除
				.collect(Collectors.toSet());
	}
	...
	private List<String> sortPackages(Set<String> packages) {
		...
		String[] fixedOrderPackages = environment.getProperty("geoair.apidoc.fixedOrderPackages", String[].class,
				new String[0]);
