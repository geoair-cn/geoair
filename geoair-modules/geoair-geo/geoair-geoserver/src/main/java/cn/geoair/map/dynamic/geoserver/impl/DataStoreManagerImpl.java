...
public class DataStoreManagerImpl implements DataStoreManager {
	...
	/** 创建或获取数据源（核心实现） */
	@Override
	public DataStoreInfo getOrCreateDataStore(WorkspaceInfo workspace, BaseDatastore baseDatastore) {
		try {
			// 1. 参数校验
			...
			// 2. 检查数据源是否已存在
			DataStoreInfo existingStore = catalog.getDataStoreByName(workspace, baseDatastore.getName());
			...
			throw new ServicePublishException("创建/获取数据源 [" + baseDatastore.getName() + "] 失败：" + e.getMessage(), e);
		}
	}
	...
	public void deleteDataStore(WorkspaceInfo workspace, String dataStoreName) {
		try {
			...
			if (!catalog.getFeatureTypesByDataStore(dataStore).isEmpty()) {
				throw new ServicePublishException("数据源 [" + dataStoreName + "] 下存在图层，无法删除，请先删除关联图层");
			}
			...
			throw new ServicePublishException("删除数据源 [" + dataStoreName + "] 失败：" + e.getMessage(), e);
		}
	}
