...
public class WorkspaceManagerImpl implements WorkspaceManager {
    ...
    @Autowired private WorkspaceConvertUtil workspaceConvertUtil;
    ...
    public WorkspaceInfo getOrCreateWorkspace(String workspaceName, String namespaceUri) {
        try {
            ...
            throw new ServicePublishException(
                    "创建/获取工作区 [" + workspaceName + "] 失败：" + e.getMessage(), e);
        }
    }
    ...
    public void deleteWorkspace(String workspaceName) {
        try {
            ...
            if (!catalog.getDataStoresByWorkspace(workspace).isEmpty()) {
                throw new ServicePublishException(
                        "工作区 [" + workspaceName + "] 下存在数据源，无法删除，请先删除关联数据源");
            }
            ...
            throw new ServicePublishException(
                    "删除工作区 [" + workspaceName + "] 失败：" + e.getMessage(), e);
        }
    }
    ...
    public NamespaceInfo createNamespace(
        ...
        try {
            ...
            throw new ServicePublishException(
                    "创建命名空间 [" + namespacePrefix + "] 失败：" + e.getMessage(), e);
        }
    }
}
