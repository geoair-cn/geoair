...
public class PostgisLayerPublisher implements LayerPublisher {
	...
	@Override
	public LayerInfo publishLayer(WorkspaceInfo workspace, DataStoreInfo dataStore, String layerName, String srs,
			String styleName) {
		try {
			...
			throw new ServicePublishException("发布 PostGIS 图层 " + layerName + " 失败：" + e.getMessage(), e);
		}
	}

	@Override
	public List<LayerInfo> batchPublishLayers(WorkspaceInfo workspace, DataStoreInfo dataStore, List<String> layerNames,
	...
	public void enableLayerService(LayerInfo layer, OgcServiceType serviceType, boolean enabled) {
		try {
			...
			throw new ServicePublishException(
					"修改图层 " + layer.getName() + " 的 " + serviceType.getDesc() + " 状态失败：" + e.getMessage(), e);
		}
	}
	...
	@Override
	public String getLayerServiceUrl(WorkspaceInfo workspace, String layerName, OgcServiceType serviceType) {
		String baseUrl = "http://localhost:8080/geoserver/" + workspace.getName() + "/"
				+ serviceType.getCode().toLowerCase();
		return baseUrl + "?service=" + serviceType.getCode() + "&version=1.1.0&request=GetCapabilities&layers="
				+ layerName;
	}
