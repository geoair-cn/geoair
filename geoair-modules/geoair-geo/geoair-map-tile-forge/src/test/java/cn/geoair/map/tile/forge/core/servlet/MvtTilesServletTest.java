...
public class MvtTilesServletTest {
    ...
    public void shouldKeepRelativeBaseUrlWhenRequestHostIsNotProvided() {
        String style =
                renderStyle(
                        "https://tile.example.com:8443/mvtTilesService/id/file/layer/style.json?version=1",
                        null);
    ...
    public void shouldPreferExplicitHostAndNeverDuplicateAbsoluteUrlOrigin() {
        ...
        Assert.assertEquals(
                "{\"url\":\"https://maps.example.com/mvtTilesService/id/file/layer/{z}/{x}/{y}.pbf\"}",
                style);
    }
    ...
    public void shouldKeepExistingProviderImplementationCompatible() {
        ...
        TileResponseProvider provider =
                new TileResponseProvider() {
                    @Override
        ...
        Assert.assertSame(
                expected, provider.getTileResponse("/tiles/1/2/3", "https://maps.example.com"));
    }
    ...
    private String renderStyle(String requestUri, String requestHost) {
        ...
        tileRequest.setBytes(
                "{\"url\":\"{BASE_URL}/{z}/{x}/{y}.pbf\"}".getBytes(StandardCharsets.UTF_8));
        tileRequest.setSize(tileRequest.getBytes().length);
        tileRequest.setExists(true);
        TileParseResult parseResult =
                TileParseResult.of().setRequestURI(requestUri).setRequestHost(requestHost);
