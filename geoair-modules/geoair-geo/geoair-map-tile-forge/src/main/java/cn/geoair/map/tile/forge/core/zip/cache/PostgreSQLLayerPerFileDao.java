...
public class PostgreSQLLayerPerFileDao implements LayerPerFileDao, AutoCloseable {
    ...
    public enum CacheStatus {
        NOT_CACHED, CACHING, CACHED, CACHE_FAILED
    }
    ...
    private void initCacheStatus() throws SQLException {
        ...
        String checkSql = String.format("SELECT 1 FROM %s WHERE layer_name = #{dataId}", CACHE_STATUS_TABLE);
        Number exists = iAdvExecutor.bSelectNumber(checkSql, SqlParamMap.of().addOne("dataId", dataId));
        ...
        if (exists == null || exists.intValue() == 0) {
            ...
            iAdvExecutor.bInsertBySql(insertSql,
                    SqlParamMap.of()
    ...
    public CacheStatus getCacheStatus() throws SQLException {
        String sql = String.format("SELECT cache_status FROM %s WHERE layer_name = #{dataId}", CACHE_STATUS_TABLE);
        GirAdvOneRow girAdvOneRow = iAdvExecutor.bSelectOne(sql, SqlParamMap.of().addOne("dataId", dataId));
    ...
    public void insert(TileCentralDirectoryModel entry) throws SQLException {
        ...
        iAdvExecutor.bInsertBySql(sql, SqlParamMap.of()
    ...
    public void batchInsert(List<TileCentralDirectoryModel> entries) throws SQLException {
        ...
        List<String> headers = ListUtil.of(
                "local_header_offset",
                "data_offset",
                "compression_method",
                "compressed_size",
