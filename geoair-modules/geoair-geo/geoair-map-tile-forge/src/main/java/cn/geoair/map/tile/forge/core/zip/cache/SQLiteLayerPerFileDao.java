...
public class SQLiteLayerPerFileDao implements LayerPerFileDao, AutoCloseable {
    ...
    private String getDbFilePath() {
        String format =
                String.format(
                        "%s%s%s_tiles.sqlite",
                        baseDir, File.separator, layerName.replaceAll("[^a-zA-Z0-9_]", "_"));
    ...
    private void initCacheStatus() throws SQLException {
        String checkSql =
                String.format("SELECT 1 FROM %s WHERE layer_name = ?", CACHE_STATUS_TABLE);
        Connection connection = getConnection();
    ...
    public CacheStatus getCacheStatus() throws SQLException {
        ...
        String sql =
                String.format(
                        "SELECT cache_status FROM %s WHERE layer_name = ?", CACHE_STATUS_TABLE);
        Connection connection = getConnection();
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public CacheStatusInfo getCacheStatusInfo() throws SQLException {
        init();
        String sql = String.format("SELECT * FROM %s WHERE layer_name = ?", CACHE_STATUS_TABLE);
        Connection connection = getConnection();
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, layerName);
            try (ResultSet rs = stmt.executeQuery()) {
    ...
    // 私有辅助方法
    private void setParameters(PreparedStatement stmt, TileCentralDirectoryModel entry)
            throws SQLException {
