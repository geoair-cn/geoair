...
/**
 * 本地解压XYZ瓦片存储支持类 用于处理ArcGIS瓦片数据的读取操作
 *
 * @author 张俊
 * @since 2025/11/17
 */
public class LocalUnzippedXYZTileStorageSupport implements ITileStorageSupport {
    ...
    /**
     * 根据图层配置和瓦片坐标获取瓦片数据 检查本地是否存在对应瓦片文件，如果存在则直接读取
     *
    ...
    public TileRequest getTileData(
        ...
        String inLocalPath =
                getTilePath(layerConfigContext.getObjectKey(), z, y, x, format, File.separator);
    ...
    /**
     * 构建瓦片文件的完整路径 根据图层配置、瓦片坐标和格式信息拼接成本地文件路径
     *
     ...
     */
    protected String getTilePath(
            String baseDir, String z, String y, String x, String format, String separator) {
