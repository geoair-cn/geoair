 ...
 *
 * <p>内部使用 {@link GirProgressReporter} 按步长节流（默认每 1% 回调一次）， 通过 {@link GiProgressListener}
 * 在日志中打印进度条与百分比，便于任务执行时观察进度。
 *
 * <p>用法：直接实现 {@link ProgressConsumer} 使用，或将其作为 {@link GiProgressReporter} 的 简化替代（两者都实现了 {@link
 * cn.geoair.base.percent.GiProgressReporter}）。
 *
 * @author ：张俊
...
public class LogProgressConsumer implements ProgressConsumer {
    ...
    private final GirProgressReporter percentReporter =
            new GirProgressReporter(
                    1,
                    new GiProgressListener() {
                        ...
                        public void onUpdate(Number percent) {
                            if (log.isInfoEnabled()) {
                                log.info(
                                        "当前进度: {}%  {}",
                                        percent,
                                        GutilPercent.getProgressDisplay(percent.doubleValue()));
                            }
                        }
                    });
