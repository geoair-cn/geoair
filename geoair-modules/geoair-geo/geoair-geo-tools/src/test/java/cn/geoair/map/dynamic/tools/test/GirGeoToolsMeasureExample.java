...
public class GirGeoToolsMeasureExample {
    ...
    public static void main(String[] args) {
        ...
        double areaKm2 = measureOpt.calculateArea(
                polygon, 4326, MeasureUnitEnum.SQUARE_KILOMETER, MeasureMethodEnum.WEB_MERCATOR);
        double areaByUtm = measureOpt.calculateArea(
                polygon, 4326, MeasureUnitEnum.SQUARE_METER, MeasureMethodEnum.UTM);
        double lengthKm = measureOpt.calculateLength(
                line, 4326, MeasureUnitEnum.KILOMETER, MeasureMethodEnum.WEB_MERCATOR);
        double lengthByUtm = measureOpt.calculateLength(
                line, 4326, MeasureUnitEnum.METER, MeasureMethodEnum.UTM);
        double pointDistance = measureOpt.calculatePointToPointDistance(
                point1, point2, 4326, MeasureUnitEnum.METER, MeasureMethodEnum.GEODETIC);
        double pointToLine = measureOpt.calculateGeometryToGeometryMinDistance(
                point1, line, 4326, MeasureUnitEnum.METER, MeasureMethodEnum.UTM);
        double converted = measureOpt.convertUnit(
                1000, MeasureUnitEnum.METER, MeasureUnitEnum.KILOMETER);
