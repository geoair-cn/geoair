 ...
 *
 * <p>除显式传入 {@code ifExceptionReturnNull=true} 的方法外，转换失败会抛出运行时异常。 相同 SRID 的转换直接返回原 {@link Geometry}
 * 或 {@link Envelope} 对象，而非副本。
 *
 * @author 张逢吉
 * @date 2024/12/05
 */
public interface GirSridConvertOpt {
     ...
     * 包围盒 SRID 转换。
     *
     * <p>该方法转换包围盒，不保留原始几何边界形状；若需要精确投影后的边界，请先转换几何对象。 相同 SRID 时返回原 {@code Envelope} 对象。
     *
     ...
     * 判断坐标参考系是否为地理坐标参考系。
     *
     * <p>返回 {@code true} 表示坐标轴以角度为单位；返回 {@code false} 表示其不是地理 CRS， 但不保证投影坐标单位一定为米。未知或无法解析的 SRID
     * 会抛出异常，而不会猜测结果。
     *
     * @param srid EPSG SRID
     * @return 是否为地理坐标参考系
     ...
     *
     * <p>不会清理 CRS 类型判断缓存；常用 SRID 的预加载转换会在下次使用时按需重新创建。
     */
     ...
     *
     * <p>实现会缓存并复用已创建的算子；该方法名是历史 API，并非每次都重新创建。 算子按宽松模式创建，以兼容轻微的 CRS 定义差异。
     *
