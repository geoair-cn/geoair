...
public class GirGeoMeasureUtils implements GirGeoMeasureOpt {
	...
	public double calculateAreaByUTM(double[][] coords, int srid, String unit) {
		try {
			// 构建多边形几何对象
			Coordinate[] coordinates = Arrays.stream(coords).map(coord -> new Coordinate(coord[0], coord[1]))
	...
	public double calculateLengthByUTM(Geometry geometry, int srid, String unit) {
		try {
			validateGeometryType(geometry, new String[] { "LineString", "MultiLineString", "Polygon", "MultiPolygon" });
			// 转换为UTM投影坐标系计算高精度长度
			Geometry utmGeom = convertToUTMCRS(geometry, srid);
			double lengthInM;
	...
	@Override
	public double calculatePointToPointDistance(double[] point1, double[] point2, int srid, String unit) {
		try {
	...
	@Override
	public double calculatePointToLineMinDistance(Point point, LineString line, int srid, String unit) {
		try {
	...
	@Override
	public double calculatePointToLineMinDistance(double[] point, double[][] lineCoords, int srid, String unit) {
		try {
			Point p = GEOMETRY_FACTORY.createPoint(new Coordinate(point[0], point[1]));
			Coordinate[] lineCoordinates = Arrays.stream(lineCoords).map(coord -> new Coordinate(coord[0], coord[1]))
					.toArray(Coordinate[]::new);
	...
	@Override
	public double calculatePointToGeometryDistance(Point point, Geometry geometry, int srid, String unit) {
		try {
			validateGeometryType(point, new String[] { "Point" });
	...
	@Override
	public double calculateLineToLineMinDistance(LineString line1, LineString line2, int srid, String unit) {
		try {
	...
	@Override
	public double calculateLineToLineMinDistance(double[][] line1Coords, double[][] line2Coords, int srid,
			String unit) {
		try {
			Coordinate[] line1CoordsArr = Arrays.stream(line1Coords).map(coord -> new Coordinate(coord[0], coord[1]))
					.toArray(Coordinate[]::new);
			LineString line1 = GEOMETRY_FACTORY.createLineString(line1CoordsArr);

			Coordinate[] line2CoordsArr = Arrays.stream(line2Coords).map(coord -> new Coordinate(coord[0], coord[1]))
					.toArray(Coordinate[]::new);
	...
	private boolean isAreaUnit(String unit) {
		return StrUtil.equalsAny(unit, UNIT_SQUARE_METER, UNIT_SQUARE_KILOMETER, UNIT_ACRE, UNIT_HECTARE);
	}
	...
	private void validateGeometryType(Geometry geometry, String[] allowedTypes) {
		...
		if (!isAllowed) {
			throw new IllegalArgumentException("不支持的几何类型：" + geomType + "，允许类型：" + Arrays.toString(allowedTypes));
		}
	}
