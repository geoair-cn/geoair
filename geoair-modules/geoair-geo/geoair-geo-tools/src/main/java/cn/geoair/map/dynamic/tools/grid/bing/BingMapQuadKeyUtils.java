...
public class BingMapQuadKeyUtils implements GirBingMapQuadKeyOpt {
		...
		for (String quadKey : quadKeys) {
			...
			if (targetLevel > currentZ) {
				throw new IllegalArgumentException(
						String.format("QuadKey[%s]级别为%d，小于指定截断级别%d", quadKey, currentZ, targetLevel));
			}
	...
	public static void main(String[] args) {
		...
		Gir.log.info("TileZxyApo(3,3,5) 转换到Z=4：" + String.join(", ", targetKeysByZxy));
	}
