...
public class GirTileResponseUtil {
     ...
     */
    public static void buildFromTileResponse(TileResponse tileResponse, HttpServletResponse response, Integer cacheMaxAge) throws IOException {
        ...
        // 设置响应头
        response.setContentType(tileResponse.getMimeType().getFormat());
        ...
        try (ServletOutputStream outputStream = response.getOutputStream()) {
            IoUtil.copy(new ByteArrayInputStream(finalBytes), outputStream);
            outputStream.flush();
        }
     ...
     * 处理瓦片不存在
     */
    private static void handleNotFound(HttpServletResponse response) throws IOException {
