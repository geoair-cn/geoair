 ...
 * 几何对象与坐标数组互转核心接口
 *
 * <p>定义 Point / LineString / Polygon 与 double 类型数组、List集合 的互相转换规范； 支持自定义坐标顺序（X在前 / Y在前），支持自定义
 * GeometryFactory； 提供常规转换（带校验）和快速转换（跳过数值校验，高性能）两种模式。 所有 {@code double} 数组 API 均为二维 XY 表达，转换三维或带 M 值
 * Geometry 时会丢弃额外维度。
 *
 * @author 张逢吉
 * @date 2024/12/06
...
public interface GirGeom2ArrayOpt {
     ...
     *
     * <p>自动识别类型： 一维数组 → Point 二维数组 → LineString
     *
