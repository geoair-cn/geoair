 ...
 * 达梦数据库空间类型转换工具类。
 *
 * <p>将达梦 {@code DmdbStruct}（Gserialized 格式）转换为 JTS Geometry。 转换链：DmdbStruct → Blob → Gserialized
 * bytes → WKB bytes → JTS Geometry。
 *
 * <p>调用方应先通过 {@link GirDMTran#isDmDriverAvailable()} 确认驱动可用。
 *
 * @author zhangjun
 */
public class GirDMSpatialTran {
     ...
     *
     * <p>反射调用链：DmdbStruct.getAttributes() → Blob → byte[] (Gserialized) →
     ...
     *
     * <p>反射调用 {@code com.dameng.geotools.util.DmGeo2Util.wkbFromGser(byte[], int)}。
     *
