...
public class AdvMvtTileUtils {
    ...
    public static TileExecParams getTileExecParamsNotHasSql(
            int zoom, int x, int y, int sourceGrid, int sourceDataSrid) {
        ...
        tileExecParams
                .setGridSrid(sourceGrid)
                .setGridExtent(gridExtent)
                .setSourceDataSrid(sourceDataSrid);
        // 3. 处理坐标系转换
        if (!ObjectUtil.equals(sourceGrid, sourceDataSrid)) {
            ...
            dataExtentBox =
                    GirGeoTools.defaultInstance()
                            .getSridOpt()
                            .convert(gridExtentBox, sourceGrid, sourceDataSrid, false);
            if (dataExtentBox == null) {
                Geometry geometry =
                        GirGeoTools.defaultInstance().getSridOpt().convertToGeom(gridExtentBox);
                throw new RuntimeException(
                        StrUtil.format(
                                "网格计算异常：z:{}  x :{} y:{} geometry:{}  gridSrid :{} sourceDataSrid:{} ",
                                zoom,
                                x,
                                y,
                                geometry.toText(),
        ...
        tileExecParams
                .setDataExtentBox(dataExtentBox)
                .setGridExtentBox(gridExtentBox)
                .setDataExtent(dataExtent);
        initExtentBuffer(
                gridExtentBox, gridExtentBufferEnvelope::set, gridExtentBufferBoxGeom::set);
        initExtentBuffer(
                dataExtentBox, dataExtentBufferEnvelope::set, dataExtentBufferBoxGeom::set);
        tileExecParams
                .setDataExtentBufferBoxGeom(dataExtentBufferBoxGeom.get())
     ...
     */
    public static void initExtentBuffer(
            Envelope originalExtentBox,
            Consumer<Envelope> setBufferEnvelope,
