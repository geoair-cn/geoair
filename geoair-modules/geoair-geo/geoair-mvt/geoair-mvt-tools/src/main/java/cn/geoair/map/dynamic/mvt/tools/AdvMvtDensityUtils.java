...
public class AdvMvtDensityUtils {
	 ...
	 * @return 合并后的要素列表
	 */
	public static List<GirAdvOneRow> doCoalesceBySpatialDensity(List<GirAdvOneRow> featureList, int limit,
			String geomFieldName, String idFieldName, int gridSrid) {
		// 1. 前置判断：若要素数未超限，直接返回
		if (featureList.size() <= limit) {
			return featureList;
		}
		double GRID_SCALE = 100.0;
		if (GirAdvTools.getSridOpt().isGeographicCRS(gridSrid)) {
			GRID_SCALE = 10.0;
		}
		...
		for (GirAdvOneRow feature : featureList) {
			try {
				...
				gridDensityMap.put("error#error", gridDensityMap.getOrDefault("error#error", 0) + 1);
			}
		}

		// 3. 计算密度阈值（仅合并密度超过阈值的网格）
		// 阈值 = 平均密度 * 1.5
		double avgDensity = gridDensityMap.values().stream().mapToInt(Integer::intValue).average().orElse(1.0);
	 ...
	 * @return
	 */
	public static List<GirAdvOneRow> coalescePointFeatures(List<GirAdvOneRow> featureList, int coalescePixel,
			double coalesceGeo, int z, PbfTileParameter parameter) {

		if (featureList == null || featureList.isEmpty() || coalescePixel <= 0 || coalesceGeo <= 0) {
		...
		for (GirAdvOneRow row : featureList) {
			...
			// 统计该格网的总点数
			CoalesceDistanceStat stat = aggKeyStatMap.computeIfAbsent(aggKey, k -> new CoalesceDistanceStat());
			stat.countTotal();
	 ...
	 * @return 合并后的要素列表
	 */
	public static List<GirAdvOneRow> doFilterBySpatialDensity(List<GirAdvOneRow> featureList, int limit,
			String geomFieldName, String idFieldName, int gridSrid) {
		...
		Collections.sort(featureWithGridList, new Comparator<FeatureWithGrid>() {
			@Override
	 ...
	 */
	public static PbfInfo doValidateAndOptimizeTileSize(String tileId, PbfInfo pbfData, List<GirAdvOneRow> featureList,
			Envelope envelope, int zoom, PbfTileParameter parameter) throws Exception {
		...
		Long tileSizeLimit = Optional.ofNullable(parameter.getTileSizeLimitByte()).orElse(512000L); // 500KB
		...
		log.info("瓦片[" + tileId + "]原始大小：" + pbfData.getData().length + "B，超过限制：" + tileSizeLimit + "B，执行二次优化");
		PbfInfo optimizedPbf = pbfData;
		List<GirAdvOneRow> optimizedFeatureList = new ArrayList<>(featureList);

		// 4.1 第一步：提升简化级别（每次+1，最多提升3级）
		int originSimplifyLevel = Optional.ofNullable(copyParameter.getSimplificationLevel()).orElse(0);
		...
		for (int level = originSimplifyLevel + 1; level <= maxSimplifyLevel; level++) {
			...
			optimizedPbf = doGetPbfByteByGeotools(optimizedFeatureList, envelope, zoom, copyParameter);
			if (optimizedPbf != null && optimizedPbf.getData().length <= tileSizeLimit) {
				log.info("瓦片[" + tileId + "]优化后大小：" + optimizedPbf.getData().length + "B（提升简化级别至" + level + "）");
				// 恢复原始简化级别
		...
		if (copyParameter.isDropDensestAsNeeded() && copyParameter.getFeatureLimit() != null) {
			...
			for (int i = 0; i < 3; i++) {
				...
				// 重新聚合要素并生成瓦片
				List<GirAdvOneRow> filteredList = doFilterBySpatialDensity(optimizedFeatureList, newLimit,
						copyParameter.getGeomFieldName(), copyParameter.getIdFieldName(), parameter.getOutGridSrid());
				optimizedPbf = doGetPbfByteByGeotools(filteredList, envelope, zoom, copyParameter);
				if (optimizedPbf != null && optimizedPbf.getData().length <= tileSizeLimit) {
					log.info("瓦片[" + tileId + "]优化后大小：" + optimizedPbf.getData().length + "B（要素数限制降至" + newLimit + "）");
					// 恢复原始要素数限制
		...
		log.info("瓦片[" + tileId + "]优化失败，大小仍为" + optimizedPbf.getData().length + "B，超过限制" + tileSizeLimit + "B ");
		// }
		// 恢复原始参数
	 ...
	 * @throws Exception
	 */
	public static PbfInfo doGetPbfByteByGeotools(List<GirAdvOneRow> rs, Envelope envelope, int zoom,
			PbfTileParameter pbfParam) throws Exception {
		int simplifyLevel = Optional.ofNullable(pbfParam.getSimplificationLevel()).orElse(DEFAULT_SIMPLIFICATION_LEVEL);
		int coalescePixel = Optional.ofNullable(pbfParam.getCoalesceDistance()).orElse(DEFAULT_COALESCE_DISTANCE);
		...
		// 地理坐标系（4326）纬度修正
		if (GirAdvTools.getSridOpt().isGeographicCRS(srid)) {
		...
		for (Map<String, Object> feature : rs) {
			...
			for (Map.Entry<String, Object> entry : entries) {
				...
				// 保留用户指定字段
				if (!pbfParam.getIncludeFields().isEmpty() && pbfParam.getIncludeFields().contains(entry.getKey())) {
			...
			if (pbfParam.isOnly()) {
				...
				PipelineBuilder pipelineBuilder = PipelineBuilder.newBuilder(envelope, pbfParam.getOutGridSrid());
				...
				if (PPbfType.rootPbf.equals(pPbfType)) {
					...
					if (pbfParam.isCreateLabel() && StrUtil.isNotBlank(pbfParam.getLayerNameLabel())) {
						...
						if (centroid != null) { // 校验几何对象非空
							...
							encoderLabel.addFeature(pbfParam.getLayerNameLabel(), featureCopy, transformLabel);
						}
						else {
					...
					// 边界图层逻辑（增加前置校验）
					if (pbfParam.isCreateBoundary() && StrUtil.isNotBlank(pbfParam.getLayerNameBoundary())) {
						...
						if (boundary != null && !boundary.isEmpty()) { // 校验边界非空且非空几何
							...
							encoderBoundary.addFeature(pbfParam.getLayerNameBoundary(), featureCopy, transformBoundary);
						}
						else {
		...
		PbfInfo pbfInfo = new PbfInfo().setData(gZip(encoder.encode())).setZoom(zoom)
	 ...
	 * @return 更新后的合并几何映射
	 */
	private static Map<String, Geometry> mergePointGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom,
		...
		try {
			...
			if (mergedGeomMap.containsKey(key)) {
				...
				MultiPoint newMultiPoint = factory.createMultiPoint(pointList.toArray(new Point[0]));
	 ...
	 * @return 更新后的合并几何映射
	 */
	private static Map<String, Geometry> mergeMultiPointGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom,
			GirAdvOneRow feature) {
		try {
	 ...
	 * @return 更新后的合并几何映射
	 */
	private static Map<String, Geometry> mergeLineStringGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom,
			GirAdvOneRow feature) {
		try {
			...
			if (mergedGeomMap.containsKey(key)) {
				...
				if (unionGeom instanceof LineString) {
					GeometryFactory factory = new GeometryFactory();
					multiLineString = factory.createMultiLineString(new LineString[] { (LineString) unionGeom });
				}
				else {
				...
				GeometryFactory factory = new GeometryFactory();
				MultiLineString multiLineString = factory.createMultiLineString(new LineString[] { lineString });
				mergedGeomMap.put(key, multiLineString);
			}
		}
		catch (Exception e) {
	 ...
	 * @return 更新后的合并几何映射
	 */
	private static Map<String, Geometry> mergeMultiLineStringGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom,
			GirAdvOneRow feature) {
		try {
			...
			if (mergedGeomMap.containsKey(key)) {
				...
				if (unionGeom instanceof LineString) {
					GeometryFactory factory = new GeometryFactory();
					newMultiLineString = factory.createMultiLineString(new LineString[] { (LineString) unionGeom });
				}
				else {
	 ...
	 * @return 更新后的合并几何映射
	 */
	private static Map<String, Geometry> mergePolygonGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom,
			GirAdvOneRow feature) {
		try {
	...
	// 获取要素ID（用于排序）
	private static String getFeatureId(GirAdvOneRow feature, String geomFieldName, String idFieldName) {
		try {
	...
	private static double calculateDynamicTolerance(int srid, int zoom, Envelope extent, int simplifyLevel) {
