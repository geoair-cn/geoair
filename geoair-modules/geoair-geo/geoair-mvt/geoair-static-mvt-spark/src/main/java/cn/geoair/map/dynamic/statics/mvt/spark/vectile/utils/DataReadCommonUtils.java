...
public class DataReadCommonUtils {
	...
	public static List<String> buildBboxPartitionConditions(BBoxApo bBoxApo, int maxPartionNum, int sourceSrid) {
		...
		for (int i = 0; i < numPerSide; i++) {
			for (int j = 0; j < numPerSide; j++) {
				...
				partitionConditions
						.add(String.format("%s,%s,%s,%s", xminPartition, xmaxPartition, yminPartition, ymaxPartition));
			}
		}
	 ...
	 * 构建BBox查询SQL
	 */
	public static String buildBboxQuerySql(String queryStatement, String geomFieldName, double xmin, double ymin,
			double xmax, double ymax, int sourceSrid) {
	 ...
	 * 构建BBox SQL函数
	 */
	public static String getBufferBboxSqlFunction(double xmin, double ymin, double xmax, double ymax, int targetGrid) {
		return StrUtil.format("public.ST_MakeEnvelope({}, {}, {}, {}, {})", xmin, ymin, xmax, ymax, targetGrid);
	}
