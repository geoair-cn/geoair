 ...
 *
 * <p>相对于原版 {@code SparkVectorTileGenerator} 的优化：
 *
 * <ul>
 *   <li>使用 {@link
 *       cn.geoair.map.dynamic.statics.mvt.spark.vectile.utils.SparkTaskSerializableUtil.MapToTileFunction}
 *       （TileIterator 懒生成）替代 MapToTileFunction1（HashMap 全量收集）
 *   <li>去掉 transform 阶段的 persist（一对一映射，不需要回溯）
 *   <li>去掉 tileFeatures 的 persist（流式传递到 reduceByKey）
 *   <li>去掉 tileFeatures.count()（避免强制物化膨胀数据）
 *   <li>仅保留 rawFeatures 的 persist（唯一一次缓存）
 *   <li>通过 {@link ProgressTracker} 实现基于 SparkListener 的进度跟踪
 * </ul>
 *
 * @see ProgressTracker
...
public class SparkVectorTileGeneratorV2 implements Serializable {
     ...
     * @param percentReporter 进度上报回调（可为 null），在 executor 线程中直接调用， report(allCount, currentCount)。内部用
     *     Serializable 包装避免闭包序列化失败
     */
    public void doGenerate(TileSliceParameter parameter, GiProgressReporter percentReporter)
            throws Exception {
        // ==================== 初始化进度跟踪 ====================
    ...
    private void runStatisticsPipeline(
        ...
        JavaPairRDD<String, List<GirAdvOneRow>> tileFeaturesByZoom =
                transformedFeatures.flatMapToPair(
                        new SparkTaskFunctions.MapToTileFunctionSingleZoom(
                ...
                (VoidFunction<Iterator<Tuple2<String, List<GirAdvOneRow>>>>)
                        partitionIterator -> {
                            ...
                            try {
                                ...
                                long partitionElapsed =
                                        System.currentTimeMillis() - partitionStartTime;
                                double tps =
                                        partitionElapsed > 0
                                                ? rootTotalCount.get() * 1000.0 / partitionElapsed
                                                : 0;
                                log.info(
                                        "{} :分区内Root表最终完成，累计写入瓦片数：{} ,outGridSrid:{},edition:{},速度: {} 瓦片/秒",
                                        rootTableName,
                                        rootTotalCount.get(),
                                        outGridSrid,
                                        edition,
