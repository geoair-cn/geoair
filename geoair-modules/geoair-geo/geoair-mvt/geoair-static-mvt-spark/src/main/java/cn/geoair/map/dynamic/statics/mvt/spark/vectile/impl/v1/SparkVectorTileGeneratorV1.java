...
public class SparkVectorTileGeneratorV1 implements Serializable {
    ...
    public SparkVectorTileGeneratorV1(SparkSession sparkSession) {
...
//        Map<String, String> pgParams = VectorTileCommonUtils.buildPgWriteParams(parameter);
        ...
        JavaRDD<GirAdvOneRow> transformedFeatures =
                persistedFeaturesRDD.map(
                                new V1SparkTaskFunctions.TransformFeatureFunction(parameter))
        ...
        tileFeatures =
                transformedFeatures.flatMapToPair(
                                new V1SparkTaskFunctions.MapToTileFunction1(parameter))
                        .persist(StorageLevel.MEMORY_AND_DISK());
        log.info("=================================================");
        ...
        if (parameter.isStatisticsEnabled()) {
            JavaPairRDD<String, List<GirAdvOneRow>> tileFeaturesByZoom =
                    transformedFeatures.flatMapToPair(
                            new V1SparkTaskFunctions.MapToTileFunctionToStatic(
            ...
            JavaPairRDD<String, List<GirAdvOneRow>> aggregatedRDDByZoom =
                    tileFeaturesByZoom.reduceByKey(
                            new V1SparkTaskFunctions.AggregateAndLimitFeatureFunction(copy),
            ...
            JavaRDD<Tuple2<String, PbfInfo>> pbfRDD =
                    aggregatedRDDByZoom.map(
                            new V1SparkTaskFunctions.GeneratePbfFunction(parameter, instance));
        ...
        JavaPairRDD<String, List<GirAdvOneRow>> aggregatedRDD =
                tileFeatures.reduceByKey(
                        new V1SparkTaskFunctions.AggregateAndLimitFeatureFunction(parameter),
                        DEFAULT_REDUCE_PARTITION);
        ...
        aggregatedRDD.foreachPartition(
                (VoidFunction<Iterator<Tuple2<String, List<GirAdvOneRow>>>>)
                        partitionIterator -> {
                            ...
                            try {
                                V1SparkTaskFunctions.GeneratePbfFunction pbfFunc =
                                        new V1SparkTaskFunctions.GeneratePbfFunction(parameter, PbfTargetInfo.newInstance());
    ...
    private JavaRDD<GirAdvOneRow> readDataByIdPage(TileSliceParameter parameter) throws Exception {
        ...
        List<Integer> pageNumbers =
                V1DataReadUtils.buildPageNumberList(totalCount, maxPartionNum);
        ...
        JavaRDD<GirAdvOneRow> featureRDD =
                pageNumRdd.flatMap(
                        new V1SparkTaskFunctions.IdPageFlatMapFunction(
                                parameter,
    ...
    private JavaRDD<GirAdvOneRow> readDataByBBox(TileSliceParameter parameter) throws Exception {
        ...
        List<String> partitionConditions =
                V1DataReadUtils.buildBboxPartitionConditions(
        ...
        JavaRDD<GirAdvOneRow> featureRDD =
                partitionConditionRdd.flatMap(
                        new V1SparkTaskFunctions.BboxFlatMapFunction(
                                parameter,
                                parameter.getQueryStatement(),
    ...
    private Row buildRow(
        ...
        if (isLabel) {
            ...
            return new V1SparkTaskFunctions.BuildRowLabelFunction(parameter).call(tuple);
        } else if (isBoundary) {
            if (tuple._2.getDataBoundary() == null) return null;
            return new V1SparkTaskFunctions.BuildRowBoundaryFunction(parameter).call(tuple);
        } else {
            if (tuple._2.getData() == null) return null;
            return new V1SparkTaskFunctions.BuildRowFunction(parameter).call(tuple);
        }
    }
