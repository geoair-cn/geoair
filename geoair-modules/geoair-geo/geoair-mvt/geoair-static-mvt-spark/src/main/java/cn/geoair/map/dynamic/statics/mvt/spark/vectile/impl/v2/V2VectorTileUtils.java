 ...
 * V2 版本专用的瓦片工具类 — 内存优化版。
 * <p>
 * 替代原版 {@link cn.geoair.map.dynamic.statics.mvt.spark.vectile.utils.VectorTileCommonUtils}
 * 中的聚合和截断方法，核心改进：
 * <ul>
 *   <li>{@link #aggregateTileFeatures} 使用 {@code addAll} 替代 {@code Stream.concat().collect()}，
 *       合并时只创建一份新 List，避免三倍内存开销</li>
 *   <li>{@link #limitTileFeatures} 始终有硬上限兜底（10000），即使用户未开启 featureLimitEnabled，
 *       也不会让高密度瓦片无限累积要素</li>
 * </ul>
 *
...
public class V2VectorTileUtils implements Serializable {
    ...
    /**
     * 硬上限：无论用户是否配置 featureLimit，单个瓦片最多保留此数量的要素。
     * 用 CPU 换内存 —— 多余的要素在聚合阶段就被丢弃，不会累积到 OOM。
     */
     ...
     * 内存优化的瓦片要素聚合。
     * <p>
     * 原版使用 {@code Stream.concat().collect(Collectors.toList())}，
     * 合并过程中同时持有 list1 + list2 + 新建的 list 三份内存。
     * 本方法使用 {@code ArrayList.addAll()} 直接拷贝底层数组，减少一次中间拷贝。
     */
     ...
     * <p>
     * 原版仅在 {@code featureLimitEnabled=true} 时才截断，默认配置下高密度瓦片可以无限累积。
     * 本方法无论用户配置如何，都保证输出列表不超过 {@link #HARD_LIMIT}。
     */
