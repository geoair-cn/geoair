                        ...
                        partitionIterator -> {
                            ...
                            try {
                                ...
                                while (partitionIterator.hasNext()) {
                                    ...
                                    Tuple2<String, PbfInfo> pbfTuple =
                                            pbfFunc.call(aggregatedTuple);
                                    ...
                                    if (StringUtils.isNotBlank(boundaryTableName)) {
                                        Row boundaryRow =
                                                buildRow(pbfTuple, false, true, parameter);
                                        if (boundaryRow != null) {
                                            ...
                                            if (boundaryBatchRows.size() >= TILE_BATCH_SIZE) {
                                                executeBatchInsert(
                                                        boundaryPstmt, boundaryBatchRows);
                                                boundaryBatchRows.clear();
                                                conn.commit();
                                                System.gc();
                                                log.info(
                                                        "分区内Boundary表批次写入完成，已写入瓦片数：{}",
                                                        boundaryBatchCount.get());
                                            }
                                        }
                                    }
                                ...
                                // 3. Boundary表剩余数据
                                if (StringUtils.isNotBlank(boundaryTableName)
                                        && !boundaryBatchRows.isEmpty()) {
                                    executeBatchInsert(boundaryPstmt, boundaryBatchRows);
                                    conn.commit();
                                    log.info(
                                            "分区内Boundary表剩余批次写入完成，累计写入瓦片数：{}",
                                            boundaryBatchCount.get());
                                }
