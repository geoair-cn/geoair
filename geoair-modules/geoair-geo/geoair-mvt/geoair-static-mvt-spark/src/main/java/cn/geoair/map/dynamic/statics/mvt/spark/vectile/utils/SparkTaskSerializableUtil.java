...
public class SparkTaskSerializableUtil implements Serializable {
	...
	public static class IdPageFlatMapFunction implements FlatMapFunction<Integer, GirAdvOneRow>, Serializable {
		...
		public IdPageFlatMapFunction(TileSliceParameter parameter, String queryStatement, String orderFieldName,
				int countPerTask) {
		...
		public Iterator<GirAdvOneRow> call(Integer pageNum) throws Exception {
			...
			// 构建排序SQL
			String orderSql = iAdvExecutor.pBuildSqlWithOrder(queryStatement,
			...
			// 打印耗时
			log.info("Spark ID分页任务" + pageNum + "耗时：" + (System.currentTimeMillis() - startTime) + "ms，读取条数："
					+ pageFeatures.size());
	...
	public static class BboxFlatMapFunction implements FlatMapFunction<String, GirAdvOneRow>, Serializable {
		...
		public BboxFlatMapFunction(TileSliceParameter parameter, String queryStatement, String geomFieldName,
				int sourceDataSrid) {
			this.parameter = parameter;
			this.queryStatement = queryStatement;
		...
		public Iterator<GirAdvOneRow> call(String partitionCondition) throws Exception {
			...
			// 构建BBox查询SQL
			String bboxQuerySql = DataReadCommonUtils.buildBboxQuerySql(queryStatement, geomFieldName, xmin, ymin, xmax,
					ymax, sourceDataSrid);
	...
	public static class TransformFeatureFunction
			implements Serializable, org.apache.spark.api.java.function.Function<GirAdvOneRow, GirAdvOneRow> {
	...
	public static class AggregateAndLimitFeatureFunction
			implements Function2<List<GirAdvOneRow>, List<GirAdvOneRow>, List<GirAdvOneRow>>, Serializable {
	...
	public static class MapToTileFunction
		...
		public Iterator<Tuple2<String, List<GirAdvOneRow>>> call(GirAdvOneRow feature) throws Exception {
			...
			Map<String, List<GirAdvOneRow>> tileMap = VectorTileCommonUtils.mapSingleFeatureToTiles(feature,
					parameter.getGeomFieldName(), parameter.getMinZoom(), parameter.getMaxZoom(),
	...
	public static class MapToTileFunctionToStatic
		...
		public Iterator<Tuple2<String, List<GirAdvOneRow>>> call(GirAdvOneRow feature) throws Exception {
			...
			Map<String, List<GirAdvOneRow>> tileMap = VectorTileCommonUtils.mapSingleFeatureToTiles(feature,
					parameter.getGeomFieldName(), zoom, zoom, parameter.getOutGridSrid());
			List<Tuple2<String, List<GirAdvOneRow>>> result = new ArrayList<>();
			tileMap.forEach((tileId, list) -> {
				if (StrUtil.isNotBlank(tileId) && list != null && !list.isEmpty()) {
					result.add(new Tuple2<>(tileId, list));
	 ...
	 * PBF生成函数（可序列化）
	 */
	public static class GeneratePbfFunction implements Serializable,
			org.apache.spark.api.java.function.Function<Tuple2<String, List<GirAdvOneRow>>, Tuple2<String, PbfInfo>> {
		...
		public Tuple2<String, PbfInfo> call(Tuple2<String, List<GirAdvOneRow>> tileFeature) throws Exception {
			PbfInfo pbf = null;
			try {
				pbf = VectorTileCommonUtils.generateSingleTilePbf(tileFeature._1, tileFeature._2, parameter,
						pbfTargetInfo);
			}
			catch (Exception e) {
	...
	static int getTmsY(int zoom, int y, int x, int gridSrid) {
		...
		if (gridSrid == 3857) {
			...
			tms_y = GirAdvTools.getTileGrid4326SeparateOpt().reverseY(y, zoom);
		}
		return tms_y;
	}
	...
	public static class BuildRowFunction
			implements Serializable, org.apache.spark.api.java.function.Function<Tuple2<String, PbfInfo>, Row> {
		...
		public Row call(Tuple2<String, PbfInfo> tuple) throws Exception {
			...
			return RowFactory.create(zoom, x, tms_y, y, gridSrid, bytes, parameter.getLayerName(),
	...
	public static class BuildRowBoundaryFunction
		...
		public Row call(Tuple2<String, PbfInfo> tuple) throws Exception {
			...
			int gridSrid = pbfInfo.getGridSrid();
			int tms_y = getTmsY(zoom, y, x, gridSrid);
			return RowFactory.create(zoom, x, tms_y, y, gridSrid, bytes, parameter.getLayerNameBoundary(),
					parameter.getEdition());
	...
	public static class BuildRowLabelFunction
		...
		public Row call(Tuple2<String, PbfInfo> tuple) throws Exception {
			...
			int gridSrid = pbfInfo.getGridSrid();
			int tms_y = getTmsY(zoom, y, x, gridSrid);
			return RowFactory.create(zoom, x, tms_y, y, gridSrid, bytes, parameter.getLayerNameLabel(),
					parameter.getEdition());
