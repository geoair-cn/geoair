...
/**
 * 动态查询接口
 * <p>自动组装WHERE条件的查询，支持链式条件构建、分页、排序等功能</p>
 * <p>使用{@link GirAdvQueryRequest}封装查询参数  </p>
 *
 * @author zhangjun
...
public interface IAdvWhereSelectOpt extends IAdvConfigOpt {
     ...
     * 查询单条记录
     *
     * <p>根据查询条件返回第一条匹配的记录，适用于根据唯一条件查询的场景</p>
     *
     ...
     * 查询多条记录列表
     *
     * <p>根据查询条件返回所有匹配的记录列表，适用于常规列表查询场景</p>
     *
     * @param query 查询请求对象，封装了表名、字段、条件、排序等参数
     ...
     * 分页查询
     *
     * <p>根据查询条件返回分页结果，包含总记录数和当前页数据，适用于列表分页展示场景</p>
     *
     ...
     * 流式查询多条记录
     *
     * <p>通过Consumer逐行处理数据，避免一次性加载大量数据到内存导致OOM，适用于大数据量导出或批处理场景</p>
     * <p>注意：流式查询会保持数据库连接直到所有数据处理完成，使用完毕后会自动释放资源</p>
     *
     * @param query       查询请求对象，封装了表名、字段、条件、排序等参数
     * @param rowConsumer 行数据消费器，用于逐行处理查询结果中的每一条GirAdvOneRow数据
     *                    示例：row -> log.info("数据：" + row.get("id"))
     */
     ...
     * 查询记录总数
     *
     * <p>根据查询条件返回匹配的记录总数，适用于分页查询前获取总记录数</p>
     *
     ...
     * <p>适用于只需要数值、不需要列名映射的场景，如统计报表数据导出</p>
     *
     * @param query 查询请求对象，封装了表名、字段、条件、排序等参数
     ...
     * 执行聚合查询并返回单个数值结果
     *
     * <p>适用于COUNT、SUM、AVG、MAX、MIN等聚合函数查询，返回单个数值结果</p>
     * <p>注意：查询请求中的fields字段应设置为聚合表达式，如"COUNT(*)"、"SUM(amount)"</p>
     *
