 ...
 * <p>
 * 这是 {@code <where>} 和 {@code <set>} 标签的基类，也可直接用于 {@code <trim>} 标签。
 *
 * <p>处理流程：
 * <ol>
 *   <li>在临时上下文中执行子节点，获取原始 SQL</li>
 *   <li>去除 SQL 首部匹配 {@code prefixesToOverride} 的前缀（如 "AND "、"OR "）</li>
 *   <li>去除 SQL 尾部匹配 {@code suffixesToOverride} 的后缀（如 ","）</li>
 *   <li>在非空 SQL 前后添加 {@code prefix} / {@code suffix}</li>
 * </ol>
 *
 * @author zhangjun
 * @see WhereSqlNode
