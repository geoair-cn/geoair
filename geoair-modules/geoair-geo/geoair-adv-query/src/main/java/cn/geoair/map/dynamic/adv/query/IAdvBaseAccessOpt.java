 ...
 * 数据插入相关的基础操作接口
 * <p>
 * 覆盖单条插入、批量插入、自定义SQL插入、忽略重复插入、插入并返回主键等全场景， 适配不同业务场景下的插入需求，保持与MyBatis-Plus风格一致的语义和命名规范
 */
public interface IAdvBaseAccessOpt {
	 ...
	 * 执行自定义插入SQL语句
	 * <p>
	 * 支持任意复杂的INSERT SQL（含多表插入、子查询插入、自定义字段映射等）， 适用于无法通过标准化方法实现的特殊插入场景
	 * @param sqlStatement 自定义插入SQL语句（支持MyBatis标签、参数占位符） <br>
	 * 示例1：INSERT INTO user (id, name, age) VALUES (#{id}, #{name}, #{age}) <br>
	 * 示例2：INSERT INTO dept (name, create_time) SELECT name, now() FROM temp_dept WHERE
	 * status = 1
	 ...
	 * 批量插入Map格式的数据
	 * <p>
	 * 适用于批量插入同结构数据，底层优化为批量SQL（如INSERT INTO ... VALUES (...), (...), ...）， 相比循环单条插入性能提升显著
	 * @param tableName 目标表名
	 ...
	 * @return Integer 成功插入的记录总数
	 */
	Integer bInsertBatchWithBatchSize(String tableName, Set<String> headers, List<Map<String, Object>> rowsData,
			int batchSize);
	 ...
	 * 插入或忽略（存在则跳过，不存在则插入）
	 * <p>
	 * 基于数据库唯一索引/主键实现，避免重复插入，适用于无需更新仅需插入的场景 （PostgreSQL：ON CONFLICT DO
	 * NOTHING；MySQL：INSERT IGNORE INTO）
	 * @param tableName 目标表名
	 * @param rowData 单行数据
