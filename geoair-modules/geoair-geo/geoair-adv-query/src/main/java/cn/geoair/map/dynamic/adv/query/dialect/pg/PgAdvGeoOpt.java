...
public class PgAdvGeoOpt extends AbstractAdvGeoOpt {
	...
	public List<String> eGetAllGeoLayerName() {
		String sqlTemp = "SELECT table_name FROM information_schema.columns " + "WHERE udt_name = 'geometry' {} "
				+ "GROUP BY table_name;";
		String schemaFilter = StrUtil.isEmpty(dataSourceGetter.getSchemaName()) ? ""
				: StrUtil.format("AND \"table_schema\" = '{}'", dataSourceGetter.getSchemaName());
	...
	public Map<String, AdvEnumsTypeGeom> eGetGeoTypeByTable(String tableName, List<String> geomFieldNames) {
		...
		for (int i = 0; i < geomFieldNames.size(); i++) {
			...
			fieldsSql.append(" public.ST_GeometryType(").append(field).append(") AS ").append(field).append("_type");
		...
		String sql = StrUtil.format("SELECT {} FROM {} WHERE {} LIMIT 1;", fieldsSql.toString(), qualifiedTableName,
				whereSql.toString());
	...
	public Map<String, AdvEnumsTypeGeom> eGetGeoTypeBySql(String sqlView, List<String> geomFieldNames) {
		...
		String sql = StrUtil.format("SELECT {} FROM ({}) AS temp WHERE {} LIMIT 1;", fieldsSql.toString(), sqlView,
				whereSql.toString());
		GirAdvOneRow row = baseOpt.bSelectOne(sql);
	...
	public List<String> eGetGeomColumnNameListByTable(String tableName) {
		...
		String tableNameWithoutSchema = dialectTableNameProcessor.tbGetTableNameNotSchema(tableName);
		...
		String schemaFilter = StrUtil.isEmpty(schemaName) ? ""
	...
	public String eGetGeomColumnNameBySql(String sqlView) {
		...
		try {
			String fieldQuerySql = StrUtil.format("SELECT * FROM ({}) AS {} LIMIT 0", sqlView, alias);
			...
			if (metaData != null) {
				for (int i = 1; i <= metaData.getColumnCount(); i++) {
					...
					if ("geometry".equals(colType) || "geography".equals(colType)
	...
	public Integer eGetSrid(String tableNameOrSqlView, String geomFieldName) {
		...
		String qualifiedName = pgAdvDDLOpt.dIsTableExists(tableNameOrSqlView)
				? dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableNameOrSqlView)
				: StrUtil.format("({}) as {}", dialectTableNameProcessor.tbRemoveSqlSpaces(tableNameOrSqlView),
						dialectTableNameProcessor.tbGetTempAliasTableName());

		String sql = StrUtil.format("SELECT public.st_srid({}) AS srid FROM {} LIMIT 1;", geomFieldName, qualifiedName);
		GirAdvOneRow row = baseOpt.bSelectOne(sql);
	...
	public Map<String, Integer> eGetSrid(String tableNameOrSqlView, List<String> geomFieldNames) {
		...
		for (int i = 0; i < geomFieldNames.size(); i++) {
			String field = geomFieldNames.get(i);
			sridSelect.append(StrUtil.format("COALESCE(public.st_srid({}), -1) AS {}_srid", field, field));
		...
		String sql = StrUtil.format("SELECT {} FROM {} {} LIMIT 1;", sridSelect.toString(), qualifiedName, where);
		GirAdvOneRow row = baseOpt.bSelectOne(sql);
	...
	public void eAddGeomColumn(String tableName, String geomFieldName, AdvEnumsTypeGeom geomType, int srid) {
		...
		String qualifiedTableName = dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableName);
		String sql = StrUtil.format("ALTER TABLE {} ADD COLUMN {} geometry({}, {});", qualifiedTableName, geomFieldName,
				geomType.getCode(), srid);
	...
	public void eTransformSrid(String tableName, String geomFieldName, int targetSrid) {
		...
		try {
			// 1. 新建临时字段
			String createTempSql = StrUtil.format("ALTER TABLE {} ADD COLUMN {} geometry({},0);", qualifiedTableName,
					tempGeomField, geomType.name().toLowerCase());
			getAdvDDLOpt().dExecuteDDL(createTempSql, tableName, "新建临时空间字段[" + tempGeomField + "]");

			// 2. 拷贝数据
			String copySql = StrUtil.format("UPDATE {} SET {} =ST_SetSRID({}, {});", qualifiedTableName, tempGeomField,
					geomFieldName, oldSrid);
			...
			String transformSql = StrUtil.format(
					...
					qualifiedTableName, tempGeomField, geomType.name().toLowerCase(), targetSrid, tempGeomField,
					targetSrid);
			getAdvDDLOpt().dExecuteDDL(transformSql, tableName, "转换SRID为" + targetSrid);

			// 4. 重命名原字段
			String renameOldSql = StrUtil.format("ALTER TABLE {} RENAME COLUMN {} TO {};", qualifiedTableName,
					geomFieldName, geomFieldName + "_old_" + IdUtil.simpleUUID().substring(0, 8));
			getAdvDDLOpt().dExecuteDDL(renameOldSql, tableName, "重命名原空间字段");

			// 5. 重命名临时字段
			String renameTempSql = StrUtil.format("ALTER TABLE {} RENAME COLUMN {} TO {};", qualifiedTableName,
					tempGeomField, geomFieldName);
			getAdvDDLOpt().dExecuteDDL(renameTempSql, tableName, "重命名临时字段为原字段名");

			// 6. 删除旧字段
			String dropOldSql = StrUtil.format("ALTER TABLE {} DROP COLUMN {};", qualifiedTableName,
	...
	public void eCreateSpatialIndex(String tableName, String geomFieldName, String indexName) {
		...
		String qualifiedTableName = dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableName);
		String sql = StrUtil.format("CREATE INDEX {} ON {} USING GIST ({});", indexName, qualifiedTableName,
				geomFieldName);
	...
	public String getQueryIntersectsSql(String qualifiedTableName, String geomFieldName, String geometry, int srid) {
		return StrUtil.format("SELECT * FROM {} WHERE public.ST_Intersects({}, public.ST_GeomFromText('{}', {}));",
	...
	public String getQueryWithinBBoxSql(String qualifiedTableName, String geomFieldName, String bboxWkt, int srid) {
		return StrUtil.format("SELECT * FROM {} WHERE public.ST_Within({}, public.ST_GeomFromText('{}', {}));",
				qualifiedTableName, geomFieldName, bboxWkt, srid);
	}

	@Override
	public String getCalculateDistanceSql(String geomFieldName, String geometry, int srid, String distanceAlias,
			String qualifiedTableName) {
		return StrUtil.format("SELECT *, public.ST_Distance({}, public.ST_GeomFromText('{}', {})) AS {} FROM {};",
	...
	public String getCentroidSql(String geomFieldName, String centerAlias, String qualifiedTableName) {
		return StrUtil.format("SELECT *, public.ST_Centroid({}) AS {} FROM {};", geomFieldName, centerAlias,
				qualifiedTableName);
	}

	@Override
	public String getValidateGeometriesSql(String qualifiedTableName, String geomFieldName) {
		return StrUtil.format("SELECT id FROM {} WHERE NOT public.ST_IsValid({});", qualifiedTableName, geomFieldName);
	}

	@Override
	public String getRepairGeometriesSql(String qualifiedTableName, String geomFieldName) {
		return StrUtil.format("UPDATE {} SET {} = public.ST_MakeValid({}) WHERE NOT public.ST_IsValid({});",
	...
	public String getGetExtentSql(String geomFieldName, String qualifiedTableName, int srid) {
		return StrUtil.format("SELECT " + "public.ST_XMin ( extent ) AS minx," + "public.ST_YMin ( extent ) AS miny,"
				...
				geomFieldName, srid, qualifiedTableName);
	}

	@Override
	public Map<String, AdvEnumsTypeGeom> eGetGeoTypeBySql(String sqlStatement, SqlParamMap sqlParam,
		...
		String sql = StrUtil.format("SELECT {} FROM ({}) AS temp WHERE {} LIMIT 1;", fieldsSql.toString(), sqlStatement,
				whereSql.toString());
	...
	public String eGetGeomColumnNameBySql(String sqlStatement, SqlParamMap sqlParam) {
		...
		try {
			String fieldQuerySql = StrUtil.format("SELECT * FROM ({}) AS {} LIMIT 0", sqlStatement, alias);
