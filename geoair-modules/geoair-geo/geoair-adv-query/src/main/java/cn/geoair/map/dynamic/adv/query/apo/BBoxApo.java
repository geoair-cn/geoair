...
/**
 * @author ：张逢吉
 * @date ：Created in 2025/10/11 08:57 @description： 查询边界的Api Object
 */
...
public class BBoxApo implements Serializable {
	...
	/**
	 * 获取带SRID的WKT字符串（如：SRID=4326;POLYGON ((...))） 用于需要显式声明坐标系的场景（如PostGIS空间查询）
	...
	/**
	 * 将4326边界范围转换为WKT字符串（如：POLYGON ((minx miny, maxx miny, maxx maxy, minx maxy, minx
	 * miny))）
	 * @return WKT字符串，若边界无效则返回空字符串
	 */
