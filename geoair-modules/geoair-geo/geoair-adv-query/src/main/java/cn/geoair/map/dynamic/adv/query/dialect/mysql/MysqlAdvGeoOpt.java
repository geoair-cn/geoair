...
public class MysqlAdvGeoOpt extends AbstractExecAdvGeoOpt {
	...
	public List<String> eGetAllGeoLayerName() {
		String sql = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS "
				...
				+ "AND TABLE_SCHEMA = ? " + "GROUP BY TABLE_NAME;";
	...
	public Map<String, AdvEnumsTypeGeom> eGetGeoTypeByTable(String tableName, List<String> geomFieldNames) {
		...
		for (int i = 0; i < geomFieldNames.size(); i++) {
			...
			fieldsSql.append("ST_GeometryType(").append(field).append(") AS ").append(field).append("_type");
		...
		try {
			...
			if (metaData != null) {
				for (int i = 1; i <= metaData.getColumnCount(); i++) {
					...
					// 判断MySQL空间类型
					if (Arrays.asList("GEOMETRY", "POINT", "LINESTRING", "POLYGON", "MULTIPOINT", "MULTILINESTRING",
							"MULTIPOLYGON").contains(colType.toUpperCase())) {
						return colName;
					}
				}
			}
	...
	public Integer eGetSrid(String tableNameOrSqlView, String geomFieldName) {
		...
		String qualifiedName = ddlOpt.dIsTableExists(tableNameOrSqlView)
		...
		String sql = StrUtil.format("SELECT ST_SRID({}) AS srid FROM {} LIMIT 1;", geomFieldName, qualifiedName);
	...
	public void eAddGeomColumn(String tableName, String geomFieldName, AdvEnumsTypeGeom geomType, int srid) {
		...
		String sql = StrUtil.format("ALTER TABLE {} ADD COLUMN {} {} SRID {};", qualifiedTableName, geomFieldName,
				geomType.getCode().toUpperCase(), srid);
	...
	public void eTransformSrid(String tableName, String geomFieldName, int targetSrid) {
		...
		try {
			// 1. 新建临时字段
			String createTempSql = StrUtil.format("ALTER TABLE {} ADD COLUMN {} {} SRID 0;", qualifiedTableName,
					tempGeomField, geomType.getCode().toUpperCase());
			ddlOpt.dExecuteDDL(createTempSql, tableName, "新建MySQL临时空间字段[" + tempGeomField + "]");

			// 2. 拷贝并转换SRID（MySQL: ST_Transform）
			String copySql = StrUtil.format("UPDATE {} SET {} = ST_Transform(ST_SetSRID({}, {}), {});",
					qualifiedTableName, tempGeomField, geomFieldName, oldSrid, targetSrid);
			ddlOpt.dExecuteDDL(copySql, tableName, "拷贝并转换SRID");

			// 3. 修改临时字段SRID
			String alterSridSql = StrUtil.format("ALTER TABLE {} MODIFY COLUMN {} {} SRID {};", qualifiedTableName,
					tempGeomField, geomType.getCode().toUpperCase(), targetSrid);
	...
	public void eCreateSpatialIndex(String tableName, String geomFieldName, String indexName) {
		...
		String sql = StrUtil.format("CREATE SPATIAL INDEX {} ON {} ({});", indexName, qualifiedTableName,
				geomFieldName);
	...
	public String getQueryIntersectsSql(String qualifiedTableName, String geomFieldName, String geometry, int srid) {
		...
		return StrUtil.format("SELECT * FROM {} WHERE ST_Intersects({}, ST_GeomFromText('{}', {}));",
	...
	public String getQueryWithinBBoxSql(String qualifiedTableName, String geomFieldName, String bboxWkt, int srid) {
		// MySQL: ST_Within
		return StrUtil.format("SELECT * FROM {} WHERE ST_Within({}, ST_GeomFromText('{}', {}));", qualifiedTableName,
				geomFieldName, bboxWkt, srid);
	}
	...
	public String getCalculateDistanceSql(String geomFieldName, String geometry, int srid, String distanceAlias,
		...
		return StrUtil.format("SELECT *, ST_Distance({}, ST_GeomFromText('{}', {})) AS {} FROM {};", geomFieldName,
				geometry, srid, distanceAlias, qualifiedTableName);
	}

	@Override
	public String getCentroidSql(String geomFieldName, String centerAlias, String qualifiedTableName) {
		// MySQL: ST_Centroid
		return StrUtil.format("SELECT *, ST_Centroid({}) AS {} FROM {};", geomFieldName, centerAlias,
				qualifiedTableName);
	}

	@Override
	public String getValidateGeometriesSql(String qualifiedTableName, String geomFieldName) {
		return StrUtil.format("SELECT id FROM {} WHERE NOT ST_IsValid({});", qualifiedTableName, geomFieldName);
	}

	@Override
	public String getRepairGeometriesSql(String qualifiedTableName, String geomFieldName) {
		// ST_MakeValid修复无效几何体（8.0.13+支持）
		return StrUtil.format("UPDATE {} SET {} = ST_MakeValid({}) WHERE NOT ST_IsValid({});", qualifiedTableName,
	...
	public String getGetExtentSql(String geomFieldName, String qualifiedTableName, int srid) {
		...
		return StrUtil.format(
				"SELECT " + "ST_XMin(ST_Extent({})) AS minx, " + "ST_YMin(ST_Extent({})) AS miny, "
						+ "ST_XMax(ST_Extent({})) AS maxx, " + "ST_YMax(ST_Extent({})) AS maxy, "
						...
						+ "ST_YMax(ST_Transform(ST_Extent({}), 4326)) AS maxy_gs " + "FROM {};",
				geomFieldName, geomFieldName, geomFieldName, geomFieldName, geomFieldName, geomFieldName, geomFieldName,
