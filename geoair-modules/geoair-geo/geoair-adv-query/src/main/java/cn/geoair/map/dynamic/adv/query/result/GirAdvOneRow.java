 ...
 * 一行查询结果数据。
 *
 * <p>基于 {@link LinkedHashMap}，通过接口继承链提供丰富的类型安全 getter： {@code getStr / getInt / getLong /
 * getGeometry / getGeoJsonStr / getWktString} 等。
 *
 * @author zhangjun
 */
public class GirAdvOneRow extends LinkedHashMap<String, Object>
     ...
     *
     * <p>如果已注入 {@link AdvTypeHandlerRegistry}（通过 {@code bSelectList} 等方法自动注入）， 则走 TypeHandler
     * 转换链路；否则回退到 Hutool BeanUtil。
     */
    ...
    /** 批量转换（使用统一 Registry） */
    public static <T> List<T> toBeanObjList(
            List<GirAdvOneRow> rowList, Class<T> clazz, AdvTypeHandlerRegistry registry) {
        if (rowList == null) return Collections.emptyList();
     ...
     *
     * <p>当 {@link #typeHandlerRegistry} 已注入时，优先使用 TypeHandler 链进行方言感知的转换 （如 PostGIS PGobject、MySQL
     * binary、Oracle SDO_GEOMETRY），性能更优； 对于 String 类型的值，保留原有的 GeoJSON → WKT → WKB 解析顺序。
     *
     * <p>当 registry 未注入时（如反序列化后的实例），回退到父类默认的试错式解析。
     */
    @Override
    ...
    private Geometry parseStringGeometry(String str) {
        ...
        try {
            ...
            if (json != null) {
                return GirGeoTools.defaultInstance().getFormatOpt().geojsonToJtsGeometry(str, true);
            }
        } catch (Exception ignored) {
        }
        // 2. 尝试 WKT
        try {
            return GirGeoTools.defaultInstance().getFormatOpt().wktToJtsGeometry(str, true);
        } catch (Exception ignored) {
        }
        // 3. 尝试 WKB
        try {
            return GirGeoTools.defaultInstance().getFormatOpt().wkbToJtsGeometry(str, true);
        } catch (Exception ignored) {
        }
        return null;
    }
