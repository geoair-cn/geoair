 ...
 * <p>实现逻辑：</p>
 * <ol>
 *   <li>通过 {@link AdvDynamicDataSourceStorage} 和 {@link DynamicDataSourceManager}
 *       根据 dataSourceId 获取或创建对应的 {@link AdvDataSourceWrapper}</li>
 *   <li>调用 {@link AdvExecutorFactory#getAdvExecutorByDataSource(DataSource, String)}
 *       自动检测数据库方言并创建匹配的 Executor</li>
 *   <li>如果 schema 不为空，设置 Executor 的 Schema 名称获取函数</li>
 * </ol>
 *
 * <p>此实现通过 {@link cn.geoair.base.Gir} SPI 机制暴露为服务，
 * 调用方可通过 {@code GirService.getPxyBeanC(IAdvExecutorAdapter.class)} 获取。</p>
 *
 * @author 张逢吉
