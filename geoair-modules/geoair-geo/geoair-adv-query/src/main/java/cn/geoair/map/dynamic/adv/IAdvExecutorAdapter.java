 ...
 *
 * <p>调用方无需关心数据源如何查找、方言如何检测、Schema 如何设置，
 * 只需传入数据源标识即可获取现成的 {@link IAdvExecutor}。</p>
 *
 * <p>{@code IAdvExecutorAdapter} 是可扩展接口：如果默认实现
 * {@link cn.geoair.map.dynamic.adv.spring.CommonAdvExecutorAdapter} 不满足需求，
 * 调用方可以实现自己的适配器来定制数据源查找方式。</p>
 *
 * <p>典型用法：</p>
 * <pre>{@code
...
public interface IAdvExecutorAdapter {
     ...
     * <p>适配器内部负责：</p>
     * <ol>
     *   <li>根据 dataSourceId 查找对应的 DataSource</li>
     *   <li>自动检测数据库方言，创建匹配的 Executor</li>
     *   <li>设置默认 Schema</li>
     * </ol>
     *
     ...
     *
     * <p>与 {@link #getIAdvExecutor(String, String)} 功能相同，
     * 额外允许指定返回类型，方便直接获取方言特定的 Executor 子类。</p>
     *
