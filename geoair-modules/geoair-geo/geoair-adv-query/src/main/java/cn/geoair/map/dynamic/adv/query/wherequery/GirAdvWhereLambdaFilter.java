 ...
 * Lambda风格的WHERE条件构建器
 *
 * <p>模仿MyBatis-Plus的LambdaWrapper，提供类型安全的查询条件构建
 *
 * <p>使用示例：
 *
 * <pre>
...
public class GirAdvWhereLambdaFilter<T> implements Serializable {
    ...
    private GirAdvWhereLambdaFilter(
            GirAdvWhereFilter whereFilter, Class<T> entityClass, boolean isToUnderlineCase) {
    ...
    public static <T> GirAdvWhereLambdaFilter<T> of(
     ...
     */
    public static <T> GirAdvWhereLambdaFilter<T> of(
            GirAdvWhereFilter whereFilter, Class<T> entityClass) {
    ...
    public GirAdvWhereLambdaFilter<T> and(Consumer<GirAdvWhereLambdaFilter<T>> consumer) {
        GirAdvWhereLambdaFilter<T> nestedWrapper =
                new GirAdvWhereLambdaFilter<>(entityClass, isToUnderlineCase);
     ...
     * @param value 值
     */
    public GirAdvWhereLambdaFilter<T> expr(
            String sqlExpr, AdvOperatorEnums operator, Object value) {
    ...
    /** 复制条件到条件组构建器 */
    private void copyConditionsToGroup(
            GirAdvWhereFilter source, GirAdvWhereFilter.ConditionGroupBuilder target) {
        // 直接访问GirAdvWhereFilter的内部结构，通过反射或添加公共方法
        // 这里假设GirAdvWhereFilter提供了getEntries方法
        try {
            // 通过反射获取entries列表
            java.lang.reflect.Field entriesField =
                    GirAdvWhereFilter.class.getDeclaredField("entries");
            ...
            for (Object entry : entries) {
                ...
                java.lang.reflect.Field connectorField =
                        entry.getClass().getDeclaredField("connector");
                java.lang.reflect.Field expressionField =
                        entry.getClass().getDeclaredField("expression");
                ...
                GirAdvWhereFilter.ConditionExpression expression =
                        (GirAdvWhereFilter.ConditionExpression) expressionField.get(entry);
    ...
    /** 添加表达式到条件组 */
    private void addExpressionToGroup(
            GirAdvWhereFilter.ConditionGroupBuilder target,
            GirAdvWhereFilter.ConditionExpression expression) {
        if (expression.isLeaf()) {
            // 叶子节点：具体条件
            if (expression.isExpression()) {
                target.expr(
                        expression.getColumn(), expression.getOperator(), expression.getValue());
            } else {
                // 根据操作符调用对应方法
            ...
            target.group(
                    group -> {
                        for (GirAdvWhereFilter.ConditionExpression child :
