 ...
 * <p>
 * 每个值关联一个 {@link CategoryGroupEnum}（大分组）和三个 {@link IgnorePolicy}（长度/精度/小数位策略）。
...
public enum CategoryEnum implements GiVisualValuable<String> {
    ...
    /** 布尔类型 — 全部忽略 */
    BOOLEAN(CategoryGroupEnum.BOOLEAN, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE),
    ...
    DATETIME(CategoryGroupEnum.DATETIME, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE),
    /** 时间戳类型，如 TIMESTAMP — 全部忽略 */
    TIMESTAMP(CategoryGroupEnum.DATETIME, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE),
    /** 集合/数组类型 — 全部忽略 */
    COLLECTION(CategoryGroupEnum.COLLECTION, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE),
    /** 空间几何类型，如 GEOMETRY、GEOGRAPHY — 全部忽略 */
    GEOMETRY(CategoryGroupEnum.GEOMETRY, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE, IgnorePolicy.IGNORE),
    /** 时间间隔类型，如 INTERVAL — 精度视情况，精度与小数位互依赖 */
    INTERVAL(CategoryGroupEnum.INTERVAL, IgnorePolicy.IGNORE, IgnorePolicy.CONDITIONAL, IgnorePolicy.MUTUAL_DEPENDENT),
    /** 其他未分类类型 — 全部忽略 */
    ...
    CategoryEnum(CategoryGroupEnum group, IgnorePolicy ignoreLength, IgnorePolicy ignorePrecision, IgnorePolicy ignoreScale) {
        this.group = group;
