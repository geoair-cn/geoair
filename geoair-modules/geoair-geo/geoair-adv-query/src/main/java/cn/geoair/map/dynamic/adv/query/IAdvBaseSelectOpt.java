...
public interface IAdvBaseSelectOpt {
    ...
    /**
     * 执行查询并返回单行结果（支持MyBatis动态SQL）
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     *     示例1（普通SQL）：SELECT id, name, age FROM user WHERE id = #{userId} <br>
     *     示例2（带MyBatis标签）：SELECT id, name, age FROM user
     *     <where>
     *         <if test="name != null">AND name LIKE CONCAT('%', #{name}, '%')</if>
     *         <if test="age != null">AND age = #{age}</if>
     *     </where>
     * @param sqlParamMap SQL语句中的参数映射（key为参数名，value为参数值） <br>
     *     示例：{"userId": 1001, "name": "张三", "age": 25}
     * @return GirAdvOneRow 单行查询结果，封装了列名与对应值的映射关系
     */
    GirAdvOneRow bSelectOne(String dynamicSql, SqlParamMap sqlParamMap);

    /**
     * 执行查询并返回多行结果列表（支持MyBatis动态SQL）
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     *     示例：SELECT id, name, create_time FROM dept
     *     <where>
     ...
     * @return List<GirAdvOneRow> 多行查询结果列表
     */
    List<GirAdvOneRow> bSelectList(String dynamicSql, SqlParamMap sqlParamMap);

    /**
     * 流式查询多行结果（支持MyBatis动态SQL），通过Consumer逐行处理数据
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     *     示例：SELECT id, log_content, create_time FROM system_log
     ...
     * @param sqlParamMap SQL语句中的参数映射（key为参数名，value为参数值） <br>
     *     示例：{"deptId": 5}
     * @return List<List<Object>> 纯值列表结果
     */
    List<List<Object>> bSelectListToValueList(String dynamicSql, SqlParamMap sqlParamMap);

    /**
     * 执行聚合查询并返回单个数值结果（支持MyBatis动态SQL）
     *
     * @param dynamicSql 支持MyBatis标签的动态聚合SQL语句 <br>
     *     示例：SELECT COUNT(*) FROM user WHERE dept_id = #{deptId}
     * @param sqlParamMap SQL语句中的参数映射（key为参数名，value为参数值） <br>
     *     示例：{"deptId": 5}
     * @return Number 聚合查询的数值结果
     */
    Number bSelectNumber(String dynamicSql, SqlParamMap sqlParamMap);

    /**
     * 查询SQL执行结果的总行数（支持MyBatis动态SQL）
     *
     * @param dynamicSql 待统计行数的动态SQL语句 <br>
     ...
     * @param sqlParamMap SQL语句中的参数映射（key为参数名，value为参数值） <br>
     *     示例：{"name": "张三"}
     * @return Number 结果集的总行数
     */
    Number bSelectRecordRowCount(String dynamicSql, SqlParamMap sqlParamMap);

    /**
     * 执行查询并将单行结果映射为指定类型的Java对象（支持MyBatis动态SQL）
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     ...
     */
    <E> E bSelectObjOne(String dynamicSql, SqlParamMap sqlParamMap, Class<E> clazz);

    /**
     * 执行查询并将多行结果映射为指定类型的Java对象列表（支持MyBatis动态SQL）
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     ...
     */
    <E> List<E> bSelectObjList(String dynamicSql, SqlParamMap sqlParamMap, Class<E> clazz);

    /**
     * 流式查询并将结果逐行映射为指定类型的Java对象（支持MyBatis动态SQL）
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     ...
     * 执行查询并返回单行结果
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, age FROM user WHERE id = ? AND status = ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：[1001, 1] <br>
     *     注意：参数顺序必须与SQL中?占位符的顺序一致
     * @return GirAdvOneRow 单行查询结果，封装了列名与对应值的映射关系；无结果时返回null
     */
    GirAdvOneRow bSelectOne(String sql, SqlParamList sqlParamList);

    /**
     * 执行查询并返回多行结果列表
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, age FROM user WHERE status = ? AND age > ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：[1, 18]
     * @return List<GirAdvOneRow> 多行查询结果列表，每个元素对应一行数据的列值映射；无结果时返回空列表
     */
    List<GirAdvOneRow> bSelectList(String sql, SqlParamList sqlParamList);

    /**
     * 流式查询多行结果，通过Consumer逐行处理数据（避免一次性加载大量数据占用内存）
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, log_content, create_time FROM system_log WHERE create_time BETWEEN ? AND ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：["2024-01-01 00:00:00", "2024-01-31 23:59:59"]
     * @param rowConsumer 行数据消费器，用于逐行处理查询结果中的每一条GirAdvOneRow数据 <br>
     *     示例：row -> log.info("日志ID：" + row.get("id") + " 内容：" + row.get("log_content"))
     */
    void bSelectList(String sql, SqlParamList sqlParamList, Consumer<GirAdvOneRow> rowConsumer);

    /**
     * 执行查询并返回多行结果，结果以纯值列表形式封装（仅保留值，无列名）
     *
     * <p>外层List代表多行数据，内层List代表单行数据的所有列值（按查询列的顺序排列），
     * 相当于将GirAdvOneRow中的value按顺序提取到List中，减少对象封装开销</p>
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, age FROM user WHERE dept_id = ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：[5]
     * @return List<List<Object>> 纯值列表结果，例如：[[1, "张三", 25], [2, "李四", 28]]；无结果时返回空列表
     */
    List<List<Object>> bSelectListToValueList(String sql, SqlParamList sqlParamList);

    /**
     * 执行聚合查询并返回单个数值结果（如COUNT、SUM、AVG等）
     *
     * @param sql 预编译聚合SQL语句（使用?作为参数占位符） <br>
     *     示例1：SELECT COUNT(*) FROM user WHERE dept_id = ? <br>
     *     示例2：SELECT SUM(salary) FROM employee WHERE position = ? AND age > ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例1：[5] <br>
     *     示例2：["工程师", 18]
     * @return Number 聚合查询的数值结果（Integer/Long/BigDecimal等）；无结果时返回null
     */
    Number bSelectNumber(String sql, SqlParamList sqlParamList);

    /**
     * 查询SQL执行结果的总行数（适配分页场景的count查询）
     *
     * <p>内部会将原SQL包装为COUNT(1)查询，无需手动编写count语句，简化分页逻辑</p>
     *
     * @param sql 待统计行数的原始查询SQL（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, age FROM user WHERE name LIKE ? AND status = ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：["%张三%", 1]
     * @return Number 结果集的总行数（Long类型）；无结果时返回0
     */
    Number bSelectRecordRowCount(String sql, SqlParamList sqlParamList);

    /**
     * 执行查询并将单行结果映射为指定类型的Java对象
     *
     * <p>自动将查询结果的列值映射到目标类的属性（字段名需匹配），支持基本类型、String、日期等类型转换</p>
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, age FROM user WHERE id = ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：[1001]
     * @param clazz 目标对象的类型（如User.class）
     * @param <E> 目标对象的泛型类型
     * @return E 映射后的单行Java对象；无结果时返回null
     */
    <E> E bSelectObjOne(String sql, SqlParamList sqlParamList, Class<E> clazz);

    /**
     * 执行查询并将多行结果映射为指定类型的Java对象列表
     *
     * <p>适合小数据量场景，结果一次性映射为对象列表，字段名需与目标类属性匹配</p>
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, dept_id FROM user WHERE dept_id = ? AND status = ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：[5, 1]
     * @param clazz 目标对象的类型（如User.class）
     * @param <E> 目标对象的泛型类型
     * @return List<E> 映射后的多行Java对象列表；无结果时返回空列表
     */
    <E> List<E> bSelectObjList(String sql, SqlParamList sqlParamList, Class<E> clazz);

    /**
     * 流式查询并将结果逐行映射为指定类型的Java对象，通过Consumer处理（避免内存溢出）
     *
     * <p>适合大数据量对象映射场景，逐行读取并处理，避免内存溢出，字段名需与目标类属性匹配</p>
     *
     * @param sql 预编译SQL语句（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, salary FROM employee WHERE dept_id = ? AND age > ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：[5, 18]
     * @param clazz 目标对象的类型（如Employee.class）
     * @param rowConsumer 行对象消费器，用于逐行处理映射后的Java对象 <br>
     ...
     * @param <E> 目标对象的泛型类型
     */
    <E> void bSelectObjList(String sql, SqlParamList sqlParamList, Class<E> clazz, Consumer<E> rowConsumer);
