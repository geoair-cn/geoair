...
public interface IAdvBaseSelectOpt extends IAdvConfigOpt {
     ...
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     *     示例：SELECT id, name, create_time FROM dept <where> <if test="status != null">AND status =
     *     #{status}</if> <if test="createTimeStart != null">AND create_time >=
     ...
     *
     * @param dynamicSql 支持MyBatis标签的动态SQL语句 <br>
     *     示例：SELECT id, log_content, create_time FROM system_log WHERE create_time BETWEEN
     *     #{startTime} AND #{endTime}
     * @param sqlParamMap SQL语句中的参数映射（key为参数名，value为参数值） <br>
     *     示例：{"startTime": "2024-01-01 00:00:00", "endTime": "2024-01-31 23:59:59"}
     * @param rowConsumer 行数据消费器，用于逐行处理查询结果
     */
    void bSelectListStream(
            String dynamicSql, SqlParamMap sqlParamMap, Consumer<GirAdvOneRow> rowConsumer);
     ...
     *     示例：SELECT id, name FROM user <where> <if test="name != null">AND name LIKE CONCAT('%',
     *     #{name}, '%')</if> </where>
     * @param sqlParamMap SQL语句中的参数映射（key为参数名，value为参数值） <br>
     *     示例：{"name": "张三"}
     ...
     * @param <E> 目标对象的泛型类型
     */
    <E> void bSelectObjListStream(
            String dynamicSql, SqlParamMap sqlParamMap, Class<E> clazz, Consumer<E> rowConsumer);
     ...
     * 执行查询并返回单行结果
     *
     * @param sqlStatement 预编译SQL语句（使用?作为参数占位符） <br>
     ...
     * @return GirAdvOneRow 单行查询结果，封装了列名与对应值的映射关系；无结果时返回null
     */
    GirAdvOneRow bSelectOne(String sqlStatement, SqlParamList sqlParamList);
     ...
     * @return List<GirAdvOneRow> 多行查询结果列表，每个元素对应一行数据的列值映射；无结果时返回空列表
     */
    List<GirAdvOneRow> bSelectList(String sqlStatement, SqlParamList sqlParamList);
     ...
     * @return List<List < Object>> 纯值列表结果，例如：[[1, "张三", 25], [2, "李四", 28]]；无结果时返回空列表
     */
    List<List<Object>> bSelectListToValueList(String sqlStatement, SqlParamList sqlParamList);

    /**
     * 执行聚合查询并返回单个数值结果（如COUNT、SUM、AVG等）
     *
     * @param sqlStatement 预编译聚合SQL语句（使用?作为参数占位符） <br>
     ...
     * @return Number 聚合查询的数值结果（Integer/Long/BigDecimal等）；无结果时返回null
     */
    Number bSelectNumber(String sqlStatement, SqlParamList sqlParamList);
     ...
     * <p>内部会将原SQL包装为COUNT(1)查询，无需手动编写count语句，简化分页逻辑
     *
     * @param sqlStatement 待统计行数的原始查询SQL（使用?作为参数占位符） <br>
     *     示例：SELECT id, name, age FROM user WHERE name LIKE ? AND status = ?
     * @param sqlParamList SQL语句中的参数列表（按?占位符顺序排列） <br>
     *     示例：["%张三%", 1]
     * @return Number 结果集的总行数（Long类型）；无结果时返回0
     */
    Number bSelectRecordRowCount(String sqlStatement, SqlParamList sqlParamList);
     ...
     * @return E 映射后的单行Java对象；无结果时返回null
     */
    <E> E bSelectObjOne(String sqlStatement, SqlParamList sqlParamList, Class<E> clazz);
     ...
     * @return List<E> 映射后的多行Java对象列表；无结果时返回空列表
     */
    <E> List<E> bSelectObjList(String sqlStatement, SqlParamList sqlParamList, Class<E> clazz);
