 ...
 *
 * <p>日常编码中不需要手动构造执行器，直接调用本类的静态方法即可：</p>
 * <pre>{@code
...
public class GirAdvQuery {
     ...
     *
     * <p>内部通过 {@link IAdvExecutorAdapter} 查找对应数据源，
     * 自动检测数据库方言创建匹配的 Executor，并设置 Schema。</p>
     *
     ...
     *
     * <p>与 {@link #getIAdvExecutor(String, String)} 功能相同，但允许指定返回类型，
     * 方便直接获取方言特定的 Executor 子类。</p>
     *
     ...
     *
     * <p>不依赖 Spring 上下文，直接从 DataSource 获取 JDBC 连接探测数据库类型。
     * 适用于非 Spring 环境或手动管理 DataSource 的场景。</p>
     *
     ...
     *
     * <p>与 {@link #getIAdvExecutor(DataSource)} 功能相同，额外指定数据源名称用于日志和追踪。</p>
     *
     * @param dataSource     数据源对象
     ...
     * {@link AdvExecutorFactory#getAdvExecutorByDataSource(DataSource, String)}
     * 中的 JDBC 连接探测，消除获取/释放连接的性能开销。</p>
     *
     * <pre>{@code
     *   // 从配置文件读取数据库类型后直接指定
