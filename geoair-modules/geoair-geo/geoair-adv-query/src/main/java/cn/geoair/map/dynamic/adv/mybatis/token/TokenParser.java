 ...
 * 支持的 token 格式：
 * <ul>
 *   <li>{@code #{expression}} — 参数占位符，由调用方替换为 {@code ?} 并收集参数值</li>
 *   <li>{@code ${expression}} — 常量替换，由调用方直接替换为表达式的值</li>
 * </ul>
 *
...
public class TokenParser {
     ...
     * 扫描逻辑：
     * <ol>
     *   <li>查找下一个 openToken</li>
     *   <li>如果前面有反斜杠 {@code \}，则当作转义字符，跳过</li>
     *   <li>否则开始匹配对应的 closeToken（同样支持反斜杠转义）</li>
     *   <li>找到匹配后调用 {@link TokenHandler#handleToken} 替换内容</li>
     * </ol>
     *
