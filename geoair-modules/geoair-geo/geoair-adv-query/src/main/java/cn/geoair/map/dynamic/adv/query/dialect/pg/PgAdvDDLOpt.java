...
public class PgAdvDDLOpt extends AbstractExecAdvDDLOpt {
	...
	public void dAddPrimaryKey(String tableName, String pkColumnName, String constraintName, PrimaryKeyType pkType,
		...
		try {
			...
			if (PrimaryKeyType.STRING.equals(pkType)) {
				...
				// 填充唯一值（自定义前缀/时间戳）
				String valuePrefix = StrUtil.isEmpty(pkValuePrefix) ? "to_char(now(), 'YYYYMMDDHH24MISS') || '_'"
						: "'" + pkValuePrefix + "' || ";
				String updateSql = StrUtil.format(
						"WITH numbered_rows AS (SELECT ctid, row_number() OVER () AS rn FROM {}) "
								+ "UPDATE {} f1 SET {} = {} f1.rn " + "FROM numbered_rows nr WHERE f1.ctid = nr.ctid",
			...
			// ========== 分支2：整数自增主键（SERIAL） ==========
				...
				String addColumnSql = StrUtil.format("ALTER TABLE {} ADD COLUMN {} SERIAL PRIMARY KEY",
			...
			// ========== 分支4：普通整数主键（非自增） ==========
				...
				String updateSql = StrUtil.format(
						"WITH numbered_rows AS (SELECT ctid, row_number() OVER () AS rn FROM {}) "
								+ "UPDATE {} f1 SET {} = nr.rn " + "FROM numbered_rows nr WHERE f1.ctid = nr.ctid",
	...
	public List<SchemaTableApo> dGetTableAndViewBySchema(String schemaName) {
		...
		if (StrUtil.isEmpty(actualSchema)) {
			...
			sql = StrUtil.format("SELECT " + fields + " FROM information_schema.tables "
					+ "WHERE   table_schema = '{}' ORDER BY table_name", actualSchema);
		}
