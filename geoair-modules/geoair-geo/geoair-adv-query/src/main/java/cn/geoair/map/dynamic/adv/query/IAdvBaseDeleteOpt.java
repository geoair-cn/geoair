...
public interface IAdvBaseDeleteOpt extends IAdvConfigOpt {
     ...
     *
     * <ul>
     *   <li>多表关联删除：DELETE FROM user u USING dept d WHERE u.dept_id = d.id AND d.name = 'test'
     *   <li>子查询删除：DELETE FROM user WHERE dept_id IN (SELECT id FROM dept WHERE status = 0)
     *   <li>带函数条件的删除：DELETE FROM log WHERE create_time < NOW() - INTERVAL '30 days'
     * </ul>
     *
     * <p><b>注意：</b>此方法不会添加任何额外的安全限制，请谨慎使用，避免误删数据
     *
     * @param sqlStatement 自定义删除SQL语句（支持MyBatis标签语法） 示例：DELETE FROM user WHERE create_time <
     *     '2024-01-01' AND status = 0
     * @return 受影响的行数
     ...
     * 执行带参数的自定义删除SQL语句
     *
     * <p>使用预编译参数绑定方式执行SQL，可有效防止SQL注入攻击。 支持MyBatis风格的参数占位符（#{paramName}）
     *
     * <p><b>优势：</b>
     *
     * <ul>
     ...
     * </ul>
     *
     * @param sqlStatement 自定义删除SQL语句（含参数占位符） 示例：DELETE FROM user WHERE dept_id = #{deptId} AND age
     *     > #{age}
     * @param sqlParam SQL参数映射（key为占位符名称，value为参数值） 示例：SqlParamMap.create().put("deptId",
     *     5).put("age", 60)
     * @return 受影响的行数
     * @throws cn.geoair.map.dynamic.adv.query.exception.SqlExecuteException SQL执行异常
     */
     ...
     *
     * @param sqlStatement 自定义更新SQL语句（含参数占位符） 示例：DELETE FROM WHERE id =？
     ...
     * <p>解决纯SQL拼接的SQL注入问题，支持动态参数绑定
     *
     * @param sqlStatement 自定义删除SQL语句（含参数占位符） 示例：DELETE FROM WHERE id =？或者DELETE FROM WHERE id =
     *     #{id}
     ...
     * 根据主键集合批量删除数据
     *
     * <p>底层优化为IN查询：DELETE FROM table WHERE id IN (?, ?, ...) 适用于批量删除少量数据。
     *
     * <p><b>性能建议：</b>
     *
     * <ul>
     *   <li>单次删除建议不超过1000条记录
     *   <li>超过1000条请使用 {@link #bDeleteByPKs(String, String, Set, int)} 分批删除
     *   <li>大量数据删除建议使用条件删除 {@link #bDeleteByMap(String, Map, int)}
     * </ul>
     *
     * <p><b>示例：</b>
     *
     ...
     *
     * <p>自动将主键集合按批次拆分，每批次执行一次DELETE操作， 适用于删除大量数据（如1万+条），避免单次SQL参数过多或锁表时间过长。
     *
     * <p><b>分批策略：</b>
     *
     * <ul>
     ...
     *
     * <p>根据传入的条件Map拼接WHERE子句，多个条件之间使用AND连接。 <b>注意：所有条件均为等值匹配（=），不支持比较运算符（如 >、<、>= 等）</b>
     *
     * <p><b>重要安全机制：</b>
     *
     * <ul>
     *   <li><b>禁止传入空条件</b>：如果whereMap为null或空，会抛出异常，防止全表删除
     *   <li>建议条件中至少包含一个索引字段，避免全表扫描
     *   <li>生产环境建议配合逻辑删除使用，而非物理删除
     * </ul>
     *
     * <p><b>使用示例：</b>
     *
     ...
     *
     * <ul>
     *   <li>{@link #bDeleteByWhere(String, GirAdvWhereFilter)} - 支持复杂条件构建
     *   <li>{@link #bDeleteByWhere(String, GirAdvWhereLambdaFilter)} - Lambda表达式版本
     *   <li>{@link #bDeleteBySql(String, SqlParamMap)} - 自定义SQL
     * </ul>
     *
     * @param tableName 目标表名
     * @param whereMap 删除条件（key=字段名，value=等值匹配的值） 示例：{ "dept_id": 5, "status": 0, "name": "张三" }
     * @return 受影响的行数（实际删除的记录数）
     ...
     *
     * <p>适用于删除大量数据，将删除操作分批执行，避免单次删除锁表时间过长或事务过大。 <b>注意：所有条件均为等值匹配（=），不支持比较运算符（如 >、<、>= 等）</b>
     *
     * <p><b>执行机制：</b>
     *
     * <ol>
     *   <li>先统计符合条件的总记录数：SELECT COUNT(*) FROM table WHERE ...
     *   <li>按batchSize分批删除：DELETE FROM table WHERE ... LIMIT batchSize
     *   <li>循环执行直到删除完毕
     *   <li>返回所有批次删除的总行数
     * </ol>
     *
     ...
     *
     * <ul>
     *   <li>MySQL使用LIMIT，PostgreSQL需要不同的实现
     *   <li>删除过程中新插入符合条件的数据可能会被遗漏
     *   <li>建议配合索引字段使用，提高分页效率
     *   <li>适合清理历史数据、过期数据等场景
     *   <li>如需复杂条件，请使用自定义SQL方式
     * </ul>
     *
     * @param tableName 目标表名
     ...
     *
     * <p>使用Lambda表达式构建类型安全的WHERE条件，避免字段名拼写错误， 提供更好的IDE支持和重构友好性。支持复杂条件构建，包括比较运算符、函数、子查询等。
     *
     * <p><b>优势：</b>
     *
     * <ul>
     *   <li>编译期检查：字段名错误在编译时就能发现
     *   <li>IDE智能提示：自动补全实体类字段
     *   <li>重构友好：实体类字段改名会自动同步
     *   <li>支持复杂表达式和函数计算
     * </ul>
     *
     * <p><b>使用示例：</b>
     ...
     * 按条件删除数据（传统方式，支持复杂条件）
     *
     * <p>使用传统的GirAdvWhereFilter构建WHERE条件，适合动态条件构建场景。 支持复杂条件构建，包括比较运算符、函数、OR/AND组合等。
     *
     * <p><b>使用场景：</b>
     *
     * <ul>
     *   <li>条件字段动态不确定
     *   <li>需要复杂的条件组合（嵌套AND/OR）
     *   <li>与其他模块兼容
     * </ul>
     *
     * <p><b>使用示例：</b>
