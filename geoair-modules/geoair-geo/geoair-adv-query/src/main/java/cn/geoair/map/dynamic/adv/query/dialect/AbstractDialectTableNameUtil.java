...
public abstract class AbstractDialectTableNameUtil implements DialectTableNameProcessor {
	...
	public String tbGetTableNameNotSchema(String fullTableName) {
		...
		Pattern pattern = Pattern.compile("(?:[\"'`][^\"'`]+[\"'`]|[a-zA-Z0-9_\\u4e00-\\u9fa5-]+)\\." + // Schema/库名部分（兼容中文、横线）
	...
	public String tbExtractSchemaName(String fullTableName) {
		...
		Pattern pattern = Pattern.compile("(?:([\"'`][^\"'`]+[\"'`]|[a-zA-Z0-9_\\u4e00-\\u9fa5-]+))\\." + // Schema/库名部分
				"(?:[\"'`][^\"'`]+[\"'`]|[a-zA-Z0-9_\\u4e00-\\u9fa5]+)" // 表名部分
		);
		Matcher matcher = pattern.matcher(processedName);
	...
	public String tbGetSchemaNameForSql(IDataSourceGetter dataSourceGetter) {
		return ObjectUtil.isEmpty(dataSourceGetter.getSchemaName()) ? getDefaultSchemaName()
