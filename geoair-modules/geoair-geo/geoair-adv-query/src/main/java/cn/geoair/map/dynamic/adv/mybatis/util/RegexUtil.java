 ...
 * <p>
 * 将 SQL 片段中的 {@code item} / {@code index} 变量名替换为实际的集合索引访问表达式。
 * 例如：将 {@code item.name} 替换为 {@code list[0].name}。
 *
 * <p>替换规则：仅匹配字符串开头的变量名，且变量名后不能紧跟标识符字符（字母、数字、下划线），
 * 以避免误替换（如 {@code itemName} 不会被替换）。
 *
 * @author zhangjun
 */
public class RegexUtil {
     ...
     * <p>
     * 使用正则 {@code ^\s*item(?![^.,:\s])} 进行匹配：
     * <ul>
     *   <li>{@code ^\s*} — 允许开头有空白</li>
     *   <li>{@code item} — 要替换的变量名</li>
     *   <li>{@code (?![^.,:\s])} — 负向前瞻，变量名后不能紧跟标识符字符</li>
     * </ul>
     *
    ...
    public static String replace(String content, String item, String newItem) {
        return content.replaceFirst(
                "^\\s*" + Pattern.quote(item) + "(?![^.,:\\s])",
                Matcher.quoteReplacement(newItem));
    }
