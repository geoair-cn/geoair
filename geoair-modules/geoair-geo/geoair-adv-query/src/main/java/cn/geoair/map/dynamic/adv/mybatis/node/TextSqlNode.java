 ...
 * 执行时：
 * <ul>
 *   <li>{@code ${expression}} — 通过 OGNL 求值后直接替换为常量值（<b>存在 SQL 注入风险，仅用于可信输入</b>）</li>
 *   <li>{@code #{expression}} — 保持不变，后续由引擎统一替换为 {@code ?} 占位符</li>
 * </ul>
 *
 * @author zhangjun
 */
public class TextSqlNode implements SqlNode {
     ...
     * 解析 ${} 表达式并追加到上下文。
     * <p>
     * <b>安全提示：</b>${} 使用原始字符串拼接，无转义。确保表达式来源可信。
     */
    @Override
    ...
    public void applyParameter(Set<String> set) {
        TokenParser dollarParser = new TokenParser("${", "}", new TokenHandler() {
            @Override
            public String handleToken(String paramName) {
                set.add(paramName);
                return paramName;
        ...
        TokenParser hashParser = new TokenParser("#{", "}", new TokenHandler() {
            @Override
            public String handleToken(String paramName) {
                set.add(paramName);
                return paramName;
