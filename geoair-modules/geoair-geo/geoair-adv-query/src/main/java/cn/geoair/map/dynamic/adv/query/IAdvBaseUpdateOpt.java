...
public interface IAdvBaseUpdateOpt extends IAdvConfigOpt {
    ...
    void bUpdateBatchByPK(String tableName, String idKey, List<Map<String, Object>> rowsData, int batchSize);
    ...
    <T> void bUpdateBatchByPKSelective(Collection<T> entities, Consumer<UpdateStrategy> strategyConsumer);
     ...
     */
    <T> Integer bUpdateByWhere(T entity, UpdateStrategy strategy, GirAdvWhereLambdaFilter<T> whereFilter);

    /**
     * 复杂条件更新（实体格式，带策略和Consumer条件）
     */
    <T> Integer bUpdateByWhere(T entity, UpdateStrategy strategy, Consumer<GirAdvWhereLambdaFilter<T>> consumer);
     ...
     * 复杂条件更新（实体格式，带策略配置）
     */
    <T> Integer bUpdateByWhere(T entity, Consumer<UpdateStrategy> strategyConsumer, Consumer<GirAdvWhereLambdaFilter<T>> consumer);
...
// 单条 UPSERT
    ...
    // 批量 UPSERT
    Integer bUpsertBatch(String tableName, List<Map<String, Object>> rowsData, List<String> conflictKeys, int batchSize);
    ...
    <T> void bUpsertBatchSelective(Collection<T> entities, Consumer<UpdateStrategy> strategyConsumer);
