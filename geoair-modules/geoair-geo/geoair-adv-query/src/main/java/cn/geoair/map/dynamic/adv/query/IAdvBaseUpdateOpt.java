...
public interface IAdvBaseUpdateOpt extends IAdvConfigOpt {
     ...
     *                     示例：UPDATE user SET name = #{name} WHERE id = #{id} sqlParam SQL参数映射（key为占位符名称，value为参数值）
     *                     <br>
     *                     示例：{ "name": "张三", "id": 1001 }
     * @return Integer 受影响的行数
     */
     ...
     * 根据主键更新单条实体对象
     *
     * <p>自动提取实体对象的属性作为更新字段，主键字段用于WHERE条件
     *
     * @param tableName 目标表名
     * @param idKey     主键字段名
     ...
     * 根据主键更新单条实体对象
     *
     * <p>适用于面向对象的单条数据更新，对象属性名需与表字段名匹配（支持驼峰转下划线）
     *
     ...
     * 条件更新单条数据（非主键更新）
     *
     * <p>适用于按自定义条件更新，避免全表更新风险（需确保条件能定位单行）
     *
     * @param tableName 目标表名
     ...
     * 更新或插入（UPSERT）
     *
     * <p>先尝试更新，无匹配数据则插入，等价于INSERT ON CONFLICT UPDATE
     *
     * @param tableName    目标表名
