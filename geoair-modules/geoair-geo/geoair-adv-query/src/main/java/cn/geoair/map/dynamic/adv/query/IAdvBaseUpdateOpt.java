...
public interface IAdvBaseUpdateOpt extends IAdvConfigOpt {
     ...
     *
     * @param sqlStatement 自定义更新SQL语句 示例：UPDATE user SET age = age + 1 WHERE id = 1001
     * @return 受影响的行数
     */
     ...
     *
     * @param dynamicSql 自定义更新SQL语句（含参数占位符） 示例：UPDATE user SET name = #{name} WHERE id = #{id}
     * @param sqlParam SQL参数映射（key为占位符名称，value为参数值） 示例：{ "name": "张三", "id": 1001 }
     * @return 受影响的行数
     */
     ...
     * <p>解决纯SQL拼接的SQL注入问题，支持动态参数绑定
     *
     * @param sqlStatement 自定义更新SQL语句（含参数占位符） 示例：UPDATE user SET name = ？ WHERE id =？
     ...
     * <p>解决纯SQL拼接的SQL注入问题，支持动态参数绑定
     *
     * @param sqlStatement 自定义更新SQL语句（含参数占位符） 示例：UPDATE user SET name = ？ WHERE id =？或者 UPDATE user
     ...
     * @param id 主键值
     * @param rowData 待更新的字段-值映射（key=字段名，value=新值） 示例：{ "name": "张三", "age": 25 }
     ...
     * @return 受影响的行数
     */
    <T> Integer bUpdateByPKSelective(
     ...
     *
     * <p>适用于按自定义条件更新，需确保条件能唯一定位到单行数据，避免全表更新风险。 <b>注意：whereMap中的所有条件均为等值匹配（=），不支持比较运算符（如 >、<、>=
     * 等）</b>
     *
     * <p><b>使用示例：</b>
     *
     ...
     *
     * <ul>
     *   <li>{@link #bUpdateByWhere(String, Map, GirAdvWhereFilter)} - 支持复杂条件构建
     *   <li>{@link #bUpdateByWhere(String, Object, GirAdvWhereLambdaFilter)} - Lambda表达式版本
     *   <li>{@link #bUpdateBySql(String, SqlParamMap)} - 自定义SQL
     * </ul>
     *
     * @param tableName 目标表名
     * @param rowData 待更新的字段-值映射（key=字段名，value=新值）
     * @param whereMap 更新条件（key=字段名，value=等值匹配的值，多个条件拼接为AND） 示例：{ "dept_id": 5, "status": 0 }
     * @return 受影响的行数
     ...
     * 批量更新数据（按主键批量更新）
     *
     * <p>适用于批量更新不同主键的多条数据，每条数据的更新字段可不同 使用CASE WHEN或逐条执行方式实现
     *
     * @param tableName 目标表名
     * @param idKey 主键字段名
     * @param rowsData 批量更新数据（每个Map需包含主键字段和待更新字段） 示例：[{ "id": 1, "age": 25 }, { "id": 2, "name":
     *     "李四" }]
     * @return 受影响的总行数
     */
     ...
     *
     * <p>使用Lambda表达式构建类型安全的WHERE条件，避免字段名拼写错误。 支持复杂条件构建，包括比较运算符、函数、子查询等。
     *
     * <p><b>使用示例：</b>
     *
     * <pre>
     ...
     *
     * <p>自动忽略实体中值为null的字段以及指定的字段，只更新非null且不在忽略列表中的字段。 使用Lambda表达式构建类型安全的WHERE条件。
     *
     * @param tableName 目标表名
     * @param entity 待更新的实体对象
     ...
     *
     * <p>自动忽略实体中值为null的字段，只更新非null字段。 使用Lambda表达式构建类型安全的WHERE条件。
     *
     * @param tableName 目标表名
     * @param entity 待更新的实体对象
     * @param whereFilter Lambda条件过滤器
     ...
     *
     * <p>自动过滤rowData中value为null或空字符串的字段，只更新有效字段。 使用GirAdvWhereFilter构建复杂的WHERE条件。
     *
     * <p><b>使用示例：</b>
     *
     * <pre>
