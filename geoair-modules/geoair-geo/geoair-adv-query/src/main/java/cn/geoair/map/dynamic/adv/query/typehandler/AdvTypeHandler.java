 ...
 *
 * <p>实现类通过 SPI 加载，按优先级链式匹配。
 * 读方向：JDBC 原始值 → Java 对象；写方向：Java 对象 → JDBC 可接受值。
 * 同时支持自定义 SQL 占位符表达式（如 MySQL 几何类型的 {@code ST_GeomFromText(?, srid)}）。</p>
 *
...
public interface AdvTypeHandler<T> {
     ...
     *
     * <p>返回 {@code null} 表示使用默认 {@code ?}，原值放入参数列表。
     * 返回非空时，{@link SqlPlaceholder#getSql()} 直接拼入 SQL 模板，
     * {@link SqlPlaceholder#getParam()} 替换原始值放入参数列表（为 {@code null} 则不放）。</p>
     *
     * <p>典型用法：</p>
     * <pre>{@code
