 ...
 * OGNL 表达式求值工具类。
 * <p>
 * 封装了 OGNL 库的常用操作，提供对 {@code Map<String, Object>} 上下文的表达式求值能力。
 * 主要用于 MyBatis 动态 SQL 中 {@code <if test="...">} 和 {@code ${}} / {@code #{}} 的参数解析。
 *
 * <p><b>安全提示：</b>OGNL 表达式可以执行任意 Java 方法调用。如果 SQL 模板来自不可信的用户输入，
 * 必须对 {@code test} 属性和 {@code ${}} 表达式进行严格的白名单校验，防止远程代码执行（RCE）。
 *
 * @author zhangjun
 */
public class OgnlUtil {
     ...
     * 支持以下类型的自动转换：
     * <ul>
     *   <li>{@link Iterable} — 直接返回</li>
     *   <li>数组（包括基本类型数组） — 转为 List</li>
     *   <li>{@link Map} — 返回 entrySet()</li>
     * </ul>
     *
