...
public interface TilePathParser {
     ...
     */
    MbtilesFromLocalFileConverter.TileInfo parse(File file, String relativePath, MbtilesFromLocalFileConverter.ConvertConfig config);

    /**
     * 默认的瓦片路径解析器
     * 支持格式：{z}/{y}/{x}.png 或 {z}/{x}/{y}.png
     * 从路径中自动提取数字
     */
    static final TilePathParser DEFAULT_ZYX_PARSER = (file, relativePath, config) -> {
        try {
            // 按分隔符分割路径
            ...
            MbtilesFromLocalFileConverter.TileInfo tile = new MbtilesFromLocalFileConverter.TileInfo();
     ...
     */
    static final TilePathParser TMS_ZYX_PARSER = (file, relativePath, config) -> {
        try {
            String[] parts = relativePath.split("[/\\\\]");
            if (parts.length < 3) {
                return null;
     ...
     */
    static final TilePathParser GOOGLE_ZXY_PARSER = (file, relativePath, config) -> {
        try {
            String[] parts = relativePath.split("[/\\\\]");
            if (parts.length < 3) {
                return null;
