...
/**
 * 带缓存的GtcTileFuser装饰器 对融合后的图像结果进行缓存，避免重复计算相同参数的融合请求
 *
 * @author ：张俊
    ...
    public byte[] toImageBytes() throws Exception {
        ...
        try {
            ...
            if (cachedResult != null && cachedResult.length > 0) {
                ...
                log.debug(
                        "缓存命中 - layer: {}, z: {}, x: {}, y: {}, 耗时: {} ms, 数据大小: {} bytes",
                        layerName,
                        z,
                        x,
                        y,
        ...
        if (result != null && result.length > 0) {
            try {
                ...
                log.debug(
                        "融合完成并已缓存 - layer: {}, z: {}, x: {}, y: {}, 融合耗时: {} ms, 总耗时: {} ms, 数据大小: {} bytes",
                        layerName,
                        z,
                        x,
                        y,
