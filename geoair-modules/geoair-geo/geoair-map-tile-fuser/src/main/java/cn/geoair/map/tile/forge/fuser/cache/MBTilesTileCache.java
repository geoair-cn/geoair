 ...
 * MBTiles 瓦片缓存实现
 *
 * <p>MBTiles 规范：https://github.com/mapbox/mbtiles-spec 每个图层独立一个 .mbtiles 文件
 *
 * <p>目录结构: cacheRoot/layerName.mbtiles
 *
 * @author 张逢吉
...
public class MBTilesTileCache implements TileCache {
    ...
    private final ConcurrentHashMap<String, LayerCacheHolder> layerCaches =
    ...
    public boolean put(String layerName, int z, int x, int y, byte[] data, GiMimeType format) {
        ...
        if (result && log.isDebugEnabled()) {
            log.debug("MBTiles 缓存保存: {} ({},{},{}) 大小: {} bytes", layerName, z, x, y, data.length);
        }
        return result;
    }
