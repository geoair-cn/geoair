...
/** 瓦片文件名处理器接口 参数：entryName - ZIP 条目名称，tileData - 瓦片二进制数据 返回：MbtilesInfo 对象，如果返回 null 则跳过该瓦片 */
@FunctionalInterface
public interface TileNameProcessor extends BiFunction<String, byte[], MbtilesInfo> {

    /** 默认的文件名处理器 支持格式：z/y/x.png 或 z/y/x.jpg */
    static final TileNameProcessor DEFAULT_PROCESSOR =
            (entryName, tileData) -> {
    ...
    static final TileNameProcessor PROCESSOR_ZXY =
            (entryName, tileData) -> {
                String[] pathArr = entryName.split("/");
                if (pathArr.length != 3) {
                    return null;
    ...
    static final TileNameProcessor PROCESSOR_ZYX_NO_REVERSE =
            (entryName, tileData) -> {
                String[] pathArr = entryName.split("/");
                if (pathArr.length != 3) {
                    return null;
                ...
                try {
                    ...
                    String yFile = pathArr[2];
                    int x = Integer.parseInt(yFile.replaceAll("\\.(png|jpg)$", ""));
                    return MbtilesInfo.of().setX(x).setZoomLevel(z).setY(y).setTileData(tileData);
                } catch (NumberFormatException e) {
                    return null;
                }
            };
