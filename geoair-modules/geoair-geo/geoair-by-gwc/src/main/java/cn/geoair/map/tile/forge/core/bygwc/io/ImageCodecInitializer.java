...
/** 图片编码器/解码器初始化工具类 模拟 GWC 中的编码器/解码器配置 */
public class ImageCodecInitializer {
    ...
    private static void initEncoders() {
        ...
        PNGImageEncoder pngEncoder =
                new PNGImageEncoder(
                        false, // isMultiImage
                        0.25f, pngParams, // pngParams
                        false);
        ...
        ImageEncoderImpl gifEncoder =
                new ImageEncoderImpl(
                        true, // isMultiImage
    ...
    private static void initDecoders() {
        ...
        ImageDecoderImpl pngDecoder =
                new ImageDecoderImpl(
                        false, // isMultiImage
        ...
        // GIF 解码器
        List<String> gifMimeTypes = Arrays.asList("image/gif");
        List<String> gifReaderSpis = Arrays.asList("com.sun.imageio.plugins.gif.GIFImageReaderSpi");

        ImageDecoderImpl gifDecoder =
                new ImageDecoderImpl(
                        true, // isMultiImage
                        gifMimeTypes);
    ...
    public static List<ImageEncoder> getAllEncodersList() {
        return Collections.unmodifiableMap(encoders).entrySet().stream()
    ...
    public static List<ImageDecoder> getAllDecodersList() {
        return Collections.unmodifiableMap(decoders).entrySet().stream()
                .map(Map.Entry::getValue)
                .collect(Collectors.toList());
    }
