...
public class MimeType extends BaseMimeType {
    ...
    /**
     * 根据资源获取MIME类型字符串 子类可重写此方法以根据资源内容动态确定MIME类型
     *
     * @param resource 资源对象
     * @return MIME类型字符串
    ...
    /**
     * 判断该格式是否支持切片（可分割为小块或组合为大块） 实际上这意味着它必须是无损栅格图像格式
     *
    ...
    /**
     * 判断是否为矢量格式而非栅格格式 矢量格式的输出不应有边距（guttering）
     *
     * @return {@code true} 表示矢量或其他非栅格格式， 这类格式应用边距会导致结果不正确
     */
