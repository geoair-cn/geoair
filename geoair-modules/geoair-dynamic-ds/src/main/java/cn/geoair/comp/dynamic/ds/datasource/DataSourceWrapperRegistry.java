...
public class DataSourceWrapperRegistry {
	...
	static {
		// 高性能连接池优先
		if (DruidDataSourceWrapper.canInit())
			registerWrapper(DruidDataSourceWrapper.class);
		if (HikariDataSourceWrapper.canInit())
			registerWrapper(HikariDataSourceWrapper.class);
		// 传统连接池
		if (DBCP2DataSourceWrapper.canInit())
			registerWrapper(DBCP2DataSourceWrapper.class);
		if (C3P0DataSourceWrapper.canInit())
			registerWrapper(C3P0DataSourceWrapper.class);
		if (BoneCPDataSourceWrapper.canInit())
			registerWrapper(BoneCPDataSourceWrapper.class);

		// 简单/非池化数据源
		if (SpringDiverManagerSourceWrapper.canInit())
			registerWrapper(SpringDiverManagerSourceWrapper.class);
		if (AdvSimpleDataSourceWrapper.canInit())
			registerWrapper(AdvSimpleDataSourceWrapper.class);
		if (DiverManagerSourceWrapper.canInit())
			registerWrapper(DiverManagerSourceWrapper.class);
		if (CommonSourceWrapper.canInit())
			registerWrapper(CommonSourceWrapper.class);
	}
	...
	public static String getSimpleDataSourceName(DataSource dataSource) {
		return getWrapper(dataSource).map(AdvDataSourceWrapper::getSimpleDataSourceName).orElse("unknown");
	}
