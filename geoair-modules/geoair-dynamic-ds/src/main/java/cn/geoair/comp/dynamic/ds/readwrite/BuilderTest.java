...
//        GirGroupByIdDataSource slaveGroupById = new GirGroupByIdDataSource("slaveGroup", slaveIds);
...
// ========== 方式4：使用 ID 列表（自动创建 GirGroupByIdDataSource） ==========
        GirReadWriteDataSource dataSource4 = GirReadWriteDataSourceBuilder.builder()
                .master("master_db")
...
// ========== 方式5：混合使用（先添加ID，再设置策略） ==========
        GirReadWriteDataSource dataSource5 = GirReadWriteDataSourceBuilder.builder()
                .master("master_db")
                .addSlave("slave1")
                .addSlave("slave2")
                .addSlave("slave3")
...
// 设置权重
//        GirGroupByIdDataSource slaveGroup = (GirGroupByIdDataSource) dataSource5.getSlaveGroup();
...
// 使用 ID 列表
        GirReadWriteDataSource dataSource8 = GirReadWriteDataSourceBuilder
