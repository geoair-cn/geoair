...
public class GirGroupSource extends AbstractDataSource {
    ...
    /** 构造方法（指定负载策略） */
    public GirGroupSource(
            String groupName, List<DataSource> dataSources, LoadStrategyType strategyType) {
    ...
    protected Integer getActiveCount(DataSource dataSource) {
        ...
        try {
            if (dataSource.getClass().getName().contains("Druid")) {
                return (Integer)
                        dataSource.getClass().getMethod("getActiveCount").invoke(dataSource);
    ...
    public GirGroupSource setStrategyType(LoadStrategyType strategyType) {
        this.strategyType = strategyType;
        RdLog.getInstance()
                .debug("Group [{}] 负载策略切换为: {}", groupName, strategyType.getDescription());
        return this;
    }
    ...
    public void close() throws IOException {
        RdLog.getInstance()
                .debug(
                        "Group [{}] close called, but no action taken - data sources managed externally",
                        groupName);
    }
}
