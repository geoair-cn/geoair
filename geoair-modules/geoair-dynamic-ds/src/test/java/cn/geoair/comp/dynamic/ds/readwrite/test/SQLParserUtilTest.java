...
/**
 * SQL解析工具类测试 包含 PostgreSQL、MySQL 等主流数据库语法
 *
 * @author 张俊
 * @date 2026/5/28
 */
public class SQLParserUtilTest {
    ...
    private static void testPostgreSQLSpecific() {
        ...
        String[] withSQLs = {
            ...
            //                "WITH recursive cte AS (SELECT 1 UNION ALL SELECT id+1 FROM cte WHERE
            // id<10) SELECT * FROM cte",
            //                "WITH user_cte AS (SELECT id, name FROM user WHERE age > 18) SELECT *
            // FROM user_cte",
            "WITH moved_rows AS (SELECT FROM users WHERE age < 18 RETURNING *)  INSERT INTO users_archive SELECT * FROM moved_rows;",
            //                "WITH orders AS (SELECT user_id, COUNT(*) as cnt FROM orders GROUP BY
            // user_id) SELECT u.name, o.cnt FROM users u JOIN orders o ON u.id = o.user_id"
        };
    ...
    private static void testBatchStatistics() {
        ...
        for (String sql : allSQLs) {
            ...
            System.out.printf(
                    "  %-45s -> %s\n",
                    sql.length() > 43 ? sql.substring(0, 40) + "..." : sql, type);
