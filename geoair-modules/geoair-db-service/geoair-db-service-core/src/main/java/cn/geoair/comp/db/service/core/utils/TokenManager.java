...
public class TokenManager {
    ...
    public static boolean validateToken(String token) {
        GirDsServiceProperties girDsServiceProperties =
                GirService.getPxyBeanC(GirDsServiceProperties.class);
    ...
    public static boolean validateToken() {
        Map<String, String> paramMap =
                GirServletUtil.getParamMap(GirHttpServletHelper.getRequest());
        String token = paramMap.get("dsToken");
        boolean b = validateToken(token);
        if (!b) {
            GirServletUtil.setNoCacheHeaders();
            GirServletUtil.toResponse(
                    GirHttpServletHelper.getResponse(),
                    "{\"code\":401,\"message\":\"Token无效\"}".getBytes(),
                    "application/json");
