 ...
 * @description： com.gtc.comp.knife4j.ext.aspect.TypeResolverAspect,\
 */
@Aspect
...
//                Class<? extends GiWebResult> aClass = GiWebResult.getResult(null).getClass();
                ...
                return ResolvedObjectType.create(aClass
                        , ((ResolvedInterfaceType) proceed).getTypeBindings()
//                        , ((ResolvedInterfaceType) proceed).getParentClass()
                        , resolve.getParentClass()
                        , resolve.getImplementedInterfaces()
                );
            }
...
//        Set<Class<? extends GiResult>>
    ...
    private ResolvedObjectType setSuperClassTypeBind(ResolvedObjectType resolvedObjectType, TypeBindings typeBindings) {
        ...
        if (parentClass == null) return resolvedObjectType;
        if (GutilClass.isNormalClass(parentClass.getErasedType())) {
            ResolvedObjectType newparentClass = ResolvedObjectType.create(parentClass.getErasedType()
                    , typeBindings
                    , parentClass.getParentClass()
                    , parentClass.getImplementedInterfaces());
            ...
            return ResolvedObjectType.create(resolvedObjectType.getErasedType(), typeBindings
                    , newNewParentClass, resolvedObjectType.getImplementedInterfaces());
        }
        return resolvedObjectType;
    }
}
