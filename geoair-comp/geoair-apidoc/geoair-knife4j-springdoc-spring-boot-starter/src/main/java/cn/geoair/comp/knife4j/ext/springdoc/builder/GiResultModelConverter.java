...
public class GiResultModelConverter implements ModelConverter {
	...
	private void addParentResultFields(ObjectSchema resultSchema) {
		resultSchema.addProperty("code", new IntegerSchema().description("响应码").example(200))
	...
	private void addGirWebResultFields(ObjectSchema resultSchema) {
		try {
			...
			// 解析@GaModelField注解
			GaModelField gaModelField = AnnotationUtils.findAnnotation(locationField, GaModelField.class);
			...
			// 添加location字段到Schema
			resultSchema.addProperty("location", new StringSchema().description(fieldDesc).example("/index"));
		}
