...
public class AutoApiConfig implements GirSwaggerApiConfig, ApplicationContextAware {
	...
	public List<DocketInfo> getDocketInfos() {
		...
		if (!"true".equals(applicationContext.getEnvironment().getProperty("geoair.apidoc.enable"))) {
	...
	public ApiModelInfo getApiModelInfo() {
		return new ApiModelInfo(swaggerProperties.getTitle(), swaggerProperties.getDescription(),
				swaggerProperties.getAuthor(), swaggerProperties.getVersion());
	}
