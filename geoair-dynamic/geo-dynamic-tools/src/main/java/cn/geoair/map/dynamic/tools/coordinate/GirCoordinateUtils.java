...
/**
 * 坐标转换工具类（单例模式）
 * 实现WGS84/GCJ02/BD09坐标系互转、墨卡托投影转换、坐标格式标准化等能力
 * 核心算法参考：高德/百度地图坐标系转换规范
 *
 * @author 张逢吉
 * @date 2024/12/05
 */
public class GirCoordinateUtils implements GirCoordinateConvertOpt {
     ...
     * 墨卡托转WGS84
     */
    public double[] mercatorToWgs84(double mercatorX, double mercatorY, boolean ifExceptionReturnNull) {
        try {
    ...
    /**
     * 度分秒转十进制度
     * 支持格式：116°23′45.6″E, 39°54′32.1″N 或 116°23'45.6"E 39°54'32.1"N
     */
    public double[] dmsToDd(String dmsStr, boolean ifExceptionReturnNull) {
        try {
            ...
            // 清理特殊字符，统一分隔符
            String cleanStr = StrUtil.replace(dmsStr, "′", "'").replace("″", "\"").replace("°", "d");
            String[] coordParts = StrUtil.split(cleanStr, StrUtil.COMMA + StrUtil.SPACE).toArray(new String[0]);
     ...
     * 解析坐标字符串为十进制度
     */
    public double[] parseCoordString(String coordStr, String separator, boolean ifExceptionReturnNull) {
        try {
            ...
            if (NumberUtil.isNumber(parts[0]) && NumberUtil.isNumber(parts[1])) {
                return new double[]{NumberUtil.parseDouble(parts[0]), NumberUtil.parseDouble(parts[1])};
            }
     ...
     * @return 转换后坐标二维数组
     */
    private double[][] batchConvert(double[][] coords, boolean ifExceptionReturnNull,
     ...
     * @return 转换后几何对象
     */
    private Geometry convertGeometry(Geometry geometry, boolean ifExceptionReturnNull,
                                     BiFunction<Double, Double, double[]> converter) {
        try {
            ...
            if (geometry instanceof Point) {
                ...
                for (int i = 0; i < holes.length; i++) {
                    ...
                    Coordinate[] newHoleCoords = Arrays.stream(holeCoords)
                            .map(c -> {
                ...
                return GEOMETRY_FACTORY.createMultiLineString(new LineString[]{
                        GEOMETRY_FACTORY.createLineString(newCoords)
                });
