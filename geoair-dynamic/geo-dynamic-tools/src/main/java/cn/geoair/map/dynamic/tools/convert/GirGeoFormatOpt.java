...
/**
 * 空间类型转换核心接口
 * 定义GeoJSON/WKT/WKB/JTS/PGGeometry等空间类型的互转规范
 *
 * @author 张逢吉
 * @date 2024/10/24
 */
public interface GirGeoFormatOpt {
    ...
    /**
     * 获取 WKT 解析器实例（Well-Known Text Reader），用于将 WKT 文本格式转换为 {@link org.locationtech.jts.geom.Geometry} 几何对象。
     * <p>
     * 核心特性：
     * 1. 支持解析标准 WKT 格式，以及带 Z/M/ZM 维度扩展的 WKT 语法（JTS 1.15+）；
     * 2. 可通过配置修复结构无效的输入（如缺失坐标的线串），但不保证拓扑有效性；
     * 3. 关键字大小写不敏感，支持非标准 "LINEARRING" 标签；
     * 4. 数值解析兼容 Java 浮点字面量语法（含科学计数法）。
     * <p>
     * 线程安全：<b>非线程安全</b>，每个线程应使用独立实例，禁止多线程共享。
     * 若需复用，建议通过 ThreadLocal 为每个线程缓存实例以提升性能。
     *
    ...
    /**
     * 获取 WKB 写入器实例（Well-Known Binary Writer），用于将 {@link org.locationtech.jts.geom.Geometry} 几何对象转换为 WKB 二进制格式。
     * <p>
     * 核心特性：
     * 1. 遵循 OGC Simple Features 规范，兼容 PostGIS 扩展 EWKB 格式（支持 3D 坐标、SRID 写入）；
     * 2. 对空几何有明确的序列化规则（如空点用 NaN 坐标表示，空线串用 0 个点表示）；
     * 3. LinearRing 会被序列化为 LineString 类型的 WKB；
    ...
    /**
     * 获取 WKB 解析器实例（Well-Known Binary Reader），用于将 WKB 二进制格式转换为 {@link org.locationtech.jts.geom.Geometry} 几何对象。
     * <p>
     * 核心特性：
     * 1. 支持标准 WKB 和 PostGIS EWKB 格式，JTS 1.15+ 额外兼容 ISO/OGC 19125 规范（Spatialite/Geopackage）；
     * 2. 自动修复结构无效的输入（如顶点不足的线串/环、未闭合的环）；
     * 3. 校验畸形/恶意 WKB 数据（如字段值超限、读取越界），异常时抛出 ParseException；
     * 4. 空点兼容 NaN 坐标表示，SRID 未指定时继承父几何的 SRID（默认 SRID=0）。
     * <p>
     * 线程安全：<b>非线程安全</b>，每个线程应使用独立实例，禁止多线程共享。
     *
     ...
     * <p>
     * 核心特性：
     * 1. 遵循 RFC 7946 标准，支持点、线、面、多几何、几何集合的 GeoJSON 解析/生成；
     * 2. 可配置坐标精度（默认保留 15 位小数），避免浮点精度冗余；
