...
/**
 * WGS84（4326）等轴瓦片转换实现类
 * 核心特征：经度/纬度轴使用相同瓦片跨度（均为360/2^z），兼容Mapbox4490逻辑
 */
public class Wgs84EqualAxisTileUtils extends AbstractWgs84TileConverter {
    ...
    public BoxReferencedEnvelope xyzToTileBox(int z, int x, int y, int targetSrid) {
        ...
        // 转换为目标坐标系
        ReferencedEnvelope envelope4326 = new ReferencedEnvelope(lon_min, lon_max, lat_min, lat_max, sridConvertOpt.getCRS(4326));
    ...
    public RangeApo tileRangeByBox(int z, Envelope envelope) {
        ...
        // 处理空范围
        if (envelope.isNull() || (Math.abs(envelope.getMaxX() - envelope.getMinX()) < PRECISION) || (Math.abs(envelope.getMaxY() - envelope.getMinY()) < PRECISION)) {
            double centerX = (envelope.getMinX() + envelope.getMaxX()) / 2;
            double centerY = (envelope.getMinY() + envelope.getMaxY()) / 2;
            envelope = new Envelope(centerX - POINT_OFFSET, centerX + POINT_OFFSET, centerY - POINT_OFFSET, centerY + POINT_OFFSET);
        }
