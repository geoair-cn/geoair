...
/**
 * 几何对象合并核心接口
 * 支持线/面的多格式合并（Geometry对象/坐标数组/WKT），兼容单对象/多对象合并场景
 *
 * @author 张逢吉
 * @date 2024/12/05
 */
public interface GirGeoMergeOpt {
     ...
     * @param wktList LineString WKT数组（如["LINESTRING(x1 y1,x2 y2)","LINESTRING(x3 y3,x4 y4)"]）
     * @return 合并后的MultiLineString
     */
     ...
     * @param wktList Polygon WKT数组（如["POLYGON((x1 y1,x2 y2,...))","POLYGON((x3 y3,x4 y4,...))"]）
     * @return 合并后的MultiPolygon
     */
