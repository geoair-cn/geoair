...
public interface GirBingMapQuadKeyOpt {
    ...
    /**
     * 根据QuadKey获取指定级别的所有QuadKey（向上聚合/向下细化）
     * 规则：
     * 1. 目标级别 < 当前级别：向上聚合 → 返回1个父级QuadKey
     * 2. 目标级别 = 当前级别：返回原QuadKey（数组长度1）
     * 3. 目标级别 > 当前级别：向下细化 → 返回4ⁿ个QuadKey（n=级别差）
     *
    ...
    default List<String> xyzToQuadKeyBatch(TileZxyApo[] apoArray) {
        ...
        return Arrays.stream(apoArray).filter(Objects::nonNull)
                .map(apo -> xyzToQuadKey(apo.getX(), apo.getY(), apo.getZ())).filter(Objects::nonNull)
