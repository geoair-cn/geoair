...
/**
 * SRID坐标转换核心接口
 * 基于GeoTools实现不同空间参考系（EPSG SRID）的几何对象转换
 *
 * @author 张逢吉
 * @date 2024/12/05
 */
public interface GirSridConvertOpt {
     ...
     * @return 转换后的几何对象
     */
    Geometry convertToGeom(Envelope envelope, int srcSrid, int targetSrid, boolean ifExceptionReturnNull);
     ...
     * @return 转换后的坐标数组 [x, y]
     */
    double[] convertPoint(double lng, double lat, int srcSrid, int targetSrid, boolean ifExceptionReturnNull);
