...
public abstract class TileConverterCommon implements GirTileConverterOpt {
    ...
    public String xyzToWkt(int z, int x, int y, int targetSrid) {
        ...
        Geometry geometry = GirAdvTools.getSridOpt().convertToGeom(envelope, envelope.getCoordinateReferenceSystem().getCoordinateSystem().getDimension(), targetSrid);
    ...
    public Set<TileZxyApo> zxyListByGeom(Geometry geometry, int srcSrid, int targetZ) {
        ...
        return zxySet.stream().distinct().map(s -> {
            List<String> split = StrUtil.split(s, "|");
            return new TileZxyApo(Integer.parseInt(split.get(0)), Integer.parseInt(split.get(1)), Integer.parseInt(split.get(2)));
