...
/**
 * 空间几何面积/长度/距离计算工具类（单例模式）
 * 基于JTS+GeoTools实现，支持多输入格式、多单位转换
...
public class GirGeoMeasureUtils implements GirGeoMeasureOpt {
    ...
    public double calculatePointToLineMinDistance(double[] point, double[][] lineCoords, int srid, String unit) {
        try {
            ...
            Coordinate[] lineCoordinates = Arrays.stream(lineCoords)
                    .map(coord -> new Coordinate(coord[0], coord[1]))
                    .toArray(Coordinate[]::new);
    ...
    @Override
    public double calculateLineToLineMinDistance(double[][] line1Coords, double[][] line2Coords, int srid, String unit) {
        try {
            Coordinate[] line1CoordsArr = Arrays.stream(line1Coords)
                    .map(coord -> new Coordinate(coord[0], coord[1]))
                    .toArray(Coordinate[]::new);
            LineString line1 = GEOMETRY_FACTORY.createLineString(line1CoordsArr);

            Coordinate[] line2CoordsArr = Arrays.stream(line2Coords)
                    .map(coord -> new Coordinate(coord[0], coord[1]))
                    .toArray(Coordinate[]::new);
