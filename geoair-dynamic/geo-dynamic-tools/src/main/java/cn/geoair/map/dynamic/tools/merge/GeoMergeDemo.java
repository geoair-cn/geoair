...
public class GeoMergeDemo {
    public static void main(String[] args) {
        ...
        LineString line1 = gf.createLineString(new Coordinate[]{new Coordinate(116.40, 39.90), new Coordinate(116.41, 39.90)});
        LineString line2 = gf.createLineString(new Coordinate[]{new Coordinate(116.41, 39.90), new Coordinate(116.42, 39.90)});
        MultiLineString multiLine = mergeUtils.mergeToMultiLineString(new LineString[]{line1, line2});
        System.out.println("MultiLineString WKT: " + multiLine.toText());

        // 2. 首尾衔接的线合并为单个LineString
        LineString singleLine = mergeUtils.mergeToSingleLineString(new LineString[]{line1, line2});
        System.out.println("Single LineString WKT: " + singleLine.toText());

        // 3. Polygon数组拓扑合并为单个Polygon（重叠面）
        Polygon poly1 = gf.createPolygon(new Coordinate[]{
                new Coordinate(116.40, 39.90),
                new Coordinate(116.41, 39.90),
