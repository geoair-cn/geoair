...
/**
 * 基于GeoTools的SRID坐标转换工具类（单例模式）
 * 支持EPSG标准SRID互转，内置常用CRS缓存
 *
 * @author 张逢吉
 * @date 2024/12/05
 */
...
public class GirSridConvertUtils implements GirSridConvertOpt {
    ...
    @Override
    public Geometry convert(Geometry geometry, int srcSrid, int targetSrid, boolean ifExceptionReturnNull) {
        try {
            // 参数校验
    ...
    public Envelope convert(Envelope envelope, int srcSrid, int targetSrid, boolean ifExceptionReturnNull) {
        try {
            // 源SRID与目标SRID相同，直接返回原对象
            if (srcSrid == targetSrid) {
                return envelope;
    ...
    @Override
    public Geometry convertToGeom(Envelope envelope, int srcSrid, int targetSrid, boolean ifExceptionReturnNull) {
    ...
    @Override
    public double[] convertPoint(double lng, double lat, int srcSrid, int targetSrid, boolean ifExceptionReturnNull) {
        try {
            // 构建点几何对象
            ...
            // 转换点对象
            Point transformedPoint = (Point) convert(point, srcSrid, targetSrid, ifExceptionReturnNull);
    ...
    private final org.locationtech.proj4j.CRSFactory CRS_FACTORY = new org.locationtech.proj4j.CRSFactory();
    ...
    public boolean isGeographicCRS(int srid) {
        ...
        try {
            org.locationtech.proj4j.CoordinateReferenceSystem crs = CRS_FACTORY.createFromName("EPSG:" + srid);
