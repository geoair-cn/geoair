...
public class StatisticUtils {
	...
	public static void statAndWriteJson(JavaRDD<Tuple2<String, PbfInfo>> infoRdd, String geometryType,
			String staticTableName, TileSliceParameter parameter, Long totalFeatureCount, SparkSession sparkSession) {
		try {

			// 根据FeatureRowID进行去重
			JavaPairRDD<String, Tuple2<String, GirAdvOneRow>> featureIdRdd = infoRdd.flatMapToPair(tuple -> {
				String tileKey = tuple._1;
				PbfInfo pbfInfo = tuple._2;
				List<GirAdvOneRow> featureList = pbfInfo.getThisPbfFeatureList() == null ? new ArrayList<>()
				...
				List<Tuple2<String, Tuple2<String, GirAdvOneRow>>> result = new ArrayList<>();
				for (GirAdvOneRow row : featureList) {
					// 严格过滤无效数据
					if (row == null)
						continue;
					Object featureRowIdObj = row.get(VecConstant.FeatureRowID);
					if (featureRowIdObj == null)
						continue;
					String featureRowId = featureRowIdObj.toString();
					result.add(new Tuple2<>(featureRowId, new Tuple2<>(tileKey, row)));
				}
				featureList.clear();
				return result.iterator();
			}).reduceByKey((t1, t2) -> t1);

			JavaPairRDD<String, GirAdvOneRow> tileKeyRdd = featureIdRdd.mapToPair(tuple -> {
			...
			// 3. 直接按原始TileKey聚合
			JavaPairRDD<String, List<GirAdvOneRow>> tileRdd = tileKeyRdd
					.combineByKey(row -> new ArrayList<>(Collections.singletonList(row)), (list, row) -> {
						list.add(row);
						return list;
					}, (list1, list2) -> {
						list1.addAll(list2);
			...
			JavaPairRDD<String, Tuple2<Object, Long>> fieldValueRDD = flatRDD
					// 先做map转换，用完即丢弃原对象
					.map(row -> {
						...
						for (String fieldName : fieldNamesByMap) {
							tempMap.put(fieldName, FieldStatUtils.getFieldValue(row, fieldName));
						}
						return tempMap;
					}).flatMapToPair(tempMap -> {
						List<Tuple2<String, Tuple2<Object, Long>>> result = new ArrayList<>();
						for (Map.Entry<String, Object> entry : tempMap.entrySet()) {
							String fieldName = entry.getKey();
							Object value = entry.getValue();
							Object finalValue = value == null ? "" : value;
							result.add(new Tuple2<>(fieldName, new Tuple2<>(finalValue, 1L)));
						}
						tempMap.clear();
						return result.iterator();
					});

			JavaPairRDD<String, Map<Object, Long>> fieldValueCountRDD = fieldValueRDD.aggregateByKey(new HashMap<>(),
					(map, tuple) -> {
						...
						if (map.size() == MAX_VALUE_COUNT_PER_FIELD) {
							System.out.printf("%s [内存优化] 字段值数量达到阈值%d，停止统计新值 %n", LOG_PREFIX, MAX_VALUE_COUNT_PER_FIELD);
						}
						return map;
					}, (map1, map2) -> {
			...
			JavaRDD<AttributeStat> attrStatRDD = fieldValueCountRDD.map(tuple -> {
				...
				if (!valueCount.isEmpty()) {
					...
					if ("Number".equals(fieldType)) {
						try {
							...
							System.err.printf("%s [字段类型处理] 字段[%s]值类型转换失败：%s %n", LOG_PREFIX, fieldName, e.getMessage());
						}
					}
					else {
						attrStat.setMin(null);
			...
			// 11. 构建主图层JSON并写入
			TileStatRoot root = buildTileStatRoot(parameter.getLayerName(), geometryType, totalFeatureCount, fieldNames,
					attributeStats, parameter);
			writeJsonToDB(root, staticTableName, parameter.getLayerName(), parameter.getEdition(), sparkSession,
					parameter, 3);

			// 12. 标签图层（如有）
			if (parameter.isCreateLabel()) {
				TileStatRoot rootLabel = buildTileStatRoot(parameter.getLayerNameLabel(), "Point", totalFeatureCount,
						fieldNames, attributeStats, parameter);
				writeJsonToDB(rootLabel, staticTableName, parameter.getLayerNameLabel(), parameter.getEdition(),
						sparkSession, parameter, 3);
			}
			else {
			...
			if (parameter.isCreateBoundary()) {
				TileStatRoot rootBoundary = buildTileStatRoot(parameter.getLayerNameBoundary(), "LineString",
						totalFeatureCount, fieldNames, attributeStats, parameter);
				writeJsonToDB(rootBoundary, staticTableName, parameter.getLayerNameBoundary(), parameter.getEdition(),
						sparkSession, parameter, 3);
			}
			else {
	 ...
	 */
	private static TileStatRoot buildTileStatRoot(String layerId, String geometryType, long totalFeatureCount,
			List<String> fieldNames, List<AttributeStat> attributeStats, TileSliceParameter parameter) {
		...
		for (AttributeStat attr : attributeStats) {
			String type = attr.getType() == null ? "String" : attr.getType().equalsIgnoreCase("number") ? "Number"
					: attr.getType().equalsIgnoreCase("boolean") ? "Boolean" : "String";
		...
		System.out.printf("%s [构建JSON] 图层[%s]的统计JSON结构构建完成，耗时=%dms，JSON长度=%d %n", LOG_PREFIX, layerId,
				buildEndTime - buildStartTime, JSON.toJSONString(root).length());
		return root;
	}
	 ...
	 * 写入JSON到数据库（带重试机制）
	 */
	private static void writeJsonToDB(TileStatRoot root, String staticTableName, String layerName, String version,
			SparkSession sparkSession, TileSliceParameter parameter, int retryTimes) {
		String jsonStr = JSON.toJSONString(root, false);
		System.out.printf("%s [写入数据库] 准备写入图层[%s]到表[%s]，JSON长度=%d %n", LOG_PREFIX, layerName, staticTableName,
				jsonStr.length());

		// 定义DataFrame的Schema
		StructType schema = DataTypes
				.createStructType(Arrays.asList(DataTypes.createStructField("track_id", DataTypes.StringType, true),
						DataTypes.createStructField("layer_name", DataTypes.StringType, true),
						DataTypes.createStructField("version", DataTypes.StringType, true),
						DataTypes.createStructField("time_create", DataTypes.StringType, true),
						DataTypes.createStructField("timestamp", DataTypes.LongType, true),
						DataTypes.createStructField("json_str", DataTypes.StringType, true)));

		StaticLog.info("TrackId:{},layerName:{},version:{}", parameter.getTrackId(), parameter.getLayerName(), version);

		// 构建单行DataFrame
		Row row = RowFactory.create(parameter.getTrackId(), layerName, version, DateUtil.now(),
				System.currentTimeMillis(), jsonStr);
		...
		for (int i = 0; i < retryTimes; i++) {
			try {
				...
				System.out.printf("%s [写入数据库-重试%d] 开始写入图层[%s]到PostgreSQL %n", LOG_PREFIX, i + 1, layerName);

				df.write().format("jdbc").option("url", pgParams.get("url"))
						.option("dbtable", StrUtil.wrap(staticTableName, "\"")).option("user", pgParams.get("user"))
						.option("password", pgParams.get("password"))
						.option("batchsize", pgParams.getOrDefault("batchSize", "1000"))
						.option("rewriteBatchedStatements", "true").mode("append").save();

				long writeEndTime = System.currentTimeMillis();
				System.out.printf("%s [写入数据库-重试%d] 图层[%s]写入成功，耗时=%dms %n", LOG_PREFIX, i + 1, layerName,
				...
				System.err.printf("%s [写入数据库-重试%d] 图层[%s]写入失败：%s，休眠%dms后重试...%n", LOG_PREFIX, i + 1, layerName,
						e.getMessage(), sleepTime);
				try {
		...
		// 所有重试失败，抛出异常
		String errorMsg = String.format("%s 写入数据库失败（重试%d次），表名：%s，layer：%s", LOG_PREFIX, retryTimes, staticTableName,
				layerName);
