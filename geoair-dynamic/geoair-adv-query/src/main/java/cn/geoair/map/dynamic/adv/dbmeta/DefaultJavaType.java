...
public enum DefaultJavaType implements DataType {
	...
	JAVA_CHAR("CHAR", String.class, 0, 1, 1) {
	}, // 对应PG CHAR类型
	JAVA_TEXT("TEXT", String.class, 1, 1, 1) {
	}, // 对应PG TEXT类型（无长度限制，忽略长度）
	JAVA_UUID("UUID", UUID.class, 1, 1, 1) {
	}, // 对应PG UUID类型
	JAVA_JSON("JSON", String.class, 1, 1, 1) {
	}, // 对应PG JSON类型
	JAVA_JSONB("JSONB", String.class, 1, 1, 1) {
	}, // 对应PG JSONB类型
	JAVA_CITEXT("CITEXT", String.class, 0, 1, 1) {
	}, // 对应PG 大小写不敏感文本类型
	...
	JAVA_SHORT("SMALLINT", Short.class, 1, 1, 1) {
	}, // 对应PG SMALLINT类型
	JAVA_BYTE("TINYINT", Byte.class, 1, 1, 1) {
	}, // 对应PG TINYINT（扩展类型）
	JAVA_BIGINT("BIGINT", Long.class, 1, 1, 1) {
	}, // 对应PG BIGINT类型
	JAVA_SERIAL("SERIAL", Integer.class, 1, 1, 1) {
	}, // 对应PG SERIAL自增类型
	JAVA_BIGSERIAL("BIGSERIAL", Long.class, 1, 1, 1) {
	}, // 对应PG BIGSERIAL自增类型
	...
	JAVA_NUMERIC("NUMERIC", BigDecimal.class, 1, 0, 0) {
	}, // 对应PG NUMERIC类型（与DECIMAL等价）
	JAVA_REAL("REAL", Float.class, 1, 0, 0) {
	}, // 对应PG REAL类型（单精度浮点）
	...
	JAVA_OFFSET_DATE_TIME("TIMESTAMPTZ", OffsetDateTime.class, 1, 1, 1) {
	}, // 对应PG带时区时间
	JAVA_OFFSET_TIME("TIMETZ", OffsetTime.class, 1, 1, 1) {
	}, // 对应PG带时区时间
	JAVA_INTERVAL("INTERVAL", String.class, 1, 2, 3) {
	}, // 对应PG INTERVAL类型（映射为String，需自定义解析）
	 ...
	 *
	 ****************************************************************************************************************/
	JAVA_BYTES("BYTEA", Byte[].class, 1, 1, 1) {
	}, // 对应PG BYTEA类型
	JAVA_BLOB("BLOB", Byte[].class, 1, 1, 1) {
	}, // 对应PG BLOB（通过扩展类型支持）
	JAVA_GEOMETRY("GEOMETRY", Byte[].class, 1, 1, 1) {
	}, // 对应PG GEOMETRY空间类型
	 ...
	 *
	 ****************************************************************************************************************/
	JAVA_MONEY("MONEY", BigDecimal.class, 1, 0, 0) {
	}; // 对应PG MONEY类型（映射为BigDecimal避免精度丢失）
