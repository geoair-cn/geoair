...
/**
 * 解决 Spring Security 对 PasswordEncoder 的歧义依赖： 1. 给 GeoServer 的 pbePasswordEncoder 标记 @Primary 2.
 * 确保在 Spring Security 自动配置前执行
 */
@Component
...
public class PasswordEncoderPrimaryResolver implements BeanDefinitionRegistryPostProcessor {
    ...
    public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry)
        ...
        if (registry.containsBeanDefinition(PRIMARY_PASSWORD_ENCODER)) {
            ...
            Gir.log.info(
                    "[GeoServer Jar] 已将 "
                            + PRIMARY_PASSWORD_ENCODER
                            + " 标记为 @Primary，解决 PasswordEncoder 歧义");
        } else {
            ...
            if (registry.containsBeanDefinition(fallbackEncoder)) {
                ...
                Gir.log.info(
                        "[GeoServer Jar] pbePasswordEncoder 不存在，将 "
                                + fallbackEncoder
                                + " 标记为 @Primary");
            }
        }
