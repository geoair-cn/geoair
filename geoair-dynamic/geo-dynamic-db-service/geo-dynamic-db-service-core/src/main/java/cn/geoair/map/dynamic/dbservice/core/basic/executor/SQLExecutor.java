...
import cn.geoair.map.dynamic.dbservice.core.basic.dto.ApiSqlDto;
import cn.geoair.map.dynamic.dbservice.core.basic.dto.SQLTaskDto;
...
import cn.geoair.map.dynamic.dbservice.core.basic.util.SqlEngineUtil;
import cn.geoair.map.dynamic.dbservice.core.dialect.BaseDialect;
...
public class SQLExecutor implements Executor {
	...
	public Object execute(JSONObject taskJson, Map<String, Object> sqlParam) throws Exception {
		...
		DataSourceApo datasource = dataSourceService.detail(task.getDatasourceId());
		if (datasource == null) {
	...
	public List<Object> executeSql(Connection connection, List<ApiSqlDto> sqlList, Map<String, Object> sqlParam,
		...
		try {
			...
			for (ApiSqlDto apiSql : sqlList) {
				...
				Object data = JdbcUtil.executeSql(connection, sqlMeta.getSql(), sqlMeta.getJdbcParamValues(), flag);
				dataList.add(data);
			}
			if (flag)
				connection.commit();
	...
	public List<Object> executeSql(Connection connection, SQLTaskDto task, Map<String, Object> sqlParam,
		...
		try {
			...
			for (ApiSqlDto apiSql : sqlList) {
				...
				if (task.pageIs()) {
					...
					Long count = JdbcUtil.executeSqlCount(connection, countSql, sqlMeta.getJdbcParamValues());
					String sql = instance.getPageSql(sqlMeta.getSql(), giPageParam.pageNum(), pageSize);
					List data = (List) JdbcUtil.executeSql(connection, sql, sqlMeta.getJdbcParamValues(),
							task.humpIs());
