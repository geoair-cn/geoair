...
public class ApiConfigService {
	...
	public List<JSONObject> getAllApiTree() {
		...
		List<Group> groups = dbApiGroupDao.gtcSearchAll().stream().map(Group::fromPo).collect(Collectors.toList());
		List<JSONObject> list = groups.stream().sorted(Comparator.comparing(Group::getUpdateTime)).map(g -> {
			...
			List<JSONObject> children = apiConfigs.stream().sorted(Comparator.comparing(ApiConfig::getUpdateTime))
					.map(t -> {
