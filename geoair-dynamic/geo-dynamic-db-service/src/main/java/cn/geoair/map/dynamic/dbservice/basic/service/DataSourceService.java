...
import cn.geoair.map.dynamic.dbservice.basic.util.DESUtils;
 ...
 * 数据源服务类
 *
 * <p>提供数据源的增删改查操作，包括： - 数据源的新增和批量插入 - 数据源的更新（支持密码加密） - 数据源的删除（检查是否被API使用） - 数据源的详情查询和缓存 -
 * 所有数据源的列表查询
 *
 * <p>使用Spring缓存管理器进行缓存控制，支持事务管理
 *
 * @author: zhangfengji
...
public class DataSourceService {
     ...
     * 删除数据源
     *
     * <p>删除前检查该数据源是否被任何API配置使用， 如果被使用则返回错误信息，否则执行删除操作 同时清除连接池和缓存
     *
     * @param id 待删除的数据源ID
     ...
     * 批量插入数据源
     *
     * <p>批量插入数据源列表，为每个数据源设置更新时间和创建元数据
     *
