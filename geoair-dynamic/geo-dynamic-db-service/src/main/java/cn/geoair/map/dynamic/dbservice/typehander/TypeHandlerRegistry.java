...
import cn.geoair.map.dynamic.dbservice.typehander.dm.DmGeomTypeHandler;
import cn.geoair.map.dynamic.dbservice.typehander.oracle.OracleBlobTypeHandler;
import cn.geoair.map.dynamic.dbservice.typehander.oracle.OracleGeomTypeHandler;
import cn.geoair.map.dynamic.dbservice.typehander.pg.PgGeomTypeHandler;
import cn.geoair.map.dynamic.dbservice.typehander.pg.PostGisGeomTypeHandler;
...
public class TypeHandlerRegistry {
	...
	static {
		try {
			...
			TypeHandlerRegistry.register(net.postgis.jdbc.PGgeometry.class, Singleton.get(PgGeomTypeHandler.class));
			TypeHandlerRegistry.register(net.postgis.jdbc.geometry.Geometry.class,
					Singleton.get(PostGisGeomTypeHandler.class));
			TypeHandlerRegistry.register(1111, Singleton.get(PgGeomTypeHandler.class));
			System.out.println("PostGIS类型处理器注册成功");
		}
		catch (Throwable e) {
	...
	static {
		try {
			// 达梦数据库相关类型处理器注册
			TypeHandlerRegistry.register(dm.jdbc.driver.DmdbStruct.class, Singleton.get(DmGeomTypeHandler.class));
			System.out.println("达梦数据库类型处理器注册成功");
		}
		catch (Throwable e) {
	...
	static {
		try {
			...
			System.out.println("Oracle BLOB/CLOB类型处理器注册成功");
		}
		catch (Throwable e) {
	...
	static {
		try {
			...
			System.out.println("Oracle空间类型处理器注册成功");
		}
		catch (Throwable e) {
