<template>
  <div>
    <el-tag :key="tag.id" v-for="tag in groups" :title="'id: ' + tag.id" closable :disable-transitions="false"
            @close="handleClose(tag.id)" effect="dark">
      {{ tag.name }}
    </el-tag>
...
export default {
  ...
  methods: {
    ...
    createGroup() {
      dbApi.createGroup(this.inputValue)
          .then((response) => {
            this.getAllGroups()
          }).catch((error) => {
