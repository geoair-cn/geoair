...
public class GeoServerSecurityConfig {
	...
	public UserDetailsService userDetailsService() {
		...
		UserDetails defaultUser = User.withUsername("admin").passwordEncoder(new BCryptPasswordEncoder()::encode)
				.password("geoserver").roles("ADMIN").build();
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
				...
				.authorizeHttpRequests(auth -> auth.antMatchers("/geoserver/**").permitAll() // 核心：/geoserver/**
																								// 无需登录
						.anyRequest().permitAll() // 其他路径也允许匿名
				).formLogin().disable().httpBasic().disable().csrf().disable().logout().disable().sessionManagement()
				.disable();
