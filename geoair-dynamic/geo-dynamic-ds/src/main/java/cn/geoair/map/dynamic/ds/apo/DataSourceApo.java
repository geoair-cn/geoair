 ...
 * <p>
 * 用于在应用程序各层之间传递数据源相关信息的数据载体， 包含数据库连接所需的各类配置参数及元数据信息
 * </p>
 */
@Data
public class DataSourceApo implements Serializable {
	...
	public boolean equals(String address, Integer port, String dbName, String user) {
		return this.address.equals(address) && this.port.equals(port) && this.dbName.equals(dbName)
	...
	public static String buildJdbcUrl(DataSourceApo dataSourceApo) {
		...
		case ORACLE:
			...
			break;
		case POSTGRESQL:
			jdbcUrl = String.format("jdbc:postgresql://%s:%d/%s%s", address, port, dbName,
