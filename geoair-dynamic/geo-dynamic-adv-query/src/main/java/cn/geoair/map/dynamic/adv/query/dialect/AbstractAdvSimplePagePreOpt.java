...
public abstract class AbstractAdvSimplePagePreOpt extends AbstractAdvSimplePageOpt implements IAdvSimplePagePreOpt {
     ...
     * 执行带参数的分页查询，返回结果列表
     */
    protected List<GirAdvOneRow> executePageSqlWithParam(String pageSql, SqlParamMap sqlParam, AdvEnumsGeomOpt advEnumsGeomOpt, List<String> geomFieldNameList) {
        return getAdvGeoPreOpt().eSelectList(pageSql, sqlParam, advEnumsGeomOpt, geomFieldNameList);
    }

    @Override
    protected List<GirAdvOneRow> executePageSql(String pageSql, AdvEnumsGeomOpt advEnumsGeomOpt, List<String> geomFieldNameList) {
