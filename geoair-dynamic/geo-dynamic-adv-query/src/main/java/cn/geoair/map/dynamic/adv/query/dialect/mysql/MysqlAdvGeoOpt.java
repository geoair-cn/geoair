...
public class MysqlAdvGeoOpt extends AbstractAdvGeoOpt {
    ...
    protected AdvEnumsTypeGeom getTypeGeomEnum(String nativeGeomType) {
        ...
        // MySQL几何类型：POINT, MULTIPOINT, LINESTRING, MULTILINESTRING, POLYGON, MULTIPOLYGON, GEOMETRY
        ...
        try {
            ...
            if (metaData != null) {
                for (int i = 1; i <= metaData.getColumnCount(); i++) {
                    ...
                    // 判断MySQL空间类型
                    if (Arrays.asList("GEOMETRY", "POINT", "LINESTRING", "POLYGON", "MULTIPOINT", "MULTILINESTRING", "MULTIPOLYGON").contains(colType.toUpperCase())) {
                        return colName;
                    }
                }
            }
    ...
    public Integer eGetSrid(String tableNameOrSqlView, String geomFieldName) {
        ...
        String sql = StrUtil.format("SELECT ST_SRID({}) AS srid FROM {} LIMIT 1;",
    ...
    public void eAddGeomColumn(String tableName, String geomFieldName, AdvEnumsTypeGeom geomType, int srid) {
        ...
        String sql = StrUtil.format(
                ...
                qualifiedTableName, geomFieldName, geomType.getCode().toUpperCase(), srid
        );
    ...
    public void eTransformSrid(String tableName, String geomFieldName, int targetSrid) {
        ...
        try {
            ...
            String createTempSql = StrUtil.format(
                    ...
                    qualifiedTableName, tempGeomField, geomType.getCode().toUpperCase()
            );
            ...
            // 3. 修改临时字段SRID
            String alterSridSql = StrUtil.format(
                    "ALTER TABLE {} MODIFY COLUMN {} {} SRID {};",
                    qualifiedTableName, tempGeomField, geomType.getCode().toUpperCase(), targetSrid
            );
    ...
    public String getCentroidSql(String geomFieldName, String centerAlias, String qualifiedTableName) {
        // MySQL: ST_Centroid
        return StrUtil.format("SELECT *, ST_Centroid({}) AS {} FROM {};", geomFieldName, centerAlias, qualifiedTableName);
    }
