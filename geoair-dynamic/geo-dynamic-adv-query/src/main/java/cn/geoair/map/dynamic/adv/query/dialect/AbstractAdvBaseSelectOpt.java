...
public abstract class AbstractAdvBaseSelectOpt implements IAdvBaseSelectOpt {
    ...
    public Number bSelectRecordRowCount(String sqlStatement, SqlParamMap sqlParam) {
        ...
        Connection connection = dataSourceGetter.getConnection();
        try {
            return SqlExecutor.query(connection, countSqlMeta.getSql(), new NumberHandler(), countSqlMeta.getJdbcParamValues());
        } catch (SQLException e) {
    ...
    protected void logExecuteSql(String methodName, String sql) {
        log.info("schema:[{}] db:[{}] {}执行的SQL为：{}",
                getSchemaName(), getDataSourceId(), methodName, sql);
    }
    ...
    protected void logExecuteSql(String methodName, String sql, SqlParamMap sqlParam) {
        log.info("schema:[{}] db:[{}] {}执行的SQL为：{}，参数：{}",
                getSchemaName(), getDataSourceId(), methodName, sql, sqlParam);
    }
    ...
    protected String buildSelectOneWrapSql(String cleanSql) {
        return StrUtil.format("select * from ({}) as {} limit 1",
