...
/**
 * PostgreSQL（PostGIS）空间操作实现类
 * 复用你原有PgAdvGeoOpt + PgAdvGeoPreOpt的核心逻辑
 */
public class PgAdvGeoOpt extends AbstractAdvGeoOpt {
    ...
    public Map<String, AdvEnumsTypeGeom> eGetGeoTypeByTable(String tableName, List<String> geomFieldNames) {
        ...
        String sql = StrUtil.format("SELECT {} FROM {} WHERE {} LIMIT 1;",
                fieldsSql.toString(), qualifiedTableName, whereSql.toString());
    ...
    public List<String> eGetGeomColumnNameListByTable(String tableName) {
        ...
        String schemaFilter = StrUtil.isEmpty(schemaName) ? "" : StrUtil.format("AND \"table_schema\" = '{}'", schemaName);
        ...
        try {
            ...
            if (metaData != null) {
                for (int i = 1; i <= metaData.getColumnCount(); i++) {
                    ...
                    if ("geometry".equals(colType) || "geography".equals(colType) || "\"public\".\"geometry\"".equals(colType)) {
                        return colName;
                    }
                }
            }
    ...
    public Integer eGetSrid(String tableNameOrSqlView, String geomFieldName) {
        ...
        String sql = StrUtil.format("SELECT public.st_srid({}) AS srid FROM {} LIMIT 1;",
                geomFieldName, qualifiedName);
        GirAdvOneRow row = baseOpt.bSelectOne(sql);
    ...
    public Map<String, Integer> eGetSrid(String tableNameOrSqlView, List<String> geomFieldNames) {
        ...
        String sql = StrUtil.format("SELECT {} FROM {} {} LIMIT 1;",
                sridSelect.toString(), qualifiedName, where);
        GirAdvOneRow row = baseOpt.bSelectOne(sql);
    ...
    public void eAddGeomColumn(String tableName, String geomFieldName, AdvEnumsTypeGeom geomType, int srid) {
        ...
        String sql = StrUtil.format(
                ...
                qualifiedTableName, geomFieldName, geomType.getCode(), srid
        );
    ...
    public void eTransformSrid(String tableName, String geomFieldName, int targetSrid) {
        ...
        try {
            // 1. 新建临时字段
            String createTempSql = StrUtil.format(
                    "ALTER TABLE {} ADD COLUMN {} geometry({},0);",
                    qualifiedTableName, tempGeomField, geomType.name().toLowerCase()
            );
            getAdvDDLOpt().dExecuteDDL(createTempSql, tableName, "新建临时空间字段[" + tempGeomField + "]");

            // 2. 拷贝数据
            String copySql = StrUtil.format(
                    "UPDATE {} SET {} =ST_SetSRID({}, {});",
                    qualifiedTableName, tempGeomField, geomFieldName, oldSrid
            );
            ...
            String transformSql = StrUtil.format(
                    ...
                    qualifiedTableName, tempGeomField, geomType.name().toLowerCase(), targetSrid,
                    tempGeomField, targetSrid
            );
            getAdvDDLOpt().dExecuteDDL(transformSql, tableName, "转换SRID为" + targetSrid);

            // 4. 重命名原字段
            String renameOldSql = StrUtil.format(
                    "ALTER TABLE {} RENAME COLUMN {} TO {};",
                    qualifiedTableName, geomFieldName, geomFieldName + "_old_" + IdUtil.simpleUUID().substring(0, 8)
            );
            getAdvDDLOpt().dExecuteDDL(renameOldSql, tableName, "重命名原空间字段");

            // 5. 重命名临时字段
            String renameTempSql = StrUtil.format(
                    "ALTER TABLE {} RENAME COLUMN {} TO {};",
                    qualifiedTableName, tempGeomField, geomFieldName
            );
            getAdvDDLOpt().dExecuteDDL(renameTempSql, tableName, "重命名临时字段为原字段名");

            // 6. 删除旧字段
            String dropOldSql = StrUtil.format(
                    "ALTER TABLE {} DROP COLUMN {};",
                    qualifiedTableName, geomFieldName + "_old_" + IdUtil.simpleUUID().substring(0, 8)
            );
    ...
    public void eCreateSpatialIndex(String tableName, String geomFieldName, String indexName) {
        ...
        String qualifiedTableName = dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableName);
        String sql = StrUtil.format("CREATE INDEX {} ON {} USING GIST ({});",
    ...
    @Override
    public String getCalculateDistanceSql(String geomFieldName, String geometry, int srid, String distanceAlias, String qualifiedTableName) {
        return StrUtil.format(
