...
public class PgAdvGeoOpt implements IAdvGeoOpt {
    ...
    public DataFieldsApo dGetColumnsByTable(String tableName) {
        ...
        try {
            Map<String, AdvEnumsTypeGeom> typeMaps = eGetGeoTypeByTable(tableName, geomFieldNameList);
    ...
    public List<String> eGetAllGeoLayerName() {
        ...
        if (ObjectUtil.isNotEmpty(dataSourceGetter.getSchemaName())) {
            schemaFilter = StrUtil.format("AND \"table_schema\" = '{}'", dataSourceGetter.getSchemaName());
        }
    ...
    public AdvEnumsTypeGeom eGetGeoTypeByTable(String tableName, String geomFieldName) {
        Map<String, AdvEnumsTypeGeom> re = eGetGeoTypeByTable(tableName, ListUtil.of(geomFieldName));
    ...
    public Map<String, AdvEnumsTypeGeom> eGetGeoTypeByTable(String tableName, List<String> geomFieldNames) {
        ...
        String formattedSql = StrUtil.format(sqlTemp, fieldsSql.toString(), tableName, whereSql.toString());
    ...
    public AdvEnumsTypeGeom eGetGeoTypeBySql(String sqlView, String geomFieldName) {
        Map<String, AdvEnumsTypeGeom> re = eGetGeoTypeBySql(sqlView, ListUtil.of(geomFieldName));
        if (MapUtil.isEmpty(re)) {
            return null;
        }
        return re.get(geomFieldName);
    ...
    public List<String> eGetGeomColumnNameListByTable(String tableName) {
        ...
        schemaName = ObjectUtil.isEmpty(schemaName) ? dataSourceGetter.getSchemaName() : schemaName;
    ...
    public String eGetGeomColumnNameBySql(String sqlView) {
        ...
        try {
            // 构建查询元数据的SQL
            String fieldQuerySql = StrUtil.format("SELECT * FROM ({}) AS {} LIMIT 0", sqlView, aliasTableName);
    ...
    public void eTransformSrid(String tableName, String geomFieldName, int targetSrid) {
        ...
        String existingGeomField = eGetGeomColumnNameByTable(tableName);
        if (StrUtil.isEmpty(existingGeomField) || !existingGeomField.equals(geomFieldName)) {
            throw new RuntimeException(StrUtil.format("表[{}]中不存在空间字段[{}]", tableName, geomFieldName));
        }
...
// 临时字段名（用于存储转换后的数据）
        ...
        try {
            ...
            pgAdvDDLOpt.dExecuteDDL(copyDataSql, tableName, "拷贝原空间字段[" + geomFieldName + "]数据到临时字段[" + tempGeomField + "]");

            // 3. 对临时字段执行坐标转换（修改类型并转换SRID）
//            String transformSql = StrUtil.format(
//                    "ALTER TABLE {} ALTER COLUMN {} TYPE geometry(Geometry, {}) USING public.ST_Transform({}, {});",
//                    qualifiedTableName,
//                    tempGeomField,
//                    targetSrid,
//                    tempGeomField,
...
//            );
            ...
            pgAdvDDLOpt.dExecuteDDL(transformSql, tableName, "转换临时字段[" + tempGeomField + "]的SRID为" + targetSrid);
            ...
            pgAdvDDLOpt.dExecuteDDL(renameOldColSql, tableName, "重命名原空间字段[" + geomFieldName + "]为 [" + renameGeomFieldName + "]");
            ...
            pgAdvDDLOpt.dExecuteDDL(renameTempColSql, tableName, "将临时字段[" + tempGeomField + "]重命名为原字段名[" + geomFieldName + "]");
    ...
    public Integer eGetSrid(String tableNameOrSqlView, String geomFieldName) {
        ...
        if (!pgAdvDDLOpt.dIsTableExists(tableNameOrSqlView)) {
            ...
            qualifiedTableName = dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableNameOrSqlView);
        }
    ...
    public Map<String, Integer> eGetSrid(String tableNameOrSqlView, List<String> geomFieldNames) {
        ...
        if (StrUtil.isEmpty(tableNameOrSqlView) || geomFieldNames == null || geomFieldNames.isEmpty()) {
            return result;
        }
    ...
    public void eCreateSpatialIndex(String tableName, String geomFieldName, String indexName) {
        if (StrUtil.isEmpty(tableName) || StrUtil.isEmpty(geomFieldName) || StrUtil.isEmpty(indexName)) {
    ...
    public List<GirAdvOneRow> eQueryIntersects(String tableName, String geomFieldName, String geometry, int srid) {
        if (StrUtil.isEmpty(tableName) || StrUtil.isEmpty(geomFieldName) || StrUtil.isEmpty(geometry)) {
        ...
        if (!pgAdvDDLOpt.dIsTableExists(tableName)) {
            ...
            qualifiedTableName = dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableName);
        }
        String sql = StrUtil.format(
    ...
    public List<GirAdvOneRow> eQueryWithinBBox(String tableName, String geomFieldName, double[] bbox, int srid) {
        if (StrUtil.isEmpty(tableName) || StrUtil.isEmpty(geomFieldName) || bbox == null || bbox.length != 4) {
            throw new IllegalArgumentException("表名、空间字段名和边界框不能为空，且边界框必须包含4个元素[minx, miny, maxx, maxy]");
        }
        if (srid <= 0) {
            throw new IllegalArgumentException("SRID必须为正数");
        }
    ...
    public List<GirAdvOneRow> eCalculateDistance(String tableName, String geomFieldName, String geometry, int srid, String distanceAlias) {
        if (StrUtil.isEmpty(tableName) || StrUtil.isEmpty(geomFieldName) || StrUtil.isEmpty(geometry) || StrUtil.isEmpty(distanceAlias)) {
    ...
    public List<GirAdvOneRow> eCalculateDistance(String tableName, String geometry, int srid, String distanceAlias) {
        String geomFieldNameByTableName = eGetGeomColumnNameByTable(tableName);
        return eCalculateDistance(tableName, geomFieldNameByTableName, geometry, srid, distanceAlias);
    }
...
//        );
        String sql = StrUtil.format(
                "\n SELECT public.ST_XMin ( extent ) AS minx,\n" +
                        ...
                        "FROM\n" +
                        "  ( SELECT public.st_setsrid ( public.ST_Extent ( {} ), {} ) AS extent FROM {} ) AS lpl666;",
                geomFieldName, i,
                qualifiedTableName
        );
     ...
     * 处理空间字段的值
     */
    public static void processGeometryField(List<GirAdvOneRow> records, AdvEnumsGeomOpt advEnumsGeomOpt, List<String> geomFieldNameList) {
     ...
     * @return
     */
    private DataFieldsApo fillGeomType(DataFieldsApo dataFieldsApo, Map<String, AdvEnumsTypeGeom> typeMaps) {
