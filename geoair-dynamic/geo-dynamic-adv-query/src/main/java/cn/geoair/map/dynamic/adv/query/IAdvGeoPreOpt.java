 ...
 * 空间处理相关操作接口
 * <p>
 * 核心功能：提供基于MyBatis的空间数据查询、空间字段类型识别、空间字段信息提取等核心能力，
 * 专注于处理包含地理几何信息（点、线、面等）的数据查询与解析。
 * </p>
 * 约定：
 * 1. 所有方法名以 e 开头（规避 get 前缀的命名冲突问题）；
 * 2. SQL 语句参数统一命名为 sqlStatement，使用MyBatis风格占位符（#{参数名}）；
 * 3. 空间字段相关参数支持单字段、多字段（List）两种传入形式，适配不同业务场景。
 *
 * @author （可补充作者信息）
 * @version 1.0
 * @since （可补充适配的最低版本/依赖）
