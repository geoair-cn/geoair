...
// String sql = StrUtil.format("TRUNCATE TABLE {} RESTART IDENTITY CASCADE",
// qualifiedTableName);
//
...
// String sql = "SELECT schema_name FROM information_schema.schemata WHERE schema_name NOT
// IN ('information_schema', 'pg_catalog', 'pg_toast') ORDER BY schema_name";
...
// .append("JOIN pg_namespace AS ns ON cl.relnamespace = ns.oid AND ns.nspname =
// c.table_schema ")
...
// .append(" WHERE ")
// .append(" conrelid = (SELECT oid FROM pg_class WHERE relname =
// '").append(notSchemaTableName).append("' ");
// if (ObjectUtil.isNotEmpty(schemaName)) {
...
//
// // 执行查询获取字段列表
// List<FieldBySchemaApo> fieldBySchemaApos =
// baseOpt.bSelectObjList(sqlBuilder.toString(), FieldBySchemaApo.class);
//
// // 处理查询结果
...
// String fieldQuerySql = StrUtil.format("SELECT * FROM ({}) AS temp_table LIMIT 0",
// sqlView);
// log.info("schema:[" + dataSourceGetter.getSchemaName() + "] " + "db:[" +
// dataSourceGetter.getDataSourceId() + "]" + " SQL的元数据查询：{}", fieldQuerySql);
...
// field.setIsNullable(metaData.isNullable(i) == ResultSetMetaData.columnNoNulls ? "NO" :
// "YES");
// // 设置字段长度信息（根据字段类型处理）
...
// String sql = StrUtil.format("ALTER TABLE {} RENAME TO {}", oldQualifiedName,
// newQualifiedName);
...
// if (!oldFieldExists) {
// throw new RuntimeException(StrUtil.format("表[{}]中原字段[{}]不存在，无法修改", tableName,
// oldColumnName));
// }
//
...
// String finalColumnName = StrUtil.isEmpty(newField.getColumnName()) ? oldColumnName :
...
// alterDef.append(StrUtil.format("({}, {})", newField.getNumericPrecision(),
...
// dExecuteDDL(sqlBuilder.toString(), tableName, "修改字段[" + oldColumnName + "→" +
// finalColumnName + "]");
// }
//
// @Override
...
// String actualSchema = ObjectUtil.isEmpty(schemaName) ? dataSourceGetter.getSchemaName()
// : schemaName;
// String sql;
//
...
//
// // 检查模式是否已存在
// String checkSql = StrUtil.format("SELECT schema_name FROM information_schema.schemata
...
// }
//
// String sql = StrUtil.format("DROP SCHEMA {} {}", schemaName, cascade ? "CASCADE" :
// "RESTRICT");
...
// String qualifiedTableName =
// dialectTableNameProcessor.tbGetTableNameWithSchema(dataSourceGetter, tableName);
// String sql = StrUtil.format("ALTER TABLE {} DROP CONSTRAINT {}", qualifiedTableName,
// constraintName);
...
// throw new RuntimeException(StrUtil.format("表[{}]中索引[{}]已存在，无法重复创建", tableName,
// indexName));
// }
//
// String qualifiedTableName =
...
// sql += StrUtil.format(" AND tco.table_schema = '{}'",
...
// tableName,
// StrUtil.isEmpty(dataSourceGetter.getSchemaName()) ? "" : StrUtil.format("AND schemaname
// = '{}'", dataSourceGetter.getSchemaName())
// );
...
// * 获取基础列名
// */
// private String getBaseColumnName(ResultSetMetaData metaData, int columnIndex) throws
// SQLException {
...
// * 设置字段长度相关信息
// */
// private void setFieldLengthInfo(ResultSetMetaData metaData, int columnIndex,
// FieldBySchemaApo field) throws SQLException {
...
// }
//
// log.error("{}失败，表名: {}, SQL: {}, 错误: {}", operation, tableName, sql, e.getMessage(),
// e);
...
// }
//
// log.error("{}失败，表名: {}, SQL: {}, 错误: {}", operation, tableName, execSql,
// e.getMessage(), e);
// throw new RuntimeException(StrUtil.format("{}失败: {}", operation, e.getMessage()), e);
// } finally {
// // 恢复自动提交模式
