...
public abstract class AbstractAdvDDLOpt implements IAdvDDLOpt {
    ...
    public DataFieldsApo dGetColumnsBySQL(String sqlView) {
        ...
        log.info("schema:[{}] db:[{}] SQL的元数据查询：{}",
                dataSourceGetter.getSchemaName(), dataSourceGetter.getDataSourceId(), fieldQuerySql);
    ...
    protected DataFieldsApo getMetadataFromSql(String sql, DataFieldsApo tableFields, SqlParamMap sqlParam) {
        ...
        try {
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                // 复用表字段元数据（如果存在）
                if (tableFields != null) {
                    Optional<FieldBySchemaApo> fieldOpt = tableFields.getDataField(fieldBySchemaApo -> fieldBySchemaApo.getOriginalColumnName()
