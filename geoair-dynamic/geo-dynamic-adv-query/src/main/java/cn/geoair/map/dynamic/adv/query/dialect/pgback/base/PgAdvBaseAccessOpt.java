...
//        String execSql = StrUtil.format("INSERT INTO {} ({}) VALUES ({})", tableName, fields, placeholders);
//
//        List<Object> params = new ArrayList<>(rowData.values());
//        Connection connection = dataSourceGetter.getConnection();
//        try {
//            log.info("schema:[{}] db:[{}] 执行单条插入，表名：{}，字段：{}", dataSourceGetter.getSchemaName(), dataSourceGetter.getDataSourceId(), tableName, fields);
...
//        String execSql = StrUtil.format("INSERT INTO {} ({}) VALUES ({}) RETURNING id", tableName, fields, placeholders);
//
//        List<Object> params = new ArrayList<>(rowData.values());
//        Connection connection = dataSourceGetter.getConnection();
//        try {
//            log.info("schema:[{}] db:[{}] 执行单条插入并返回主键，表名：{}", dataSourceGetter.getSchemaName(), dataSourceGetter.getDataSourceId(), tableName);
//            // 执行并获取主键
//            return SqlExecutor.executeForGeneratedKey(connection, execSql, params.toArray()).longValue();
//        } catch (SQLException e) {
...
//            String placeholders = headers.stream().map(key -> "?").collect(Collectors.joining(","));
//            String execSql = StrUtil.format("INSERT INTO {} ({}) VALUES ({})", tableName, fields, placeholders);
//
...
//                    dataSourceGetter.getSchemaName(), dataSourceGetter.getDataSourceId(), tableName, totalSuccess, batchSize);
//            return totalSuccess;
//        } catch (SQLException e) {
//            try {
//                connection.rollback();
...
//        Connection connection = dataSourceGetter.getConnection();
//        try {
//            log.info("schema:[{}] db:[{}] 执行插入忽略操作，表名：{}", dataSourceGetter.getSchemaName(), dataSourceGetter.getDataSourceId(), tableName);
//            return SqlExecutor.execute(connection, execSql, params.toArray());
//        } catch (SQLException e) {
...
//                    dataSourceGetter.getSchemaName(), dataSourceGetter.getDataSourceId(), tableName, updateClause);
//            return SqlExecutor.execute(connection, execSql, params.toArray());
//        } catch (SQLException e) {
...
//     * 校验表名、字段头和批量数据
//     */
//    private void validateTableNameAndData(String tableName, Set<String> headers, List<Map<String, Object>> rowsData) {
//        validateTableName(tableName);
