 ...
 * 数据更新相关的基础操作接口
 * <p>
 * 覆盖单条更新、批量更新、自定义SQL更新、条件更新、乐观锁更新、UPSERT等全场景， 适配PostgreSQL/MySQL等主流数据库，保持语义化命名和易用性
 *
 * @author 张逢吉
 */
public interface IAdvBaseUpdateOpt {
	 ...
	 * @param sqlStatement 自定义更新SQL语句（支持MyBatis标签） <br>
	 ...
	 * @param sqlStatement 自定义更新SQL语句（含参数占位符） <br>
	 * 示例：UPDATE user SET name = #{name} WHERE id = #{id} sqlParam
	 ...
	 * @param id 主键值
	 * @param rowData 待更新的字段-值映射（key=字段名，value=新值） <br>
	 * 示例：{ "name": "张三", "age": 25 }
	 ...
	 * @param whereMap 更新条件（key=字段名，value=条件值，拼接为AND条件） <br>
	 ...
	 * @param tableName 目标表名
	 * @param idKey 主键字段名
	 * @param rowsData 批量更新数据（每个Map需包含主键字段和待更新字段） <br>
	 ...
	 * @return Integer 受影响的总行数
	 */
	Integer bUpdateBatchWithBatchSize(String tableName, String idKey, List<Map<String, Object>> rowsData,
			int batchSize);
	 ...
	 */
	Integer bUpdateWithOptimisticLock(String tableName, String idKey, Object id, Map<String, Object> rowData,
