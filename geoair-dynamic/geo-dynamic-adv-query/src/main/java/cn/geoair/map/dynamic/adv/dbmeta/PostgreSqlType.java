...
public enum PostgreSqlType implements TypeMetadata {
	...
	VARCHAR("varchar", "VARCHAR", DefaultJavaType.JAVA_STRING, CATEGORY.CHAR), CHAR("char", "CHAR",
			DefaultJavaType.JAVA_CHAR, CATEGORY.CHAR), TEXT("text", "TEXT", DefaultJavaType.JAVA_TEXT,
					CATEGORY.TEXT), UUID("uuid", "UUID", DefaultJavaType.JAVA_UUID, CATEGORY.CHAR), JSON("json", "JSON",
							DefaultJavaType.JAVA_JSON,
							CATEGORY.TEXT), JSONB("jsonb", "JSONB", DefaultJavaType.JAVA_JSONB, CATEGORY.TEXT), CITEXT(
									"citext", "CITEXT", DefaultJavaType.JAVA_CITEXT, CATEGORY.CHAR),
	...
	NUMERIC("numeric", "NUMERIC", DefaultJavaType.JAVA_NUMERIC, CATEGORY.FLOAT), DECIMAL("decimal", "DECIMAL",
			DefaultJavaType.JAVA_DECIMAL, CATEGORY.FLOAT),

	// 日期时间类（关联CATEGORY.DATE/TIME/DATETIME/TIMESTAMP）
	DATE("date", "DATE", DefaultJavaType.JAVA_LOCAL_DATE, CATEGORY.DATE), TIME("time", "TIME",
			DefaultJavaType.JAVA_LOCAL_TIME, CATEGORY.TIME), TIMETZ("timetz", "TIME WITH TIME ZONE",
					DefaultJavaType.JAVA_OFFSET_TIME, CATEGORY.TIME), TIMESTAMP("timestamp", "TIMESTAMP",
							DefaultJavaType.JAVA_LOCAL_DATE_TIME, CATEGORY.TIMESTAMP), TIMESTAMPTZ("timestamptz",
									"TIMESTAMP WITH TIME ZONE", DefaultJavaType.JAVA_OFFSET_DATE_TIME,
									CATEGORY.TIMESTAMP), INTERVAL("interval", "INTERVAL", DefaultJavaType.JAVA_INTERVAL,
											CATEGORY.INTERVAL),

	// 二进制/特殊类（关联CATEGORY.BYTES/GEOMETRY/OTHER）
	BYTEA("bytea", "BYTEA", DefaultJavaType.JAVA_BYTES, CATEGORY.BYTES), BLOB("blob", "BLOB", DefaultJavaType.JAVA_BLOB,
			CATEGORY.BLOB), GEOMETRY("geometry", "GEOMETRY", DefaultJavaType.JAVA_GEOMETRY,
					CATEGORY.GEOMETRY), MONEY("money", "MONEY", DefaultJavaType.JAVA_MONEY, CATEGORY.FLOAT); // 货币类型归为浮点类
