...
public class PgAdvDDLOpt extends AbstractAdvDDLOpt {
    ...
    public DataFieldsApo dGetColumnsByTable(String tableName) {
        ...
        sqlBuilder.append("    JOIN pg_attribute A ON C.OID = A.attrelid ")
                ...
                .append("    WHERE conrelid = (SELECT oid FROM pg_class WHERE relname = '").append(notSchemaTableName).append("' ");
        if (ObjectUtil.isNotEmpty(schemaName)) {
            sqlBuilder.append("    AND relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = '").append(schemaName).append("') ");
        }
        sqlBuilder.append(") AND contype = 'p' ")
    ...
    public List<String> dGetPrimaryKeys(String tableName) {
        ...
        String sql = StrUtil.format("SELECT kcu.column_name " +
    ...
    @Override
    protected String buildCreateIndexSql(String qualifiedTableName, String indexName, String columns, boolean isUnique) {
    ...
    protected boolean checkSchemaExists(String schemaName) {
        String sql = StrUtil.format("SELECT schema_name FROM information_schema.schemata WHERE schema_name = '{}'", schemaName);
        return ObjectUtil.isNotEmpty(baseOpt.bSelectList(sql));
    }
    ...
    @Override
    protected void setFieldLengthInfo(ResultSetMetaData metaData, int columnIndex, FieldBySchemaApo field) throws SQLException {
