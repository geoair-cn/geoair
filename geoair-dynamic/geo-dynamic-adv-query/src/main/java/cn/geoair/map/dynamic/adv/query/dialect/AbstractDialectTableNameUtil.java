...
/**
 * 数据库方言表名处理器抽象父类
 * 封装所有数据库通用的表名/字段名处理逻辑，差异化逻辑由子类实现
 */
public abstract class AbstractDialectTableNameUtil implements DialectTableNameProcessor {
    ...
    public String tbGetTableNameNotSchema(String fullTableName) {
        ...
        Pattern pattern = Pattern.compile(
                "(?:[\"'`][^\"'`]+[\"'`]|[a-zA-Z0-9_\\u4e00-\\u9fa5-]+)\\." +  // Schema/库名部分（兼容中文、横线）
    ...
    public String tbQuoteTableName(String tableName) {
        // 通用加引号逻辑：避免重复加引号
        if (StrUtil.isEmpty(tableName) || (tableName.startsWith(getQuoteChar()) && tableName.endsWith(getQuoteChar()))) {
            return tableName;
        }
    ...
    public String tbGetSchemaNameForSql(IDataSourceGetter dataSourceGetter) {
        return ObjectUtil.isEmpty(dataSourceGetter.getSchemaName()) ? getDefaultSchemaName() : dataSourceGetter.getSchemaName();
    }
}
