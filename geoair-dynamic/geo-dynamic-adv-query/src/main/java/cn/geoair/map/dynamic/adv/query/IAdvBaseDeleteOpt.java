 ...
 * 数据删除相关的基础操作接口
 * <p>
 * 覆盖单条删除、批量删除、自定义SQL删除、条件删除、逻辑删除、防误删保护等全场景，
...
public interface IAdvBaseDeleteOpt {
     ...
     * 执行自定义删除SQL语句（无参数）
     * <p>
     * 支持任意复杂的DELETE SQL（含多表关联删除、子查询删除等）
     *
     * @param sqlStatement 自定义删除SQL语句（支持MyBatis标签）
     *                     <br>示例：DELETE FROM user WHERE create_time < '2024-01-01' AND status = 0
     ...
     * 根据主键集合批量删除数据
     * <p>
     * 底层优化为IN查询（DELETE FROM ... WHERE id IN (?, ?, ...)），
     * 适用于批量删除少量数据（建议不超过1000条）
     *
     * @param tableName 目标表名
     * @param idKey     主键字段名
     * @param ids       主键值集合
     ...
     */
    Integer bDeleteBatchWithBatchSize(String tableName, String idKey, Set<Object> ids, int batchSize);
     ...
     * 按条件删除数据（Map格式条件）
     * <p>
     * 条件拼接为AND关系，支持多字段组合条件，**禁止传入空条件（避免全表删除）**
     *
     * @param tableName 目标表名
     * @param whereMap  删除条件（key=字段名，value=条件值）
     *                  <br>示例：{ "dept_id": 5, "status": 0 }
     * @return Integer 受影响的行数
     */
    Integer bDeleteByCondition(String tableName, Map<String, Object> whereMap);

    /**
     * 按条件批量删除（分批次执行）
     * <p>
     * 适用于删除大量数据，避免单次删除锁表时间过长
     *
     * @param tableName 目标表名
     * @param whereMap  删除条件
     ...
     * @return Integer 受影响的总行数
     */
    Integer bDeleteBatchByCondition(String tableName, Map<String, Object> whereMap, int batchSize);
     ...
     * 逻辑删除（更新删除标记，非物理删除）
     * <p>
     * 适用于需要保留数据痕迹的场景，更新删除标记字段（如：deleted = 1）
     *
     * @param tableName  目标表名
     * @param idKey      主键字段名
     * @param id         主键值
     ...
     * @return Integer 受影响的行数
     */
    Integer bLogicDelete(String tableName, String idKey, Object id, String deleteKey, Object deleteValue);
     ...
     * @return Integer 受影响的总行数
     */
    Integer bLogicDeleteBatch(String tableName, String idKey, Set<Object> ids, String deleteKey, Object deleteValue);

    /**
     * 安全删除（带防误删校验）
     * <p>
     * 当删除行数超过阈值时，自动终止操作，避免误删大量数据
     *
     * @param tableName 目标表名
     * @param whereMap  删除条件
