 ...
 * 描述: 将JDBC URL拆分为各个组件，支持解析参数为Map（适配PostgreSQL/MySQL等）
 * 支持URL格式：jdbc:postgresql://host:port/db?param1=value1&param2=value2 或 jdbc:derby://host:port/db;param1=value1;param2=value2
 */
public class AdvJdbcUrlUtil {
    ...
    public AdvJdbcUrlUtil(String jdbcUrl) {
        ...
        if (driverEndPos == -1) {
            throw new IllegalArgumentException("无效的JDBC URL：无法提取驱动名，格式应为jdbc:driver://host:port/db?params");
        }
    ...
    /**
     * 新增方法：获取不带参数的核心JDBC URL字符串
     * 示例：jdbc:postgresql://10.11.14.182:5432/bdh
     *
