 ...
 * 查询的基础操作接口
 * <p>
 * 设计初衷模仿MyBatis-Plus的BaseMapper，提供通用的数据库查询操作封装， 支持传入带MyBatis标签的SQL语句，适配动态查询场景下的各类基础查询需求
 */
public interface IAdvBaseSelectOpt {
	 ...
	 * 示例：row -> System.out.println("日志ID：" + row.get("id") + " 内容：" +
	 * row.get("log_content"))
	 */
	 ...
	 * 示例：emp -> System.out.println("员工姓名：" + emp.getName() + " 薪资：" + emp.getSalary())
	 * @param <E> 目标对象的泛型类型
	 */
	 ...
	 * 示例2（带MyBatis标签）： SELECT id, name, age FROM user <where> <if test="name != null">AND
	 * name LIKE CONCAT('%', #{name}, '%')</if> <if test="age != null">AND age =
	 * #{age}</if> </where>
	 ...
	 * 示例2（带MyBatis标签）： SELECT id, name, create_time FROM dept <where>
	 ...
	 * @param sqlStatement 待执行的SQL语句（支持MyBatis标签） <br>
	 * 示例（大数据量查询）： SELECT id, log_content, create_time FROM system_log WHERE create_time
	 ...
	 * <p>
	 * 外层List代表多行数据，内层List代表单行数据的所有列值（按查询列的顺序排列）， 相当于将GirAdvOneRow中的value按顺序提取到List中
	 * @param sqlStatement 待执行的SQL语句（支持MyBatis标签） <br>
	 ...
	 * 示例：SELECT COUNT(*) FROM user <where> <if test="name != null">AND name LIKE
	 * CONCAT('%', #{name}, '%')</if> </where>
	 * @param sqlParam SQL语句中的参数映射（key为参数名，value为参数值） <br>
	 * 示例：{ "name": "张三" }
