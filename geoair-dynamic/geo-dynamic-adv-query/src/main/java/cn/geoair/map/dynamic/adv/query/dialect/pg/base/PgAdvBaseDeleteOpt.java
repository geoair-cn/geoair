...
public class PgAdvBaseDeleteOpt implements IAdvBaseDeleteOpt {
    ...
    public Integer bDeleteBatchByPrimaryKey(String tableName, String idKey, Set<Object> ids) {
        ...
        try {
            ...
            for (List<Object> idBatch : idBatches) {
                ...
                String execSql = StrUtil.format("DELETE FROM {} WHERE {} IN ({})", tableName, idKey, placeholders);
    ...
    public Integer bSafeDeleteByCondition(String tableName, Map<String, Object> whereMap, int maxDelete) {
        ...
        String countWhereClause = whereMap.keySet().stream()
                .map(field -> StrUtil.format("{} = ?", field))
                .collect(Collectors.joining(" AND "));
        String countSql = StrUtil.format("SELECT COUNT(1) FROM {} WHERE {}", tableName, countWhereClause);
