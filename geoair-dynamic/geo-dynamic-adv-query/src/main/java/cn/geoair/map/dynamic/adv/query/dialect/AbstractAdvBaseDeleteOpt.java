...
public abstract class AbstractAdvBaseDeleteOpt implements IAdvBaseDeleteOpt {
	...
	public Integer bDeleteBySql(String sqlStatement, SqlParamMap sqlParam) {
		...
		try {
			log.info("schema:[{}] db:[{}] 执行自定义删除SQL：{}，参数：{}", getSchemaName(), getDataSourceId(), execSql, sqlParam);
	...
	public Integer bDeleteByPrimaryKey(String tableName, String idKey, Object id) {
		...
		Connection connection = dataSourceGetter.getConnection();
		try {
			log.info("schema:[{}] db:[{}] 按主键删除，表名：{}，主键：{}={}", getSchemaName(), getDataSourceId(), tableName, idKey,
					id);
	...
	public Integer bDeleteBatchByPrimaryKey(String tableName, String idKey, Set<Object> ids) {
		...
		try {
			...
			for (List<Object> idBatch : idBatches) {
				...
				log.info("schema:[{}] db:[{}] 批量主键删除，表名：{}，主键数量：{}", getSchemaName(), getDataSourceId(), tableName,
						idBatch.size());
	...
	public Integer bDeleteBatchWithBatchSize(String tableName, String idKey, Set<Object> ids, int batchSize) {
		...
		log.info("schema:[{}] db:[{}] 分批次主键删除完成，表名：{}，总删除行数：{}，批次大小：{}", getSchemaName(), getDataSourceId(), tableName,
	...
	public Integer bDeleteByCondition(String tableName, Map<String, Object> whereMap) {
		...
		Connection connection = dataSourceGetter.getConnection();
		try {
			log.info("schema:[{}] db:[{}] 条件删除，表名：{}，条件：{}", getSchemaName(), getDataSourceId(), tableName, whereMap);
	...
	public Integer bDeleteBatchByCondition(String tableName, Map<String, Object> whereMap, int batchSize) {
		...
		try {
			...
			connection.commit();
			log.info("schema:[{}] db:[{}] 分批次条件删除完成，表名：{}，总删除行数：{}，批次大小：{}", getSchemaName(), getDataSourceId(),
	...
	public Integer bLogicDelete(String tableName, String idKey, Object id, String deleteKey, Object deleteValue) {
		...
		Connection connection = dataSourceGetter.getConnection();
		try {
			log.info("schema:[{}] db:[{}] 逻辑删除，表名：{}，主键：{}={}，删除标记：{}={}", getSchemaName(), getDataSourceId(),
	...
	public Integer bLogicDeleteBatch(String tableName, String idKey, Set<Object> ids, String deleteKey,
		...
		try {
			...
			connection.commit();
			log.info("schema:[{}] db:[{}] 批量逻辑删除完成，表名：{}，删除行数：{}", getSchemaName(), getDataSourceId(), tableName,
					totalSuccess);
			return totalSuccess;
		}
		catch (SQLException e) {
