...
public abstract class AbstractAdvBaseUpdateOpt implements IAdvBaseUpdateOpt {
    ...
    public Integer bUpdateByPrimaryKey(String tableName, String idKey, Object id, Map<String, Object> rowData) {
        ...
        try {
            log.info("schema:[{}] db:[{}] 按主键更新，表名：{}，主键：{}={}，更新字段：{}",
                    getSchemaName(), getDataSourceId(), tableName, idKey, id, rowData.keySet());
    ...
    public Integer bUpdateByCondition(String tableName, Map<String, Object> rowData, Map<String, Object> whereMap) {
        ...
        try {
            log.info("schema:[{}] db:[{}] 条件更新，表名：{}，更新字段：{}，条件：{}",
                    getSchemaName(), getDataSourceId(), tableName, rowData.keySet(), whereMap);
    ...
    public Integer bUpdateWithOptimisticLock(String tableName, String idKey, Object id, Map<String, Object> rowData, String versionKey, Integer version) {
        ...
        try {
            log.info("schema:[{}] db:[{}] 乐观锁更新，表名：{}，主键：{}={}，版本号：{}={}",
                    getSchemaName(), getDataSourceId(), tableName, idKey, id, versionKey, version);
    ...
    public Integer bUpdateOrInsert(String tableName, Map<String, Object> rowData, Set<String> conflictKeys) {
        ...
        try {
            log.info("schema:[{}] db:[{}] 更新或插入，表名：{}，冲突字段：{}，更新字段：{}",
                    getSchemaName(), getDataSourceId(), tableName, conflictFields, updateClause);
     ...
     * 构建乐观锁更新SQL
     */
    protected String buildUpdateWithOptimisticLockSql(String tableName, String setClause, String idKey, String versionKey) {
        //  ：乐观锁更新语法（WHERE 主键 = ? AND 版本号 = ?）
        return StrUtil.format("UPDATE {} SET {} WHERE {} = ? AND {} = ?",
     ...
     * 构建更新或插入（UPSERT）SQL
     */
    protected abstract String buildUpdateOrInsertSql(String tableName, String fields, String placeholders, String conflictFields, String updateClause);
}
