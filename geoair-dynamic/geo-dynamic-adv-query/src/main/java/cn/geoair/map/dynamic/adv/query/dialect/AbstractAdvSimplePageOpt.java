...
public abstract class AbstractAdvSimplePageOpt implements IAdvSimplePageOpt {
	 ...
	 * 执行分页查询，返回结果列表
	 */
	protected abstract List<GirAdvOneRow> executePageSql(String pageSql, AdvEnumsGeomOpt advEnumsGeomOpt,
	...
	@Override
	public PageApo<GirAdvOneRow> pPage(String noPageSql, int pageNum, int pageSize, boolean pageNumStartZero,
			AdvEnumsGeomOpt advEnumsGeomOpt, List<OrderApo> orders) {
	...
	public PageApo<GirAdvOneRow> pPage(String noPageSql, int pageNum, int pageSize, boolean pageNumStartZero,
		...
		String refactorNoPageSql = StrUtil.format("SELECT {} FROM ({}) AS {}", quotedFields,
				dialectTableNameProcessor.tbRemoveSqlSpaces(noPageSql), tableAlias);
	...
	protected void validateFullPageParams(String noPageSql, int pageNum, int pageSize, boolean pageNumStartZero,
			List<OrderApo> orders) {
		if (StrUtil.isEmpty(noPageSql)) {
		...
		if ((pageNumStartZero && pageNum < 0) || (!pageNumStartZero && pageNum < 1)) {
			throw new IllegalArgumentException(StrUtil.format("页码不合法：页码{}从{}开始，当前值：{}", pageNumStartZero ? "允许" : "不允许",
					pageNumStartZero ? 0 : 1, pageNum));
		}
	...
	protected PageApo<GirAdvOneRow> createPageApo(long total, int pageNum, int pageSize, boolean pageNumStartZero,
			int lastPageNum, long startRow, List<GirAdvOneRow> records) {
		PageApo<GirAdvOneRow> pageApo = new PageApo<>();
		pageApo.setTotal(total).setPageNum(pageNum).setPageSize(pageSize).setPageNumStartZero(pageNumStartZero)
				.setLastPageNum(lastPageNum).setStartRow(startRow).setRecords(records);
		return pageApo;
	}
	...
	public String pBuildSqlWithOrder(String baseSql, List<OrderApo> orders, String tableAlias) {
		...
		String orderByClause = orders.stream().map(order -> {
			String orderTarget;
			if (order.isFunction()) {
				...
				if (StrUtil.isEmpty(tableAlias)) {
					...
					orderTarget = StrUtil.format("{}.{}", tableAlias, quoteFieldName(order.getFieldName()));
				}
			}
			return StrUtil.format("{} {}", orderTarget, order.getAdvEnumsOrder().getValue());
		}).collect(Collectors.joining(", "));

		return StrUtil.format("{} ORDER BY {}", dialectTableNameProcessor.tbRemoveSqlSpaces(baseSql), orderByClause);
	}
