...
/**
 * @author ：zhangjun
 * @date ：Created in 2025/1/8 15:09
 * @description： 空间类型的通用get器并适配hutools的 OptNullBasicTypeFromObjectGetter
 */
public interface OptNullGeomAndBasicTypeFromObjectGetter extends OptNullBasicTypeFromObjectGetter<String> {
    ...
    default Geometry getGeometry(String key) {
        ...
        if (value instanceof Map) { // 判断是否为json对象
            ...
            /**
             * 若JDBCURL上面显示指定 currentSchema=onemap_tile_builder 。
             * 然而空间类型的元数据（如类型定义）仅存在于 public 中，
             * 驱动在  onemap_tile_builder 下找不到对应的类型定义，
             * 就无法将其识别为 PgGeom，
             * 只能降级为通用的 PgObject 类型。
             */
