...
/**
 * @author ：张逢吉
 * @date ：Created in 2025/12/19 11:29
 * @description： TODO
 */
@Slf4j
public abstract class AbstractITileExecutor implements ITileExecutor {
     ...
     */
    @Getter
    protected boolean successIs = false;
    ...
    public TileExecParams getTileExecParams(int zoom, int x, int y) {
        tileExecParams = AdvMvtTileUtils.getTileExecParamsNotHasSql(zoom, x, y, gridSrid, sourceDataSrid);
    ...
    public TileRequest getTileData(int zoom, int x, int y) {
        ...
        try {
            ...
            if (zoom >= requestParams.getMinZoom() || ignoreMinZoom) {
                VectorTileBuilderConsumer vectorTileBuilder = GirRealMvtHelper.getInstance()
                        .getVectorTileBuilderConsumer(this.tileExecParams.getGridExtent(), layerName, gridSrid, getTileGlobalConfig());
        ...
        tileRequest.setPbfInfo(new PbfInfo().setData(pbfData).setZoom(zoom).setGridSrid(tileExecParams.getGridSrid()));
