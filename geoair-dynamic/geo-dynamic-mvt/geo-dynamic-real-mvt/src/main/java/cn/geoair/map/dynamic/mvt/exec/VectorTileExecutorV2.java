...
public class VectorTileExecutorV2 extends AbstractITileExecutor {
	...
	public TileGlobalConfig getTileGlobalConfig() {
		...
		tileGlobalConfig.setTileRequestParams(requestParams.copy()).setTileExecConfig(this.tileExecutorConfig.copy())
				.setTileExecParams(this.tileExecParams.copy()).setLayerName(layerName).setVersion(2)
	...
	final static String geomBox = "geo_box";

	/**
	 * 空间字段的别名
	 */
	final static String GEOM_FIELD_ALIAS_IN_SQL = "geo_root";

	final static String GEOM_FIELD_ALIAS_IN_TRAN = "geom";

	public static VectorTileExecutorV2 getInstance(TileRequestParams requestParams, String layerName) {
	...
	public VectorTileExecutorV2(TileRequestParams requestParams, String layerName) {
		...
		if (ObjectUtil.isNotEmpty(keepField)) {
			...
			keepFieldListTemp = requestParams.getKeepFieldList() == null ? new ArrayList<>()
	...
	protected String getBufferBboxSqlFunction(TileExecParams tileExecParams) {
		...
		return StrUtil.format("public.ST_MakeEnvelope({}, {}, {}, {}, {})", xmin, ymin, xmax, ymax, sourceDataSrid);
	}
	...
	public String getExecSql(TileExecParams tileExecParams) {
		...
		if (isSqlView) {
			...
			String tbGetTableNameWithSchema = iAdvExecutor.tbGetTableNameWithSchema(tbNameOrSql, schema);
			finalTbName = StrUtil.format("( select * from  {})   as {} ", tbGetTableNameWithSchema, tableAlias);
		}
		// 校验必要参数
		// 2. 校验必要参数（从Params读取）
		Envelope dataExtent = tileExecParams.getDataExtent();
		if (ObjectUtils.anyNull(dataExtent, requestParams.getSrid(), requestParams.getGeomFieldName(),
		...
		// 构建WITH查询块 - 定义查询区域
		withSQL.append("\n").append("WITH   ").append(withQueryAlias).append(" AS ( select  ").append(polygonFunction)
				.append("   AS  ").append(geomBox).append(") ");
		...
		if (ObjectUtil.isEmpty(keepFieldList) && !requestParams.isKeepFieldAll()) {
			rootSql.append("encode(public.ST_AsBinary(public.ST_Force2D(").append(geomField)
					.append(")), 'base64') as  ").append(GEOM_FIELD_ALIAS_IN_SQL);
		}
		else if (requestParams.isKeepFieldAll()) {
			rootSql.append(tableAlias).append(".*").append(", encode(")
					.append(StrUtil.format("public.ST_AsBinary({})", geomField)).append(", 'base64') as  ")
			...
			rootSql.append(String.join(", ", aliasFields))
					// 拼接geom字段（处理交集逻辑）
					.append(", encode(").append(StrUtil.format("public.ST_AsBinary({})", geomField))
					.append(", 'base64') as  ").append(GEOM_FIELD_ALIAS_IN_SQL);
		}

		// 构建FROM和WHERE部分（主表添加别名T）
		rootSql.append(" FROM ").append(finalTbName).append(" ").append(",  ").append(withQueryAlias).append(" ")
				.append(withQueryAlias).append(" WHERE public.ST_Intersects(").append(geomField).append(",")
				.append(withQueryAlias).append(".").append(geomBox).append(" )").append(" and ").append(geomField)
				.append(" is not null  ");
	...
	protected void doExecSQL(VectorTileBuilderConsumer vectorTileBuilder, String excuteSQL) {
		TileExecutorConfig.LowLevelOptStrategy lowLevelOptStrategy = tileExecutorConfig.getLowLevelOptStrategy();
		int pagingStartLevel = tileExecutorConfig.getPagingStartLevel();
		int limitStartLevel = tileExecutorConfig.getLimitStartLevel();
		TileExecutorConfig.DensityOptStrategy densityOptStrategy = tileExecutorConfig.getDensityOptStrategy();
		final int DEFAULT_DENSITY_COUNT = 3000;
		List<GirAdvOneRow> girAdvOneRows = new ArrayList<>();
		boolean needTrans = true;
		try {
			if (lowLevelOptStrategy.equals(TileExecutorConfig.LowLevelOptStrategy.PAGING) && zoom <= pagingStartLevel) {
				// 策略1：分页查询
				...
				if (totalCount > pagingThreshold) {
					girAdvOneRows = parallelPageQueryReturnList(excuteSQL, totalCount, GEOM_FIELD_ALIAS_IN_SQL,
		...
		processGirAdvOneRowList(girAdvOneRows, needTrans, densityOptStrategy, vectorTileBuilder, DEFAULT_DENSITY_COUNT,
				gridSrid);
	}
	 ...
	 */
	private void processGirAdvOneRowList(List<GirAdvOneRow> girAdvOneRows, boolean needTransform,
			TileExecutorConfig.DensityOptStrategy densityOptStrategy, Consumer<GirAdvOneRow> vectorTileBuilder,
			int limit, int gridSrid) {
		if (CollectionUtil.isEmpty(girAdvOneRows)) {
			return;
		}
		List<GirAdvOneRow> transList = girAdvOneRows;
		if (needTransform) {
			transList = girAdvOneRows.stream().peek(this::featuresTransform).collect(Collectors.toList());
		}
		if (TileExecutorConfig.DensityOptStrategy.DENSITY_MERGING.equals(densityOptStrategy)) {
			List<GirAdvOneRow> girAdvOneRowsCoalesce = AdvMvtDensityUtils.doCoalesceBySpatialDensity(transList,
					Math.toIntExact(limit), GEOM_FIELD_ALIAS_IN_TRAN, GEOM_FIELD_ALIAS_IN_TRAN, gridSrid);
		...
		try {
			...
			if (geometry != null) {
				try {
					...
					Geometry gridExtentBufferBoxGeom = this.tileExecParams.getGridExtentBufferBoxGeom();
	 ...
	 */
	public List<GirAdvOneRow> parallelPageQueryReturnList(String excuteSQL, Long totalCount, String orderFileIdName,
		...
		PageActuator<GirAdvOneRow> pageActuatorOpt = GirAdvTools
				.getPageActuatorOpt(new PageConditionDef<GirAdvOneRow>() {
					...
					@Override
					public boolean handlePageException(Integer pageNo, Integer pageSize, Exception e) {
						log.warn("分页查询异常：页码[{}]，页大小[{}]，异常信息：{}", pageNo, pageSize, e.getMessage());
						return true;
					}

					@Override
					public List<GirAdvOneRow> getPageRecords(Integer pageNo, Integer pageSize) {
						String orderSql = iAdvExecutor.pBuildSqlWithOrder(excuteSQL,
								ListUtil.of(OrderApo.create(orderFileIdName, AdvEnumsOrder.升序)));
						String pageSQL = iAdvExecutor.pBuildPageSql(orderSql, pageSize, pageNo, false);
						List<GirAdvOneRow> girAdvOneRows = iAdvExecutor.bSelectList(pageSQL);
						return girAdvOneRows.stream().parallel().peek(featuresTransform).collect(Collectors.toList());
					}
				});
