...
public class VectorTileExecutorV2 extends AbstractITileExecutor {
    ...
    public TileGlobalConfig getTileGlobalConfig() {
        ...
        tileGlobalConfig
                ...
                .setLayerName(layerName).setVersion(2)
    ...
    public VectorTileExecutorV2(TileRequestParams requestParams, String layerName) {
        ...
        if (ObjectUtil.isNotEmpty(keepField)) {
            ...
            keepFieldListTemp = requestParams.getKeepFieldList() == null ? new ArrayList<>() : requestParams.getKeepFieldList();
        }
    ...
    public String getExecSql(TileExecParams tileExecParams) {
        ...
        if (ObjectUtils.anyNull(dataExtent, requestParams.getSrid(), requestParams.getGeomFieldName(), requestParams.getDsId())) {
...
// 构建WITH查询块 - 定义查询区域
        withSQL.append("\n")
                .append("WITH   ").append(withQueryAlias).append(" AS ( select  ").append(polygonFunction).append("   AS  ").append(geomBox).append(") ");
...
// 构建FROM和WHERE部分（主表添加别名T）
        rootSql.append(" FROM ").append(finalTbName).append(" ")
                .append(",  ").append(withQueryAlias).append(" ").append(withQueryAlias)
                .append(" WHERE public.ST_Intersects(").append(geomField).append(",").append(withQueryAlias).append(".").append(geomBox).append(" )").append(" and ").append(geomField).append(" is not null  ");
    ...
    protected void doExecSQL(VectorTileBuilderConsumer vectorTileBuilder, String excuteSQL) {
        ...
        try {
            if (lowLevelOptStrategy.equals(TileExecutorConfig.LowLevelOptStrategy.PAGING) && zoom <= pagingStartLevel) {
                ...
                if (totalCount > pagingThreshold) {
                    girAdvOneRows = parallelPageQueryReturnList(excuteSQL, totalCount, GEOM_FIELD_ALIAS_IN_SQL, this::featuresTransform);
                    needTrans = false;
                } else {
                    girAdvOneRows = iAdvExecutor.bSelectList(excuteSQL);
                }
            } else if (lowLevelOptStrategy.equals(TileExecutorConfig.LowLevelOptStrategy.LIMIT) && zoom <= limitStartLevel) {
        ...
        processGirAdvOneRowList(girAdvOneRows, needTrans, densityOptStrategy, vectorTileBuilder, DEFAULT_DENSITY_COUNT, gridSrid);
    }
    ...
    private void processGirAdvOneRowList(
        ...
        if (needTransform) {
            transList = girAdvOneRows.stream()
                    .peek(this::featuresTransform)
                    .collect(Collectors.toList());
        }
        if (TileExecutorConfig.DensityOptStrategy.DENSITY_MERGING.equals(densityOptStrategy)) {
            List<GirAdvOneRow> girAdvOneRowsCoalesce = AdvMvtDensityUtils.doCoalesceBySpatialDensity(
                    transList,
    ...
    public List<GirAdvOneRow> parallelPageQueryReturnList(String excuteSQL, Long totalCount, String orderFileIdName,
        ...
        PageActuator<GirAdvOneRow> pageActuatorOpt = GirAdvTools.getPageActuatorOpt(new PageConditionDef<GirAdvOneRow>() {
            ...
            public List<GirAdvOneRow> getPageRecords(Integer pageNo, Integer pageSize) {
                String orderSql = iAdvExecutor.pBuildSqlWithOrder(
                        excuteSQL,
                        ListUtil.of(OrderApo.create(orderFileIdName, AdvEnumsOrder.升序))
                );
                String pageSQL = iAdvExecutor.pBuildPageSql(
                        orderSql,
                        pageSize,
                        pageNo,
                        false
