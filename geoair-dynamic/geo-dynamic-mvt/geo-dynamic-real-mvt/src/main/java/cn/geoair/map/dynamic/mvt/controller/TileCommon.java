...
/**
 * @author ：张逢吉
 * @date ：Created in 2025/10/17 13:42
 * @description： TODO
 */
@Slf4j
public class TileCommon {
    ...
    public void doMvt(
            ...
            HttpServletRequest request, int version) throws Exception {
        ...
        if (successIs) {
            ...
            if (Objects.isNull(data) || data.length == 0) {
                log.info("未读取瓦片：layerName: {},schemaName: {}, z: {}, x: {}, y: {}", layerName, schemaName, zoom, col, row);
        ...
        try (ServletOutputStream out = response.getOutputStream()) {
            ...
            if (!response.isCommitted()) {
                response.reset();
                response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, String.format("查询失败！%s", e.getMessage()));
            }
        }
