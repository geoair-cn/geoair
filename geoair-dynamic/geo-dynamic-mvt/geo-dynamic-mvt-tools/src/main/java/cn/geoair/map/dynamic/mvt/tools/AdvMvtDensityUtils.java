...
/**
 * @author ：张逢吉
 * @date ：Created in 2025/12/29 13:56
 * @description： 处理瓦片丢弃的相关处理策略，与spark无关
 */
@Slf4j
public class AdvMvtDensityUtils {
     ...
     * @return 合并后的要素列表
     */
    public static List<GirAdvOneRow> doCoalesceBySpatialDensity(List<GirAdvOneRow> featureList, int limit, String geomFieldName, String idFieldName, int gridSrid) {
        // 1. 前置判断：若要素数未超限，直接返回
...
//            mergedResult = doFilterBySpatialDensity(mergedResult, limit, geomFieldName, idFieldName, gridSrid);
//        }
     ...
     * @return
     */
    public static List<GirAdvOneRow> coalescePointFeatures(List<GirAdvOneRow> featureList, int coalescePixel, double coalesceGeo, int z, PbfTileParameter parameter) {
...
//                // 存储聚合后的第一个点
     ...
     * @return 合并后的要素列表
     */
    public static List<GirAdvOneRow> doFilterBySpatialDensity(List<GirAdvOneRow> featureList, int limit, String geomFieldName, String idFieldName, int gridSrid) {
...
//        for (int i = 0; i < Math.min(limit, featureWithGridList.size()); i++) {
    ...
    public static PbfInfo doValidateAndOptimizeTileSize(
            ...
            Envelope envelope,
            int zoom,
            PbfTileParameter parameter) throws Exception {
        ...
        if (copyParameter.isDropDensestAsNeeded() && copyParameter.getFeatureLimit() != null) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                // 重新聚合要素并生成瓦片
                List<GirAdvOneRow> filteredList = doFilterBySpatialDensity(optimizedFeatureList, newLimit, copyParameter.getGeomFieldName(), copyParameter.getIdFieldName(), parameter.getOutGridSrid());
...
//        }
    ...
    /**
     * 入参替换为 PbfTileParameter，解耦 TileSliceParameter
     * 主要可以用于后期实时矢量瓦片的优化
     *
     * @param rs       要素列表
     * @param envelope 瓦片范围
    ...
    public static PbfInfo doGetPbfByteByGeotools(
            List<GirAdvOneRow> rs,
            Envelope envelope,
            int zoom,
            PbfTileParameter pbfParam) throws Exception {
        int simplifyLevel = Optional.ofNullable(pbfParam.getSimplificationLevel())
                .orElse(DEFAULT_SIMPLIFICATION_LEVEL);
        int coalescePixel = Optional.ofNullable(pbfParam.getCoalesceDistance())
        ...
        PbfInfo pbfInfo = new PbfInfo()
                ...
                .setZoom(zoom).setGridSrid(pbfParam.getOutGridSrid());
        if (pbfParam.isCreateLabel()) {
     ...
     * @return 更新后的合并几何映射
     */
    private static Map<String, Geometry> mergePointGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom, GirAdvOneRow feature) {
        try {
     ...
     * @return 更新后的合并几何映射
     */
    private static Map<String, Geometry> mergeMultiPointGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom, GirAdvOneRow feature) {
        try {
     ...
     * @return 更新后的合并几何映射
     */
    private static Map<String, Geometry> mergeLineStringGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom, GirAdvOneRow feature) {
        try {
     ...
     * @return 更新后的合并几何映射
     */
    private static Map<String, Geometry> mergeMultiLineStringGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom, GirAdvOneRow feature) {
        try {
     ...
     * @return 更新后的合并几何映射
     */
    private static Map<String, Geometry> mergePolygonGeoms(Map<String, Geometry> mergedGeomMap, Geometry geom, GirAdvOneRow feature) {
        try {
