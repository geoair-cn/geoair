...
public class StatisticUtils {
    ...
    public static void statAndWriteJson(
        ...
        try {
            ...
            // 根据FeatureRowID进行去重
            JavaPairRDD<String, Tuple2<String, GirAdvOneRow>> featureIdRdd = infoRdd
                    .flatMapToPair(tuple -> {
                        String tileKey = tuple._1;
                        PbfInfo pbfInfo = tuple._2;
                        List<GirAdvOneRow> featureList = pbfInfo.getThisPbfFeatureList() == null
                                ? new ArrayList<>()
            ...
            JavaPairRDD<String, GirAdvOneRow> tileKeyRdd = featureIdRdd
                    .mapToPair(tuple -> {
            ...
            JavaPairRDD<String, Map<Object, Long>> fieldValueCountRDD = fieldValueRDD.aggregateByKey(
                    new HashMap<>(),
                    (map, tuple) -> {
                        ...
                        if (map.size() == MAX_VALUE_COUNT_PER_FIELD) {
                            System.out.printf("%s [内存优化] 字段值数量达到阈值%d，停止统计新值 %n",
                ...
                if (!valueCount.isEmpty()) {
                    ...
                    if ("Number".equals(fieldType)) {
                        try {
                            ...
                            System.err.printf("%s [字段类型处理] 字段[%s]值类型转换失败：%s %n",
            ...
            // 11. 构建主图层JSON并写入
            TileStatRoot root = buildTileStatRoot(parameter.getLayerName(), geometryType, totalFeatureCount,
                    fieldNames, attributeStats, parameter);
            writeJsonToDB(root, staticTableName, parameter.getLayerName(), parameter.getEdition(),
            ...
            if (parameter.isCreateBoundary()) {
                TileStatRoot rootBoundary = buildTileStatRoot(parameter.getLayerNameBoundary(), "LineString", totalFeatureCount,
                        fieldNames, attributeStats, parameter);
    ...
    private static TileStatRoot buildTileStatRoot(
        ...
        for (AttributeStat attr : attributeStats) {
            String type = attr.getType() == null ? "String" :
                    attr.getType().equalsIgnoreCase("number") ? "Number" :
        ...
        System.out.printf("%s [构建JSON] 图层[%s]的统计JSON结构构建完成，耗时=%dms，JSON长度=%d %n",
                LOG_PREFIX, layerId, buildEndTime - buildStartTime, JSON.toJSONString(root).length());
        return root;
    }
    ...
    private static void writeJsonToDB(
        ...
        System.out.printf("%s [写入数据库] 准备写入图层[%s]到表[%s]，JSON长度=%d %n",
        ...
        for (int i = 0; i < retryTimes; i++) {
            try {
                ...
                System.out.printf("%s [写入数据库-重试%d] 开始写入图层[%s]到PostgreSQL %n",
                        LOG_PREFIX, i + 1, layerName);
                ...
                System.out.printf("%s [写入数据库-重试%d] 图层[%s]写入成功，耗时=%dms %n",
                        LOG_PREFIX, i + 1, layerName, writeEndTime - writeStartTime);
                return;
            } catch (Exception e) {
                lastException = e;
                long sleepTime = TimeUnit.SECONDS.toMillis(i + 1);
                System.err.printf("%s [写入数据库-重试%d] 图层[%s]写入失败：%s，休眠%dms后重试...%n",
        ...
        // 所有重试失败，抛出异常
        String errorMsg = String.format("%s 写入数据库失败（重试%d次），表名：%s，layer：%s",
