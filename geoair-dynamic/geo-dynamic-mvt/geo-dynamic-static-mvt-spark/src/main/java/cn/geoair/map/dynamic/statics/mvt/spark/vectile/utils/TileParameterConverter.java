...
/**
 * TileSliceParameter 与 tippecanoe 命令行参数双向转换工具类
 * 实现：
 * 1. toTippecanoeParams: TileSliceParameter -> 命令行参数字符串
 * 2. fromTippecanoeParams: 命令行参数字符串 -> TileSliceParameter
 */
public class TileParameterConverter {
    ...
    static {
        ...
        SWITCH_PARAM_MAPPING.put("enableFeatureLimitIs", "--feature-limit");          // DTO中false对应--no-feature-limit
        SWITCH_PARAM_MAPPING.put("enableFeatureSizeLimit", "--tile-size-limit");      // DTO中false对应--no-tile-size-limit
     ...
     * 解析短带值参数（-Z/-z/-r）
     */
    private static void parseShortValuedParam(TileSliceParameter parameter, String key, String value) {
        try {
...
//                    parameter.setResolution(intValue);
     ...
     * 解析带值参数（如 -f 1000）
     */
    private static void parseValuedParam(TileSliceParameter parameter, String dtoField, String value) {
        try {
            switch (dtoField) {
                case "sourceDataSrid":
