...
public class SparkStreamListener implements StreamingListener {
    ...
    public void onBatchCompleted(StreamingListenerBatchCompleted event) {
        ...
        // 打印处理延迟（若有）
        Optional.ofNullable(batchInfo.processingDelay()).ifPresent(delay ->
                log.info("任务处理延迟: {} ms", delay));
        Optional.ofNullable(batchInfo.totalDelay()).ifPresent(delay ->
                log.info("批次总延迟（提交→完成）: {} ms", delay));
