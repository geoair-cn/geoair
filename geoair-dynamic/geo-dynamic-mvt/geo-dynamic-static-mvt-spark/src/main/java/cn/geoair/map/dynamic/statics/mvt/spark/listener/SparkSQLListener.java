...
public class SparkSQLListener extends SparkListener {
	...
	public void onTaskStart(SparkListenerTaskStart taskStart) {
		...
		log.info("任务开始:stageId： {},taskId：{}, 分片Id：{}  执行器ID：{}节点信息{}", taskStart.stageId(), taskInfo.taskId(),
				taskInfo.partitionId(), taskInfo.executorId(), taskInfo.host());
	}
	...
	public void onTaskEnd(SparkListenerTaskEnd taskEnd) {
		...
		if (failed) {
			log.info("任务失败taskID: {},  分片ID：{}  失败原因：{},类型：{},节点信息{}", taskInfo.taskId(), taskInfo.partitionId(),
					taskEnd.reason(), taskEnd.taskType(), taskEnd.taskInfo().host());
		}
		else {
			log.info("任务完成taskID: {},  分片ID：{} 结束原因：{},类型：{},节点信息{}", taskInfo.taskId(), taskInfo.partitionId(),
					taskEnd.reason(), taskEnd.taskType(), taskEnd.taskInfo().host());
		}
	}
