...
public class VectorTileCommonUtils {
	...
	public static GirAdvOneRow transformSingleFeature(GirAdvOneRow feature, TileSliceParameter parameter) {
		...
		// 坐标系转换
		Geometry convertedGeom = GirAdvTools.getSridOpt().convert(geometry, parameter.getSourceDataSrid(),
	 ...
	 * 通用要素映射到瓦片（单条要素）
	 */
	public static Map<String, List<GirAdvOneRow>> mapSingleFeatureToTiles(GirAdvOneRow feature, String geomFieldName,
			Integer minZoom, Integer maxZoom, int outGridSrid) {
		...
		for (int zoom = minZoom; zoom <= maxZoom; zoom++) {
			Tuple4<Integer, Integer, Integer, Integer> tileRange = TileUtils.rangeToIndex(zoom, geom, outGridSrid);
	...
	public static Map<String, GirAdvOneRow> mapSingleFeatureToTiles1(GirAdvOneRow feature,
		...
		for (int zoom = minZoom; zoom <= maxZoom; zoom++) {
			Tuple4<Integer, Integer, Integer, Integer> tileRange = TileUtils.rangeToIndex(zoom, geom,
					parameter.getOutGridSrid());
			int xmin = tileRange._1();
			int xmax = tileRange._2();
		...
		if (parameter.isEnableFeatureLimitIs() && parameter.getFeatureLimit() != null) {
			...
			if (limitList.size() > limit) {
				...
				if (parameter.isCoalesceDensestAsNeeded()) {
					limitList = AdvMvtDensityUtils.doCoalesceBySpatialDensity(limitList, limit,
							parameter.getGeomFieldName(), parameter.getIdFieldName(), parameter.getOutGridSrid());
				}
				// 空间密度过滤
				if (limitList.size() > limit && parameter.isDropDensestAsNeeded()) {
					limitList = AdvMvtDensityUtils.doFilterBySpatialDensity(limitList, limit,
							parameter.getGeomFieldName(), parameter.getIdFieldName(), parameter.getOutGridSrid());
				}
				// 兜底截断
				if (limitList.size() > limit) {
	 ...
	 * 通用PBF生成
	 */
	public static PbfInfo generateSingleTilePbf(String tileId, List<GirAdvOneRow> features,
			TileSliceParameter parameter, PbfTargetInfo pbfTargetInfo) throws Exception {
		...
		return AdvMvtDensityUtils.doValidateAndOptimizeTileSize(tileId, rawPbf, features, envelope, zoom,
				pbfTileParameter);
	}
