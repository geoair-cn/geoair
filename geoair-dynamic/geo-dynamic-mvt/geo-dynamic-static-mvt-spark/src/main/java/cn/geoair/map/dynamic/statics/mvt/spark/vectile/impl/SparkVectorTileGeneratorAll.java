...
public class SparkVectorTileGeneratorAll implements Serializable {
    ...
    //  1000 个分区代表总共有 1000 个任务要执行，但 Spark 会根据集群的并行度（由 Executor 数量、每个 Executor 的核心数决定）来控制「同时运行的任务数」；
//    执行节点只有 8 个：假设每个节点只运行 1 个 Executor、每个 Executor 分配 1 个核心，那么集群的最大并行度就是 8—— 同一时间最多只有 8 个分区任务在执行；
//    任务分批执行：先执行 8 个分区的任务（创建 8 个数据库连接），其中一个任务执行完成后，再调度第 9 个分区的任务（创建第 9 个连接，此时前 8 个中已有 1 个释放）。
    ...
    public void doGenerate(TileSliceParameter parameter) throws Exception {
        ...
        ReadStrategy strategy = Optional.ofNullable(parameter.getReadStrategy())
                .orElse(ReadStrategy.ID_PAGE);
        ...
        // 2. 空间转换（无持久化，流式处理）
        JavaRDD<GirAdvOneRow> transformedFeatures = persistedFeaturesRDD.map(
                new SparkTaskSerializableUtil.TransformFeatureFunction(parameter)
        );


        // 3. 要素映射到瓦片（无持久化）
        JavaPairRDD<String, List<GirAdvOneRow>> tileFeatures = transformedFeatures.flatMapToPair(
        ...
        if (parameter.isStatisticsIs()) {
            ...
            copy.setFeatureLimit(1000).setEnableFeatureLimitIs(true)
            ...
            Seq<Tuple2<String, PbfInfo>> tuple2Seq = JavaConverters.asScalaIteratorConverter(tuple2s.iterator()).asScala().toSeq();
            ClassTag<Tuple2<String, PbfInfo>> classTag = ClassTag$.MODULE$.apply(Tuple2.class);
            JavaRDD<Tuple2<String, PbfInfo>> tuple2sRDD = sparkSession.sparkContext().parallelize(tuple2Seq, DEFAULT_REDUCE_PARTITION, classTag).toJavaRDD();
    ...
    private void streamWriteToPg(JavaPairRDD<String, List<GirAdvOneRow>> aggregatedRDD,
                                 TileSliceParameter parameter,
                                 Map<String, String> pgParams) {

        // 1. 生成PBF并在分区内逐批构建Row、批量写入三个表
        aggregatedRDD.foreachPartition((VoidFunction<Iterator<Tuple2<String, List<GirAdvOneRow>>>>) partitionIterator -> {
            Log log = Log.get();
            ...
            try {
                ...
                if (StringUtils.isNotBlank(rootTableName)) {
                    rootPstmt = conn.prepareStatement(String.format(insertSqlTemplate, StrUtil.wrap(rootTableName, "\"")));
                }
                // Label表
                if (StringUtils.isNotBlank(labelTableName)) {
                    labelPstmt = conn.prepareStatement(String.format(insertSqlTemplate, StrUtil.wrap(labelTableName, "\"")));
                }
                // Boundary表
                if (StringUtils.isNotBlank(boundaryTableName)) {
                    boundaryPstmt = conn.prepareStatement(String.format(insertSqlTemplate, StrUtil.wrap(boundaryTableName, "\"")));
                }

                // 初始化PBF生成函数
                SparkTaskSerializableUtil.GeneratePbfFunction pbfFunc = new SparkTaskSerializableUtil.GeneratePbfFunction(parameter, PbfTargetInfo.getInstance());
        ...
        // 日志汇总
        log.info("所有图层流式写入完成，表名：Root={}, Label={}, Boundary={}",
    ...
    private JavaRDD<GirAdvOneRow> readDataByIdPage(TileSliceParameter parameter) throws Exception {
        ...
        JavaRDD<GirAdvOneRow> featureRDD = pageNumRdd.flatMap(
    ...
    private JavaRDD<GirAdvOneRow> readDataByBBox(TileSliceParameter parameter) throws Exception {
        ...
        List<String> partitionConditions = DataReadCommonUtils.buildBboxPartitionConditions(
        ...
        JavaRDD<GirAdvOneRow> featureRDD = partitionConditionRdd.flatMap(
    ...
    private Row buildRow(Tuple2<String, PbfInfo> tuple,
                         boolean isLabel,
                         boolean isBoundary,
