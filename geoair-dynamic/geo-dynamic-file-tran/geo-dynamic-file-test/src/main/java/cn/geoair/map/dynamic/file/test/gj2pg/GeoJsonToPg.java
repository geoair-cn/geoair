...
public class GeoJsonToPg {
	...
	public static void main(String[] args) throws IOException {
		GeoJsonLinkInfo geoJsonLinkInfo = new GeoJsonLinkInfo()
				.setGeoJsonFilePath("C:\\Users\\Administrator\\Documents\\aaaa.geojson").setCharset("UTF-8");
		GeoJsonGeoFileReader geoJsonReader = new GeoJsonGeoFileReader();
		geoJsonReader.setLinkInfo(geoJsonLinkInfo);

		PostgisLinkInfo postgisWriterLinkInfo = new PostgisWriterLinkInfo().setTableName("geo_tran_demo")
		...
		GeoFileTran tran = new GeoFileTranImpl()
				...
				.setProgressListener(progress -> {
					System.out.println(String.format("进度更新：已处理 %d 条，成功率 %.2f%%，状态 %s", progress.getTotalCount(),
