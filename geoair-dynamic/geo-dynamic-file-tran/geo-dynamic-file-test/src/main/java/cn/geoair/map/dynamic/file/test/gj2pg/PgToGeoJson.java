...
public class PgToGeoJson {
    public static void main(String[] args) throws IOException {
        ...
        LinkInfo writeLink = new GeoJsonLinkInfo()
                .setGeoJsonFilePath("C:\\Users\\Administrator\\Documents\\6666.geojson")
                .setCharset("UTF-8");
        ...
        PostgisLinkInfo postgisLinkInfo = new PostgisReadLinkInfo()
                .setQuerySqlByOutPut("select * from geo_tran_demo ")
        ...
        TranContext context = new TranContext()
        ...
        if (result.getStatus() == TranStatus.SUCCESS) {
            log.info("转换成功！总条数：{}，成功率：{:.2f}%，耗时：{}ms",
                    result.getTotalCount(),
                    result.getSuccessRate(),
                    result.getElapsedTime());
        } else {
            log.error("转换失败！错误信息：{}，异常列表：{}",
                    result.getErrorMsg(),
