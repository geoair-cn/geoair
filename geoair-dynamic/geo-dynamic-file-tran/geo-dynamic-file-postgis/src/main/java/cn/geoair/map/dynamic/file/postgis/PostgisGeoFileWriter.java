...
public class PostgisGeoFileWriter implements GeoFileWriter {
    ...
    public GeoFileWriter writeHeader(SimpleFeatureType featureType, ExceptionConsumer exceptionConsumer) {
        try {
            ...
            CoordinateReferenceSystem crs = GirAdvTools.getSridOpt().getCRS(writeConfig.getOutPutSrid());
            ...
            // 初始化要素写入器
            this.featureStore = (FeatureStore<SimpleFeatureType, SimpleFeature>) postgisDataStore.getFeatureSource(linkInfo.getTableName());
        ...
        try {
            ...
            for (Map.Entry<String, Object> entry : girAdvOneRow.entrySet()) {
                ...
                if (value instanceof Geometry) {
                    ...
                    Geometry convert = GirAdvTools.getSridOpt().convert(geom, srid, writeConfig.getOutPutSrid());
                    if (convert == null) {
