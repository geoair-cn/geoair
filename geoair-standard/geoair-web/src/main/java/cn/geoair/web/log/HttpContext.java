 ...
 * <ul>
 *   <li><b>请求信息</b>：请求方法、URI、查询参数、请求头、请求体等</li>
 *   <li><b>响应信息</b>：状态码、响应头、响应体、内容类型等</li>
 *   <li><b>性能指标</b>：请求到达时间、响应开始/结束时间、总耗时等</li>
...
public class HttpContext {
     ...
     * 查询字符串（URL 中 ? 后面的部分）
     * <p>
     * 示例：{@code page=1&size=20&sort=asc}
     */
     ...
     * 客户端真实 IP 地址
     * <p>
     * 注意：如果请求经过代理（如 Nginx），此字段可能需要从
     * {@code X-Forwarded-For} 或 {@code X-Real-IP} 头中获取真实 IP。
     */
     ...
     * <p>
     * 示例：{@code Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36}
     */
     ...
     * <b>注意：</b>
     * <ul>
     *   <li>仅适用于文本类型（如 JSON、XML、Form 表单），二进制数据（如图片、文件）不记录</li>
     *   <li>可能包含密码、Token 等敏感信息，记录前需要进行脱敏处理</li>
     *   <li>建议设置大小上限（如 4096 字符），超出部分截断</li>
     * </ul>
     */
     ...
     * 请求体大小（字节数）
     * <p>
     * 通过 {@code request.getContentLength()} 获取，用于分析请求大小分布。
     */
     ...
     * <b>注意：</b>
     * <ul>
     *   <li>为避免内存溢出，建议对大响应体（如文件下载）不记录或采样记录</li>
     *   <li>如需日志输出，建议转换为字符串并做截断处理</li>
     *   <li>二进制响应（如图片、视频）不应记录</li>
     * </ul>
     */
     ...
     * 常见状态码：
     * <ul>
     *   <li>2xx：成功（如 200 OK、201 Created）</li>
     *   <li>3xx：重定向（如 301 Moved Permanently）</li>
     *   <li>4xx：客户端错误（如 400 Bad Request、404 Not Found）</li>
     *   <li>5xx：服务端错误（如 500 Internal Server Error）</li>
     * </ul>
     */
     ...
     * <p>
     * 记录业务处理完成、开始输出响应的时间。
     * 通过 {@code responseStartTime - requestStartTime} 可计算业务处理耗时。
     */
     ...
     * <p>
     * 记录响应完全写回客户端的时间。
     * 通过 {@code responseEndTime - responseStartTime} 可计算响应输出耗时。
     */
     ...
     * 请求总耗时（毫秒）
     * <p>
     * 从请求到达（{@code requestStartTime}）到响应完成（{@code responseEndTime}）的总时间。
     * <p>
     * 总耗时 = 业务处理耗时 + 响应输出耗时 + 网络传输耗时
     */
     ...
     * <p>
     * 记录请求处理过程中抛出的异常消息。
     * 示例：{@code "Connection refused: connect"}
     */
     ...
     * <p>
     * 记录异常类的全限定名。
     * 示例：{@code "java.net.ConnectException"}
     */
     ...
     * <b>注意：</b>
     * <ul>
     *   <li>只保留前 N 行（如前 10 行），避免日志过大</li>
     *   <li>仅在调试模式或特定错误级别下记录</li>
     *   <li>生产环境建议只记录 {@link #errorMessage} 和 {@link #errorType}</li>
     * </ul>
     */
     ...
     * 记录关键请求头，如：
     * <ul>
     *   <li>{@code Authorization}：认证信息（需脱敏）</li>
     *   <li>{@code Content-Type}：请求体类型</li>
     *   <li>{@code Accept}：客户端可接受的响应类型</li>
     *   <li>{@code X-Request-ID}：请求追踪ID</li>
     * </ul>
     * <p>
     * <b>注意：</b>包含 Cookie、Authorization 等敏感信息时需要脱敏。
     */
     ...
     * 包含：
     * <ul>
     *   <li>查询参数（Query Parameters）：URL 中 ? 后面的参数</li>
     *   <li>表单参数（Form Parameters）：POST 表单提交的参数</li>
     * </ul>
     * <p>
     ...
     * 记录关键响应头，如：
     * <ul>
     *   <li>{@code Content-Type}：响应体类型（同时记录到 {@link #responseMimeType}）</li>
     *   <li>{@code Content-Length}：响应体大小（同时记录到 {@link #responseBodySize}）</li>
     *   <li>{@code Cache-Control}：缓存策略</li>
     *   <li>{@code Set-Cookie}：设置的 Cookie（需脱敏）</li>
     * </ul>
     */
     ...
     * 响应内容的 MIME 类型（字符串形式）
     * <p>
     * 从响应头 {@code Content-Type} 中获取。
     * <p>
     * 示例：{@code "application/json;charset=UTF-8"}
     * <p>
     * <b>注意：</b>此字段为 {@link #responseMimeType} 的字符串补充，
     * 当 {@code GiMimeType} 无法解析时保留原始值。
     */
     ...
     * 响应内容编码方式
     * <p>
     * 从响应头 {@code Content-Encoding} 中获取。
     * <p>
     * 常见值：
     * <ul>
     *   <li>{@code gzip}：GZIP 压缩</li>
     *   <li>{@code deflate}：Deflate 压缩</li>
     *   <li>{@code br}：Brotli 压缩</li>
     *   <li>{@code null}：无压缩（默认）</li>
     * </ul>
     * <p>
     * 用于评估压缩效果和排查解压问题。
     */
