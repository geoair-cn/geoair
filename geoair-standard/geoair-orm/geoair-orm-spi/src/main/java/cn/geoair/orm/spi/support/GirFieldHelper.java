...
public class GirFieldHelper {
    ...
    static class Jdk8FieldHelper implements IFieldHelper {
         ...
         * @return
         */
        private List<GirEntityField> _getFields(
                Class<?> entityClass, List<GirEntityField> fieldList, Integer level) {
            if (fieldList == null) {
    ...
    static class Jdk6_7FieldHelper implements IFieldHelper {
            ...
            for (PropertyDescriptor desc : descriptors) {
                if (desc != null && !"class".equals(desc.getName())) {
                    ...
                    if (desc.getReadMethod() != null
                        ...
                        girEntityField.setJavaType(
                                genericMap.get(
                                        ((TypeVariable) desc.getReadMethod().getGenericReturnType())
                                                .getName()));
         ...
         * @param level
         */
        private void _getFields(
                Class<?> entityClass,
                List<GirEntityField> fieldList,
                Map<String, Class<?>> genericMap,
                Integer level) {
            if (fieldList == null) {
                    ...
                    if (field.getGenericType() != null
                        ...
                        if (genericMap == null
                                || !genericMap.containsKey(
                                        ((TypeVariable) field.getGenericType()).getName())) {
                            throw new RuntimeException(
                                    entityClass + "字段" + field.getName() + "的泛型类型无法获取!");
                        } else {
                            girEntityField.setJavaType(
                                    genericMap.get(
                                            ((TypeVariable) field.getGenericType()).getName()));
        ...
        private Map<String, Class<?>> _getGenericTypeMap(Class<?> entityClass) {
            ...
            if (superClass != null
                ...
                if (entityClass.getGenericSuperclass() instanceof ParameterizedType) {
                    Type[] types =
                            ((ParameterizedType) entityClass.getGenericSuperclass())
