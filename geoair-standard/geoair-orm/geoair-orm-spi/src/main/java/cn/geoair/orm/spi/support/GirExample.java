...
public class GirExample {
     ...
     * @param entityClass
     */
    public GirExample(Class<?> entityClass) {
     ...
     */
    public GirExample(Class<?> entityClass, boolean exists) {
    ...
    /**
     * 不带 entityClass 的构造函数 用于继承替换拓展 部分参数用于给子类填充
     *
     * @param
     * @param exists - true时，如果字段不存在就抛出异常，false时，如果不存在就不使用该字段的条件
     * @param notNull - true时，如果值为空，就会抛出异常，false时，如果为空就不使用该字段的条件
     */
    public GirExample(boolean exists, boolean notNull) {
        this.exists = exists;
        this.notNull = notNull;
        oredCriteria = new ArrayList<Criteria>();
        // 之类注意要填充这三个参数
    ...
    private GirExample(Builder builder) {
     ...
     * @return
     */
    public GirExample excludeProperties(String... properties) {
     ...
     * @return
     */
    public GirExample selectProperties(String... properties) {
        if (properties != null && properties.length > 0) {
    ...
    public static class OrderBy {
        ...
        public OrderBy orderBy(String property) {
            ...
            if (GutilStr.isNotEmpty(girExample.getOrderByClause())) {
                girExample.setOrderByClause(girExample.getOrderByClause() + "," + column);
            } else {
                girExample.setOrderByClause(column);
            }
            isProperty = true;
            return this;
        }
        ...
        public OrderBy desc() {
            if (isProperty) {
                girExample.setOrderByClause(girExample.getOrderByClause() + " DESC");
                isProperty = false;
            }
            return this;
        }
        ...
        public OrderBy asc() {
            if (isProperty) {
                girExample.setOrderByClause(girExample.getOrderByClause() + " ASC");
                isProperty = false;
            }
            return this;
        }
    ...
    protected abstract static class GeneratedCriteria {
        ...
        protected GeneratedCriteria(
                Map<String, GirEntityColumn> propertyMap, boolean exists, boolean notNull) {
            super();
            this.exists = exists;
            this.notNull = notNull;
         ...
         *
         * @param param 参数对象
         * @author Bob {@link}0haizhu0@gmail.com @Date 2015年7月17日 下午12:48:08
         */
    ...
    public static class Builder {
        ...
        private List<GirSqls.Criteria> sqlsCriteria;

        // 动态表名
        private List<GirExample.Criteria> exampleCriterias;
        ...
        public Builder where(GirSqls girSqls) {
            GirSqls.Criteria criteria = girSqls.getCriteria();
            criteria.setAndOr("and");
        ...
        public Builder andWhere(GirSqls girSqls) {
            GirSqls.Criteria criteria = girSqls.getCriteria();
            criteria.setAndOr("and");
            this.sqlsCriteria.add(criteria);
            return this;
        ...
        public Builder orWhere(GirSqls girSqls) {
            GirSqls.Criteria criteria = girSqls.getCriteria();
            criteria.setAndOr("or");
            this.sqlsCriteria.add(criteria);
            return this;
        ...
        public GirExample build() {
            this.exampleCriterias = new ArrayList<Criteria>();
            for (GirSqls.Criteria criteria : sqlsCriteria) {
                GirExample.Criteria exampleCriteria =
                        new GirExample.Criteria(this.propertyMap, this.exists, this.notNull);
                exampleCriteria.setAndOr(criteria.getAndOr());
                for (GirSqls.Criterion criterion : criteria.getCriterions()) {
            ...
            if (this.orderByClause.length() > 0) {
                this.orderByClause =
                        new StringBuilder(
                                this.orderByClause.substring(1, this.orderByClause.length()));
            }

            return new GirExample(this);
        }

        private void transformCriterion(
                GirExample.Criteria exampleCriteria,
                String condition,
                String property,
                Object[] values,
            ...
            if (values.length == 0) {
                ...
                if ("and".equals(andOr)) {
                    exampleCriteria.addCriterion(
                            column(property) + " " + condition, values[0], property(property));
                } else {
                    exampleCriteria.addOrCriterion(
                            column(property) + " " + condition, values[0], property(property));
                }
    ...
    public Set<String> getSelectColumns() {
        if (selectColumns != null && selectColumns.size() > 0) {
            ...
            selectColumns =
                    new LinkedHashSet<String>(girEntityColumns.size() - excludeColumns.size());
