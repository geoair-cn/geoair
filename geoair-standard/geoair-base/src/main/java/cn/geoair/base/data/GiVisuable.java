...
public interface GiVisuable extends Serializable {
    ...
    public static final Predicate<Field> displayFieldPredicate =
            s -> {
                GaModelField mf = s.getAnnotation(GaModelField.class);
                return mf != null && mf.isDisplay();
            };
    ...
    default String display() {
        ...
        if (myClass.isEnum()) {
            ...
            if (flen == 0) {
                ...
                Gir.log.error(
                        "枚举 {} 没有标记显示键,在显示属性上加注解@GaModelField(isDisplay = true)或覆写 display()方法",
                        myClass.getName());
            }
            ...
            Gir.log.error(
                    "类  {} 没有标记显示键,在值属性上加注解@GaModelField(isDisplay = true)或覆写 display()方法",
                    myClass.getName());
        }
        return null;
    }
