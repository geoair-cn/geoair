...
public interface GiCreateDao<M extends GiEntitySaveable<PK>, PK extends Serializable>
        extends GiDao<M, PK> {

    @SuppressWarnings("unchecked")
    public static <M extends GiEntitySaveable<PK>, PK extends Serializable>
            GiCreateDao<M, PK> getDao(Class<M> modelCls) {
     ...
     *
     * <p>注意：由于ORM框架会判断字段注解@Column(insertable=false)时不处理字段， 所以也不能说完全无视数据库默认值，前提是插入语句包含该字段
     *
     * @param t 实体对象
     * @return PK 主键
     */
     ...
     *
     * <p>批量插入语句字段是固定的，所以默认实现只是循环调用单条插入方法
     *
     * @param records 实体对象列表
     * @return 主键 List<PK>
     */
