			...
			try {
				...
				for (Class<?> t : ts) {
					if (!requiredType.isAssignableFrom(t)) {
						throw new GirException("类 {} 配置sp 默认placeHolderClass实现类 ：{} 类型不匹配", requiredType.getName(),
								t.getName());
					}
				}
				...
				if (clsNames.length > 0) {
					...
					for (int i = 0; i < clsNames.length; i++) {
						try {
							...
							throw new GirException(e, "类 {} 配置sp 默认placeHolder实现类 ：{} 未找到", requiredType.getName(),
									clsNames[i]);
						}
						if (!requiredType.isAssignableFrom(cls2[i])) {
							throw new GirException("类 {} 配置sp 默认placeHolder实现类 ：{} 类型不匹配", requiredType.getName(),
									clsNames[i]);
						}
					}
		...
		if (res == null) {
			...
			if (len > 0) {
				...
				try {
					...
					throw new GirException(e, "类 {} 配置sp 默认placeHolder实现类 ：{} 实例化对象发生错误", requiredType.getName(),
							cls.getName());
				}
			}
		}
