...
public class GkMethodHand {
    ...
    private static StackTraceElement getCallerStackTraceElement() {
        ...
        for (StackTraceElement ste : stes) {
            if (!ste.getClassName().equals(java.lang.Thread.class.getName())
                    && !ste.getClassName().equals(GkMethodHand.class.getName())) {
                return ste;
            }
        }
        return null;
    ...
    private static boolean matchType(Object[] args, Class<?>[] types) {
        ...
        if (len == types.length) {
            for (int i = 0; i < len; i++) {
                if (args[i] != null
                        && !GutilClass.resolvePrimitiveIfNecessary(types[i])
            ...
            if (!GutilStr.isEmpty(expectClassName)) {
                try {
                    return Class.forName(
                            expectClassName, true, Thread.currentThread().getContextClassLoader());
                } catch (Exception e) {
                }
            }
            return null;
    ...
    public static GkMethodHand defineFromMethod(Method method) {
        GaMethodHandDefine conf = method.getAnnotation(GaMethodHandDefine.class);
        if (conf == null) {
            return GkMethodHand.define(
                    getMethodHandleId(method), method.getReturnType(), method.getParameterTypes());
        } else {
            return GkMethodHand.define(
                    getMethodHandleId(method),
                    GutilStr.emptyToDefault(conf.expectClassName(), null),
                    GutilStr.emptyToDefault(conf.expectMethodName(), method.getName()),
                    method.getReturnType(),
                    method.getParameterTypes());
        }
    }

    public static GkMethodHand define(
            String handleId, Class<?> returnType, Class<?>... parameterTypes) {
        return define(handleId, null, null, returnType, parameterTypes);
    }

    private static GkMethodHand define(
            String handleId,
    ...
    private class Impl {
        ...
        public boolean isExpect() {
            ...
            for (Impl imp : impls.values()) {
                if (imp != this) {
                    if (imp.isInitExpect) {
                        if (Objects.equals(imp.method, this.method)
                                || Objects.equals(imp.methodHandle, this.methodHandle)) {
                            return true;
                        }
                    }
                }
        ...
        private Impl(
                ...
                Method method,
                MethodHandle handle,
                GaMethodHandImpl.ImplType implType) {
            ...
            if (method != null) {
                ...
                try {
                    ...
                    throw new IllegalStateException(
                            "GkMethodHand.Impl(Method)发生错误:" + getMethodHandleId(method), e);
                }
            }
        }
    ...
    public static void implFromClass(Class<?> clz) {
        Method[] methods = clz.getDeclaredMethods();
        for (Method method : methods) {
            if (method.getAnnotation(GaMethodHandImpl.class) != null) implFromMethod(method);
        }
    }
    ...
    public static void implFromMethod(Method method) {
        ...
        if (conf != null) {
                ...
                try {
                    md =
                            implClass.getDeclaredMethod(
                                    GutilStr.emptyToDefault(conf.implMethod(), method.getName()),
                                    method.getParameterTypes());
                } catch (Exception e) {
                    throw new IllegalStateException(
                            "GkMethodHand.implFromMethod(Method)发生错误,所需要实现的类中没有找到方法:"
                                    + method.toString(),
                            e);
                }
            ...
            throw new IllegalStateException(
                    "GkMethodHand.implFromMethod(Method)发生错误,方法必须带有GaMethodHandImpl注解,"
                            + method.toString());
        }
    }
    ...
    public static void prints(String flag) {
        for (Entry<String, GkMethodHand> ent : setups.entrySet()) {
            GkConsole.log(
                    "GkMethodHand print --------------------------------------------------start-------------------------------------------------------"
                            + flag);
            ent.getValue().print();
            GkConsole.log(
                    "GkMethodHand print ---------------------------------------------------end------------------------------------------------------"
                            + flag);
        }
    }
    ...
    public static Object invokeSelf(Object... args) {
        ...
        throw new IllegalStateException(
                GutilStr.format(
                        "GkMethodHand.invokeSelf(Object...)执行失败,可能的原因是方法参数个数与声明方法参数个数不一致 at {}",
                        selfId));
    }
    ...
    public Object invoke(Object... args) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "GkMethodHand(" + this.getId() + ").invoke(Object...)发生错误 at " + getCallerId(),
                    e);
        }

        /*
         * try { Object res = null; int len = args.length; switch(len) { case 0: res =
         * mh.invoke();break; case 1: res = mh.invoke(args[0]);break; case 2: res =
         * mh.invoke(args[0],args[1]);break; case 3: res =
         * mh.invoke(args[0],args[1],args[2]);break; case 4: res =
         * mh.invoke(args[0],args[1],args[2],args[3]);break; case 5: res =
         * mh.invoke(args[0],args[1],args[2],args[3],args[4]);break; case 6: res =
         * mh.invoke(args[0],args[1],args[2],args[3],args[4],args[5]);break; case 7: res =
         * mh.invoke(args[0],args[1],args[2],args[3],args[4],args[5],args[6]);break;
         * default:res = mh.invoke(args); } return res;
         *
         * } catch (Throwable e) { throw new
         * gtcException(" gtcMethodHand("+this.getId()+").invoke(Object...)发生错误 at "
         * +getCallerId(),e); }
         */
    }

    private Impl addImpl(
            Method method, MethodHandle methodHandle, GaMethodHandImpl.ImplType implType) {
    ...
    private boolean addDefine(
            MethodType methodType, String expectClassName, String expectMethodName) {
        String key = getMyCallerId();
    ...
    private GkMethodHand setMethodType(MethodType methodType) {
        if (this.methodType == null) {
            ...
            throw new IllegalStateException(
                    "GkMethodHand.setMethodType(MethodType)设置了不同的methodType at " + getCallerId());
        }
        return this;
    }
