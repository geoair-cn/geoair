...
public class GkMethodHand {
	...
	private static boolean matchType(Object[] args, Class<?>[] types) {
		...
		if (len == types.length) {
			for (int i = 0; i < len; i++) {
				if (args[i] != null
						&& !GutilClass.resolvePrimitiveIfNecessary(types[i]).isAssignableFrom(args[i].getClass())) {
					return false;
				}
			}
			return true;
	...
	public static GkMethodHand defineFromMethod(Method method) {
		...
		if (conf == null) {
			...
			return GkMethodHand.define(getMethodHandleId(method), GutilStr.emptyToDefault(conf.expectClassName(), null),
					GutilStr.emptyToDefault(conf.expectMethodName(), method.getName()), method.getReturnType(),
	...
	private static GkMethodHand define(String handleId, String expectClassName, String expectMethodName,
	...
	public static enum ImplStatus {
		...
		empty, expected, appointed, appointed_expected, other
	...
	public static void implFromMethod(Method method) {
		...
		if (conf != null) {
			...
			if (GutilStr.isEmpty(implId)) {
				...
				Method md = null;
				try {
					md = implClass.getDeclaredMethod(GutilStr.emptyToDefault(conf.implMethod(), method.getName()),
							method.getParameterTypes());
				}
				catch (Exception e) {
					throw new IllegalStateException(
							"GkMethodHand.implFromMethod(Method)发生错误,所需要实现的类中没有找到方法:" + method.toString(), e);
				}
			...
			throw new IllegalStateException(
					"GkMethodHand.implFromMethod(Method)发生错误,方法必须带有GaMethodHandImpl注解," + method.toString());
		}
	}
	...
	public static Object invokeSelf(Object... args) {
		...
		throw new IllegalStateException(
				GutilStr.format("GkMethodHand.invokeSelf(Object...)执行失败,可能的原因是方法参数个数与声明方法参数个数不一致 at {}", selfId));
	}
	...
	public Object invoke(Object... args) {
		...
		if (mh == null) {
			throw new IllegalStateException(
					"GkMethodHand(" + this.getId() + ").invoke(Object...)没有对应的实现 at " + getCallerId());
		}
		...
		if (mh.isVarargsCollector()) {
			...
			if (lastArray != null && lastArray.length > 0) {
				...
				try {
					...
					throw new IllegalStateException(
							"GkMethodHand(" + this.getId() + ").invoke(Object...)发生错误 at " + getCallerId(), e);
				}
			}
		}
		try {
