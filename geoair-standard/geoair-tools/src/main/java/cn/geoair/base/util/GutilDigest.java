 ...
 *
 * <p>注意：MD5 已不再被视为密码学安全算法，碰撞攻击已可行。密码存储等安全敏感场景请使用带盐的
 * 加盐哈希算法（如 PBKDF2、bcrypt、scrypt）；本工具适用于文件完整性校验、幂等键生成等非安全场景。
 */
public abstract class GutilDigest {
     ...
     * 计算给定字节数组的 MD5 摘要。
     *
     * <p>注意：MD5 已不再被视为密码学安全算法，密码存储场景请使用带盐的加盐哈希算法
     * （如 PBKDF2、bcrypt、scrypt）。
     *
     ...
     * 签名与校验双方必须使用相同编码，否则结果不一致；跨系统或与其它语言交互时建议显式使用
     * {@link #md5(String, Charset)} 指定字符集。
     *
     * <p>注意：MD5 已不再被视为密码学安全算法，密码存储场景请使用带盐的加盐哈希算法
     * （如 PBKDF2、bcrypt、scrypt）。
     *
    ...
    /**
     * 将字节数组编码为小写十六进制字符数组，长度自适应（每个字节对应 2 个字符），
     * 可正确处理任意摘要算法（MD5/SHA-1/SHA-256 等）的输出长度。
     *
     ...
     *
     * <p>文件按 8KB 缓冲分块读取更新摘要。文件不存在、读取失败或计算异常时返回 null
     * （null 表示失败，调用方需自行区分）。
     *
     * <p>注意：MD5 已不再被视为密码学安全算法，密码存储场景请使用带盐的加盐哈希算法
     * （如 PBKDF2、bcrypt、scrypt）。
