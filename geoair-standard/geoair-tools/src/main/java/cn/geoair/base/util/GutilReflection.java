...
public abstract class GutilReflection {
     ...
     * @param <T> 类型
     * @return 返回设置后的对象
     *     <p>public static <T extends AccessibleObject> T setAccessible(T object) { return
     ...
     * @param filter 过滤器
     * @return 过滤后的方法列表
     *     <p>public static List<Method> getPublicMethods(Class<?> clazz, Filter<Method> filter) {
     *     if (null == clazz) { return null; }
     *     <p>final Method[] methods = getPublicMethods(clazz); List<Method> methodList; if (null !=
     ...
     * @return 过滤后的方法列表
     *     <p>public static List<Method> getPublicMethods(Class<?> clazz, Method... excludeMethods)
     *     { final HashSet<Method> excludeMethodSet = gtcCollectionUtil.newHashSet(excludeMethods);
     ...
     * @return 过滤后的方法列表
     *     <p>public static List<Method> getPublicMethods(Class<?> clazz, String...
     *     excludeMethodNames) { final HashSet<String> excludeMethodNameSet =
     *     gtcCollectionUtil.newHashSet(excludeMethodNames); return getPublicMethods(clazz, method
     *     -> false == excludeMethodNameSet.contains(method.getName())); }
     */
     ...
     *     <p>public static <T> T invokeStatic(Method method, Object... args) throws gtcException {
     ...
     *     <p>public static <T> T invokeWithCheck(Object obj, Method method, Object... args) throws
     *     gtcException { final Class<?>[] types = method.getParameterTypes(); if (null != args) {
     *     gtcAssert.isTrue(args.length == types.length, gtcStrUtil.format( "Params length [{}] is
     *     not fit for param length [{}] of method !", args.length, types.length)); Class<?> type;
     *     for (int i = 0; i < args.length; i++) { type = types[i]; if (type.isPrimitive() && null
     *     == args[i]) { // 参数是原始类型，而传入参数为null时赋予默认值 args[i] = gtcClassUtil.getDefaultValue(type); }
     *     } }
     *     <p>return invoke(obj, method, args); }
     */
     ...
     * @param args 参数对象
     * @return 结果
     * @throws GirException 一些列异常的包装 @SuppressWarnings("unchecked") public static <T> T
     *     invoke(Object obj, Method method, Object... args) throws gtcException {
     *     setAccessible(method);
     *     <p>// 检查用户传入参数： // 1、忽略多余的参数 // 2、参数不够补齐默认值 // 3、通过NullWrapperBean传递的参数,会直接赋值null //
     ...
     *     Object[parameterTypes.length]; if (null != args) { for (int i = 0; i < actualArgs.length;
     *     i++) { if (i >= args.length || null == args[i]) { // 越界或者空值 actualArgs[i] =
     *     gtcClassUtil.getDefaultValue(parameterTypes[i]); } else if (args[i] instanceof
     *     NullWrapperBean) { //如果是通过NullWrapperBean传递的null参数,直接赋值null actualArgs[i] = null; } else
     *     if (false == parameterTypes[i].isAssignableFrom(args[i].getClass())) {
     *     //对于类型不同的字段，尝试转换，转换失败则使用原对象类型 final Object targetValue =
     *     Convert.convert(parameterTypes[i], args[i]); if (null != targetValue) { actualArgs[i] =
     *     targetValue; } } else { actualArgs[i] = args[i]; } } }
     *     <p>try { return (T) method.invoke( gtcClassUtil.isStatic(method) ? null : obj,
     ...
     * @see NullWrapperBean
     * @since 3.1.2
     *     <p>public static <T> T invoke(Object obj, String methodName, Object... args) throws
     *     gtcException { final Method method = getMethodOfObj(obj, methodName, args); if (null ==
     *     method) { throw new gtcException( gtcStrUtil.format("No such method: [{}]", methodName));
