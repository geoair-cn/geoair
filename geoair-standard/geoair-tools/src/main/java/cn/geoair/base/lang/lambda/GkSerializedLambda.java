...
public class GkSerializedLambda implements Serializable {
    ...
    /**
     * capturingClass "捕获类"，当前的Lambda表达式出现的所在类 functionalInterfaceClass 名称，并且以"/"分隔，返回的Lambda对象的静态类型
     * functionalInterfaceMethodName 函数式接口方法名称 functionalInterfaceMethodSignature
     * 函数式接口方法签名（其实是参数类型和返回值类型，如果使用了泛型则是擦除后的类型） implClass
     * 名称，并且以"/"分隔，持有该函数式接口方法的实现方法的类型（实现了函数式接口方法的实现类） implMethodName 函数式接口方法的实现方法名称
     * implMethodSignature 函数式接口方法的实现方法的方法签名（实是参数类型和返回值类型） instantiatedMethodType 用实例类型变量替换后的函数式接口类型
     * capturedArgs Lambda捕获的动态参数 implMethodKind 实现方法的MethodHandle类型
     */
