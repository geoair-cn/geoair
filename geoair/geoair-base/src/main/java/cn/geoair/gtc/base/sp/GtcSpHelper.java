...
import cn.geoair.gtc.base.sp.support.GtcPlaceHolderSpLoader;
 ...
 * <p>
 * 提供服务发现与加载功能，支持基于注解的SPI机制，可以根据接口类型动态加载其实现类。
 * 支持单例模式缓存，避免重复创建实例，提高系统性能。
 *
 * @author Ray
 */
public class GtcSpHelper {
	 ...
	 * GkSpLoader加载器缓存
	 * <p>
	 * 用于缓存已创建的GkSpLoader实例，避免重复创建，提高性能。
	 * Key为Loader类类型，Value为对应的Loader实例。
	 */
     ...
     * 获取GkSpLoader实例
     * <p>
     * 首先从缓存中查找指定类型的Loader实例，如果不存在则创建新实例并加入缓存。
     * 使用重入锁保证多线程环境下的线程安全。
     *
     * @param loaderClass Loader类类型
     * @return GkSpLoader实例
     * @throws GtcException 当创建Loader实例失败时抛出异常
     */
    private static GkSpLoader getGkSpiLoader(Class<? extends GkSpLoader> loaderClass) {
	    ...
	    if (LOADER_CACHE.containsKey(loaderClass)) {
	        ...
	        try {
				...
				throw new GtcException(e,"创建GkSpLoader发生错误：{} ",loaderClass.getName());
			} finally {
	            LOCK.unlock();
	        }
	    }
     ...
     * <p>
     * 使用GkConcurrentReferenceHashMap作为缓存容器，支持弱引用以避免内存泄漏。
     * 当接口标记为非单例时，不会缓存其实例。
     */
     ...
     * @throws GtcException 当接口未添加@GkSP注解时抛出异常
     */
    ...
    public static <T> T load(Class<T> requiredType,Type... types) {
    	...
    	if(res == null) {
	    	...
	    	if(res == null) {
	    		res = getGkSpiLoader( GtcPlaceHolderSpLoader.class).load(requiredType,types);
	    	}
