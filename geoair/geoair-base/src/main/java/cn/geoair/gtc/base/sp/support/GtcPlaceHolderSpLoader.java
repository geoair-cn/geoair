...
public class GtcPlaceHolderSpLoader extends GtcCacheSpLoader {


	/**
	 * placeHolder 缓存
	 */
	private final static  Map<Class<?>, Class<?>[]> PLACEHOLDERCLASS_CACHE = new ConcurrentHashMap<Class<?>, Class<?>[]>();
	...
	public static <T> Class<? extends T>[] getPlaceHolderClasses(Class<T> requiredType) {
    	...
    	if(spiAn == null) {
    		throw new GtcException("获取sp实现的接口必须包含GkSP注解：{} ",requiredType.getName());
    	}
		Class<?>[] ts = null;
	        ...
	        try {
            	...
            	for(Class<?> t:ts) {
            		if(!requiredType.isAssignableFrom(t)) {
	    				throw new GtcException("类 {} 配置sp 默认placeHolderClass实现类 ：{} 类型不匹配",requiredType.getName(),t.getName());
	    			}
            	}
    			...
    			if(clsNames.length > 0) {
					...
					for(int i = 0;i<clsNames.length;i++) {
						try {
	    					...
	    					throw new GtcException(e,"类 {} 配置sp 默认placeHolder实现类 ：{} 未找到",requiredType.getName(),clsNames[i]);
	    				}
						if(!requiredType.isAssignableFrom(cls2[i])) {
		    				throw new GtcException("类 {} 配置sp 默认placeHolder实现类 ：{} 类型不匹配",requiredType.getName(),clsNames[i]);
		    			}
					}
	 ...
	 * placeHolder Type 缓存
	 */
	private final static  Map<String, Class<?>[]> PLACEHOLDERCLASS_TYPE_CACHE = new ConcurrentHashMap<String, Class<?>[]>();
	...
	public <T> T load(Class<T> requiredType, Type[] types) {
    	...
    	if(res == null) {
    		...
    		if(len > 0) {
	    		if(phClzs.length != 1) {
	    			throw new GtcNoUniqueBeanException("类 {} 配置sp 默认placeHolder发现多个实现类",requiredType.getName());
	    		}
	    		Class<? extends T> cls = phClzs[0];
	    		try {
					res = cls.newInstance();
				} catch (Exception e) {
					throw new GtcException(e,"类 {} 配置sp 默认placeHolder实现类 ：{} 实例化对象发生错误",requiredType.getName(),cls.getName());
				}
    		}
    	}
    	if(res != null) {
