...
import cn.geoair.gtc.base.sp.support.GirPlaceHolderSpLoader;
import cn.geoair.gtc.base.tool.GkConcurrentReferenceHashMap;

/**
 * Service Provider 工具类
 * <p>
 * 提供服务发现与加载功能，支持基于注解的SPI机制，可以根据接口类型动态加载其实现类。 支持单例模式缓存，避免重复创建实例，提高系统性能。
 *
 * @author Ray
 */
public class GirSpHelper {
	 ...
	 * GkSpLoader加载器缓存
	 * <p>
	 * 用于缓存已创建的GkSpLoader实例，避免重复创建，提高性能。 Key为Loader类类型，Value为对应的Loader实例。
	 */
	 ...
	 * 获取GkSpLoader实例
	 * <p>
	 * 首先从缓存中查找指定类型的Loader实例，如果不存在则创建新实例并加入缓存。 使用重入锁保证多线程环境下的线程安全。
	 ...
	 * <p>
	 * 使用GkConcurrentReferenceHashMap作为缓存容器，支持弱引用以避免内存泄漏。 当接口标记为非单例时，不会缓存其实例。
	 */
	 ...
	 * 根据接口类型加载其实现类实例
	 * <p>
	 * 支持泛型类型参数，按以下顺序查找实现类： 1. 检查接口是否标记为单例，如果是则优先从缓存获取 2. 按照@GkSP注解中指定的loader顺序加载 3.
