...
import cn.geoair.gtc.base.convert.support.GtcConverterImpl;
...
public class GtcConvertHelper {

	static {
		GkMethodHand.implFromClass( GtcConvertHelper.class);
	}
	...
	@GaMethodHandImpl(implClass= GtcConvertHelper.class,implMethod="getProvider",type= GaMethodHandImpl.ImplType.comity)
	private static GiConverterProvider _getProvider() {
		return new GtcConverterImpl();
	}
	...
	public static<S, T> T convert(S source,Class<T> targetClass){
		...
		GiConverter<S, T> converter =  GtcConvertHelper.getProvider().getConverter(sc, targetClass);
