...
public class GirSdkUtil {
    ...
    private static HttpURLConnection setHeaders(HttpURLConnection conn) throws Exception {
        ...
        String clientId =
                GirSdkProfileConfig.getConfig(
                        GirSdkProfileConfig.ProfileLocal.get(),
    ...
    public static <T> T parseResult(String resString, Class<T> clazz, Type type) {
        try {
            if (clazz != null) {
                return Gir.toJson(resString)
                        .toBean(clazz); // JSONObject.parseObject(resString,clazz);
            }
            if (type != null) {
                return Gir.toJson(resString)
                        .toBean(type); // JSONObject.parseObject(resString,type);
            }
        } catch (Exception e) {
            log.error(e);
            ...
            try {
                ...
                String alertMsgKey =
                        GirSdkProfileConfig.getConfig(
                                GirSdkProfileConfig.ProfileLocal.get(),
                                GirSdkProfileConfig.BusType_reslut_msgkey,
                                "alertMsg");
    ...
    public static String appendUrl(String url, Map<String, Object> data, boolean encodeValue)
            throws Exception {
        if (url.contains("?")) {
    ...
    public static String appendUrl(String url, String key, String value, boolean encodeValue)
            throws Exception {
        if (value != null) {
            if (url.indexOf("?") >= 0) {
                return url
                        ...
                        + key
                        + "="
                        + (encodeValue ? URLEncoder.encode(value, CharsetName) : value);
            } else {
                return url
                        + "?"
                        + key
    ...
    public static String mapToUrl(Map<String, Object> data, boolean encodeValue) throws Exception {
        ...
        for (String key : data.keySet()) {
            ...
            if (value != null) {
                ...
                param.append(
                        encodeValue
                                ? URLEncoder.encode(data.get(key).toString(), CharsetName)
                                : value.toString());
                param.append("&");
            }
        }
    ...
    public static void validate(HttpServletRequest request) throws Exception {
        ...
        validateClient(
                request,
                GirSdkProfileConfig.getConfig(pe, GirSdkProfileConfig.BusType_clientId),
