...
import cn.geoair.gtc.sdk.body.GtcMultipartBody;
import  cn.geoair.gtc.sdk.GtcSdkProfileConfig.ProfileEnum;

public class GtcSdkUtil {
	private static final GiLoger log =  GtcLoger.getLoger( GtcSdkUtil.class);
	...
	private static HttpURLConnection setHeaders(HttpURLConnection conn) throws Exception {
		...
		String clientId =  GtcSdkProfileConfig.getConfig( GtcSdkProfileConfig.ProfileLocal.get(),  GtcSdkProfileConfig.BusType_clientId);
		String clientSecret =  GtcSdkProfileConfig.getConfig( GtcSdkProfileConfig.ProfileLocal.get(),  GtcSdkProfileConfig.BusType_clientSecret);
		...
		conn.setRequestProperty("Profile",  GtcSdkProfileConfig.ProfileLocal.get().name());
		return conn;
	}
	...
	public static<T> T postMultipartFile(String urlStr,Map<String,Object> data,Class<T> clazz,Type type) throws Exception {
		 GtcMultipartBody gtcMultipartBody =  GtcMultipartBody.create(data, StandardCharsets.UTF_8);
		String post = post(urlStr,  gtcMultipartBody);
	...
	public static<T> T postJson(String urlStr,Object data,Class<T> clazz,Type type) throws Exception {
		...
		if(data == null) {
			...
			dataString =  Gtc.toJson(data).toJSONString();//JSON.parseObject(data);
		}
	...
	public static<T> T parseResult(String resString,Class<T> clazz,Type type) {
		try {
			if(clazz != null) {
				return  Gtc.toJson(resString).toBean(clazz);//JSONObject.parseObject(resString,clazz);
			}
			if(type != null) {
				return  Gtc.toJson(resString).toBean(type);//JSONObject.parseObject(resString,type);
			}
		}catch(Exception e) {
			log.error(e);
			...
			try {
				...
				String alertMsgKey =  GtcSdkProfileConfig.getConfig( GtcSdkProfileConfig.ProfileLocal.get(),  GtcSdkProfileConfig.BusType_reslut_msgkey,"alertMsg");
				String aMsg =  Gtc.toJson(resString).getByPath(alertMsgKey, String.class);
				if(hasText(aMsg)) {
					alertMsg = aMsg;
				}
				/*
				...
				if(serverResponse != null) {
					String aMsg = serverResponse.getObject( gtcSdkProfileConfig.getConfig( gtcSdkProfileConfig.ProfileLocal.get(),  gtcSdkProfileConfig.BusType_reslut_msgkey,"alertMsg"),String.class);
					if(hasText(aMsg)) {
						alertMsg = aMsg;
					}
				}
			...
			throw new GtcSdkException(alertMsg,e);
		}
		return null;
	}
	...
	public static void parseError(String resString) {
       ...
       String alertMsg = "SDK调用出错";
		...
		throw new GtcSdkException(alertMsg);
	}
	...
	public static void validate(HttpServletRequest request) throws Exception {
		ProfileEnum pe =  GtcSdkProfileConfig.ProfileLocal.get();
		String reqProfile = request.getHeader("Profile");
		if(hasText(reqProfile)) {
			pe = ProfileEnum.valueOf(reqProfile.toUpperCase());
			if(pe == null) {
				throw new GtcSdkException("Sdk请求发送了错误的profile参数:"+reqProfile);
			}
		}

		validateClient(request, GtcSdkProfileConfig.getConfig(pe,  GtcSdkProfileConfig.BusType_clientId), GtcSdkProfileConfig.getConfig(pe,  GtcSdkProfileConfig.BusType_clientSecret));
	}
	...
	public static void validate(HttpServletRequest request, gtcSdkSecretProvider secretProvider) throws Exception {
		...
		if(!hasText(clientSecret)) {
			...
			if(clientSecret == null) {
				throw new GtcSdkException("商户号密钥信息不正确");
			}
	...
	public static void validateClient(HttpServletRequest request,String clientId,String clientSecret) throws Exception {
    	...
    	if(hasText(clientId) && hasText(nonce) && hasText(curTime) && hasText(checkSum)) {
    		if(hasText(clientSecret)) {
    	    	...
    	    	if(Objects.equals(checkSum, myCheckSum)){
    	    		...
    	    		throw new GtcSdkException("非法访问,clientId:" + clientId + "验证密钥不正确");
    	    	}
    		}
    	}
    	throw new GtcSdkException("非法访问");
	}
