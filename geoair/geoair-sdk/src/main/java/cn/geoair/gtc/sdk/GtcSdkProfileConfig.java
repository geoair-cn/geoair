...
public class GtcSdkProfileConfig {
	...
	public static void setConfig(ProfileEnum profile, String busType ,String value) {
		...
		if(map.containsKey(busType)) {
			throw new GtcSdkException("重复的属性注册:profile="+profile.name()+",busType="+busType+",value="+value);
		}
	...
	public static void initialize(String clientId, String clientSecret, ProfileEnum profile) throws GtcSdkException {

		if(clientId == null) {
			throw new GtcSdkException("clientId不能为空");
		}
		if(clientSecret == null) {
			throw new GtcSdkException("clientSecret不能为空");
		}
		if(profile == null) {
			throw new GtcSdkException("profile不能为空");
		}
