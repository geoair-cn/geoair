 ...
 *
 */
public class GkSnowflake implements Serializable {
 ...
 */
	 ...
	 * @param workerId         终端ID
	 * @param dataCenterId     数据中心ID
	 * @param isUseSystemClock 是否使用{@link GkSystemClock} 获取当前时间戳
	 */
	 ...
	 * @since 5.8.0
	 */
	public GkSnowflake(Date epochDate, long workerId, long dataCenterId,
		...
		this.dataCenterId = checkBetween(dataCenterId, 0, MAX_DATA_CENTER_ID);
		this.useSystemClock = isUseSystemClock;
		this.timeOffset = timeOffset;
		this.randomSequenceLimit = checkBetween(randomSequenceLimit, 0, SEQUENCE_MASK);
	}

	private long checkBetween(long value, long min, long max) {
		if (value < min || value > max) {
			throw new IllegalArgumentException(GutilStr.format("The value must be between {} and {}.", min,max));
		}
		return value;
	}
	...
	public synchronized long nextId() {
		...
		if (timestamp < this.lastTimestamp) {
			if (this.lastTimestamp - timestamp < timeOffset) {
				...
				// 如果服务器时间有问题(时钟后退) 报错。
				throw new IllegalStateException(GutilStr.format("Clock moved backwards. Refusing to generate id for {}ms", lastTimestamp - timestamp));
			}
		}
	...
	private long tilNextMillis(long lastTimestamp) {
		...
		if (timestamp < lastTimestamp) {
			...
			throw new IllegalStateException(
					GutilStr.format("Clock moved backwards. Refusing to generate id for {}ms", lastTimestamp - timestamp));
		}
		return timestamp;
	}
