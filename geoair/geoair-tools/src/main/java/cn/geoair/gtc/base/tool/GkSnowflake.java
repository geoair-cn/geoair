...
public class GkSnowflake implements Serializable {
	 ...
	 *
	 * public Snowflake() {
	 * this(IdUtil.getWorkerId(IdUtil.getDataCenterId(MAX_DATA_CENTER_ID),
	 * MAX_WORKER_ID)); }
	 */
	/**
	 * 构造
	 * @param workerId 终端ID
	 *
	 * public Snowflake(long workerId) { this(workerId,
	...
	public synchronized long nextId() {
		...
		if (timestamp < this.lastTimestamp) {
			if (this.lastTimestamp - timestamp < timeOffset) {
				...
				// 如果服务器时间有问题(时钟后退) 报错。
				throw new IllegalStateException(GutilStr
	...
	private long tilNextMillis(long lastTimestamp) {
		...
		if (timestamp < lastTimestamp) {
			...
			throw new IllegalStateException(GutilStr.format("Clock moved backwards. Refusing to generate id for {}ms",
					lastTimestamp - timestamp));
		}
		return timestamp;
	}
