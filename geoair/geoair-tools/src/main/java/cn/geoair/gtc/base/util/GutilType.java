...
import cn.geoair.gtc.base.lang.GkActualTypeMapperPool;
import cn.geoair.gtc.base.lang.GkParameterizedTypeImpl;
...
public class GutilType {
	...
	public static Type getFieldType(Class<?> clazz, String fieldName) {
		return getType(GutilReflection.getField(clazz, fieldName));
	}
	...
	public static ParameterizedType toParameterizedType(Type type) {
		...
		if (type instanceof ParameterizedType) {
			...
			if (null == genericSuper || Object.class.equals(genericSuper)) {
				...
				if (GutilArray.isNotEmpty(genericInterfaces)) {
					// 默认取第一个实现接口的泛型Type
	...
	public static Map<Type, Type> getTypeMap(Class<?> clazz) {
		return GkActualTypeMapperPool.get(clazz);
	}
	...
	public static Type getActualType(Type type, Field field) {
		...
		return getActualType(GutilObject.defaultIfNull(type, field.getDeclaringClass()), field.getGenericType());
	}
	...
	public static Type getActualType(Type type, Type typeVariable) {
		...
		if (typeVariable instanceof TypeVariable) {
			return GkActualTypeMapperPool.getActualType(type, (TypeVariable<?>) typeVariable);
		}
	...
	public static Type getActualType(Type type, ParameterizedType parameterizedType) {
		...
		// 泛型对象中含有未被转换的泛型变量
		if (GutilType.hasTypeVeriable(actualTypeArguments)) {
			actualTypeArguments = getActualTypes(type, parameterizedType.getActualTypeArguments());
			if (GutilArray.isNotEmpty(actualTypeArguments)) {
				// 替换泛型变量为实际类型，例如List<T>变为List<String>
				parameterizedType = new GkParameterizedTypeImpl(actualTypeArguments, parameterizedType.getOwnerType(), parameterizedType.getRawType());
			}
		}
	...
	public static Type[] getActualTypes(Type type, Type... typeVariables) {
		return GkActualTypeMapperPool.getActualTypes(type, typeVariables);
	}
}
