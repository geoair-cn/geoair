...
import cn.geoair.gtc.base.log.GtcConsoleLog;
...
public class Log4Gtc{
	...
	static {
		...
		if(GutilClass.isPresent("org.apache.commons.logging.LogFactory", ApacheCommonsLog.class.getClassLoader())) {
			Log4Gtc.setLogType(LogType.APPACHECOMMONS);
		}else
		if(GutilClass.isPresent("cn.hutool.log.LogFactory", HutoolLog.class.getClassLoader())) {
			Log4Gtc.setLogType(LogType.HUTOOL);
		}else {
			Log4Gtc.setLogType(LogType.CONSOLE);
		}
	}
	...
	@GaMethodHandImpl(implClass= GtcLoger.class,implMethod="getLoger",type=ImplType.expectfirst)
	...
	private static class ApacheCommonsLog implements GiLoger{
		...
		public static GiLoger createLog(String name) {
