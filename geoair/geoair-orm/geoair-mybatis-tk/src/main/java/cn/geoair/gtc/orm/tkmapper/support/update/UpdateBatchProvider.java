package cn.geoair.gtc.orm.tkmapper.support.update;
...
public class UpdateBatchProvider extends MapperTemplate {
	...
	/**
	 * 根据主键选择性批量更新 <foreach collection="list" item="record" separator=";"> update
	 * t_product_base <set>
	 * <if test="record.productSku != null and record.productSku != ''"> product_sku =
	 * #{record.productSku,jdbcType=VARCHAR}, </if> </set> where id =
	 * #{record.id,jdbcType=INTEGER} </foreach>
	 */
