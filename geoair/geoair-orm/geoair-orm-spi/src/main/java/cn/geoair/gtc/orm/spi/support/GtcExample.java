...
public class GtcExample {
     ...
     * @param entityClass
     */
    public GtcExample(Class<?> entityClass) {
     ...
     */
    public GtcExample(Class<?> entityClass, boolean exists) {
     ...
     */
    public GtcExample(Class<?> entityClass, boolean exists, boolean notNull) {
        this.exists = exists;
        this.notNull = notNull;
        oredCriteria = new ArrayList<Criteria>();
        this.entityClass = entityClass;
        table =  GtcEntityHelper.getEntityTable(entityClass);
     ...
     * @param notNull - true时，如果值为空，就会抛出异常，false时，如果为空就不使用该字段的条件
     */
    public GtcExample(boolean exists, boolean notNull) {
        this.exists = exists;
        this.notNull = notNull;
        oredCriteria = new ArrayList<Criteria>();
        // 之类注意要填充这三个参数
...
//        propertyMap = table.getPropertyMap();
    ...
    private GtcExample(Builder builder) {
     ...
     * @return
     */
    public GtcExample excludeProperties(String... properties) {
     ...
     * @return
     */
    public GtcExample selectProperties(String... properties) {
        if (properties != null && properties.length > 0) {
    ...
    public static class OrderBy {
        //属性和列对应
        protected Map<String, GtcEntityColumn> propertyMap;
        private GtcExample gtcExample;
        private Boolean isProperty;

        public OrderBy(GtcExample gtcExample, Map<String, GtcEntityColumn> propertyMap) {
            this. gtcExample =  gtcExample;
            this.propertyMap = propertyMap;
        }

        private String property(String property) {
            if (GutilStr.isEmpty(property) || GutilStr.isEmpty(property.trim())) {
        ...
        public OrderBy orderBy(String property) {
            ...
            if (GutilStr.isNotEmpty( gtcExample.getOrderByClause())) {
                 gtcExample.setOrderByClause( gtcExample.getOrderByClause() + "," + column);
            } else {
                 gtcExample.setOrderByClause(column);
            }
            isProperty = true;
            return this;
        }
        ...
        public OrderBy desc() {
            if (isProperty) {
                 gtcExample.setOrderByClause( gtcExample.getOrderByClause() + " DESC");
                isProperty = false;
            }
            return this;
        }
        ...
        public OrderBy asc() {
            if (isProperty) {
                 gtcExample.setOrderByClause( gtcExample.getOrderByClause() + " ASC");
                isProperty = false;
            }
            return this;
        }
    ...
    protected abstract static class GeneratedCriteria {
        ...
        public Criteria andEqualTo(Object param) {
            Map<String, Field> fieldMap = GutilReflection.getFieldMap(param.getClass());
        ...
        public Criteria andAllEqualTo(Object param) {
            ...
            List<GtcEntityField> fields =  GtcFieldHelper.getFields(param.getClass());
            for ( GtcEntityField field : fields) {
                String property = field.getName();
                //属性和列对应Map中有此属性
                if (propertyMap.get(property) != null) {
                    Object value = GutilReflection.getFieldValue(param, property);
                    //属性值不为空
                    if (value != null) {
                        andEqualTo(property, value);
                    } else {
    ...
    public static class Builder {
        ...
        private List< GtcSqls.Criteria> sqlsCriteria;
        //动态表名
        private List< GtcExample.Criteria> exampleCriterias;
        //动态表名
        ...
        public Builder(Class<?> entityClass, boolean exists, boolean notNull) {
            ...
            this.table =  GtcEntityHelper.getEntityTable(entityClass);
            this.propertyMap = table.getPropertyMap();
            this.sqlsCriteria = new ArrayList< GtcSqls.Criteria>(2);
        }
        ...
        public Builder where( GtcSqls gtcSqls) {
             GtcSqls.Criteria criteria =  gtcSqls.getCriteria();
            criteria.setAndOr("and");
...
//        }
        ...
        public Builder andWhere( GtcSqls gtcSqls) {
             GtcSqls.Criteria criteria =  gtcSqls.getCriteria();
            criteria.setAndOr("and");
            this.sqlsCriteria.add(criteria);
            return this;
...
//        }
        ...
        public Builder orWhere( GtcSqls gtcSqls) {
             GtcSqls.Criteria criteria =  gtcSqls.getCriteria();
            criteria.setAndOr("or");
            this.sqlsCriteria.add(criteria);
            return this;
...
//        }
        ...
        public GtcExample build() {
            this.exampleCriterias = new ArrayList<Criteria>();
            for ( GtcSqls.Criteria criteria : sqlsCriteria) {
                 GtcExample.Criteria exampleCriteria = new  GtcExample.Criteria(this.propertyMap, this.exists, this.notNull);
                exampleCriteria.setAndOr(criteria.getAndOr());
                for ( GtcSqls.Criterion criterion : criteria.getCriterions()) {
            ...
            return new GtcExample(this);
        }

        private void transformCriterion(GtcExample.Criteria exampleCriteria, String condition, String property, Object[] values, String andOr) {
            if (values.length == 0) {
                if ("and".equals(andOr)) {
