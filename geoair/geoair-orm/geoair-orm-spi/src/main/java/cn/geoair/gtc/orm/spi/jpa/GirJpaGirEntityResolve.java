...
public class GirJpaGirEntityResolve implements GirEntityResolve {
	...
	public GirEntityTable resolveEntity(Class<?> entityClass) {
		...
		for (GirEntityField field : fields) {
			...
			if (!field.isAnnotationPresent(Column.class) && !(GirSimpleTypeUtil.isSimpleType(field.getJavaType())
	...
	protected void processField(GirEntityTable girEntityTable, GirEntityField field) {
		...
		if (columnName == null || columnName.equals("")) {
			String entityTableField = Gir.beans.getBean(EntityManagerProvider.class)
		...
		if (field.getJavaType().isPrimitive()) {
			log.warn(
					"警告信息: <[" + girEntityColumn + "]> 使用了基本类型，基本类型在动态 SQL 中由于存在默认值，因此任何时候都不等于 null，建议修改基本类型为对应的包装类型!");
		}
		// OrderBy
	 ...
	 */
	protected void processOrderBy(GirEntityTable girEntityTable, GirEntityField field,
			GirEntityColumn girEntityColumn) {
