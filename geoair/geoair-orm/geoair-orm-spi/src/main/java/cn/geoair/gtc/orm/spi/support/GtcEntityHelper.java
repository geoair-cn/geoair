...
import cn.geoair.gtc.orm.spi.jpa.GtcJpaGtcEntityResolve;
 ...
 * @author zhangjun
 */
public class GtcEntityHelper {

    private static GiLoger logger =  GtcLoger.getLoger( GtcEntityHelper.class);
    /**
     * 实体类 => 表对象
     */
     ...
     * 实体类解析器
     */
    private static GtcEntityResolve resolve = new GtcJpaGtcEntityResolve();
     ...
     * @return
     */
    public static GtcEntityTable getEntityTable(Class<?> entityClass, GtcEntityResolve resolve) {
     ...
     * @return
     */
    public static GtcEntityTable updateEntityTable(Class<?> entityClass, GtcEntityResolve resolve) {
         GtcEntityTable gtcEntityTable = resolve.resolveEntity(entityClass);
        if ( gtcEntityTable == null) {
            throw new RuntimeException("无法通过该解析器" + resolve.getClass().getName() + "获取实体对象!");
        } else {
    ...
    public static void setResolve( GtcEntityResolve resolve) {
         GtcEntityHelper.resolve = resolve;
    }
}
