    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if ("update".equals(invocation.getMethod().getName())) {
            ...
            if (sqlType == SqlCommandType.INSERT) {
            	 gtcModelInserter.beforeInsertModel(parameter,entityClass);
            	Object obj = invocation.proceed();
            	 gtcModelInserter.afterInsertModel(parameter,entityClass,obj);
            	return obj;
            }else if(sqlType == SqlCommandType.UPDATE) {
            	 gtcModelUpdater.beforeUpdateModel(parameter,entityClass);
            	Object obj = invocation.proceed();
            	 gtcModelUpdater.afterUpdateModel(parameter,entityClass,obj);
            	return obj;
            }else if(sqlType == SqlCommandType.DELETE) {
            	 gtcModelDeleter.beforeDeleteModel(parameter,entityClass);
            	Object obj = invocation.proceed();
            	 gtcModelDeleter.afterDeleteModel(parameter,entityClass,obj);
            	return obj;
            }
        }
        */
